...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Include rustc_complete module
    code.push_str("// Include our rustc_complete module\n");
    code.push_str("#[path = \"src/rustc_complete.rs\"]\n");
    code.push_str("mod rustc_complete;\n\n");
    
    // Extern crates
    code.push_str("// Add extern crate declarations needed for rustc_driver\n");
    ...
    // Add resolver macros
    code.push_str("// Resolver macros to use processed rustc types\n");
    for crate_name in &crate_order {
        code.push_str(&format!("use_crate_{}!();\n", crate_name));
    }
    code.push_str("\n");
    
    // Main function
    code.push_str("fn main() {\n");
    code.push_str(&format!("    println!(\"🚀 Unified rustc wrapper with {} crates loaded\");\n", crate_order.len()));
