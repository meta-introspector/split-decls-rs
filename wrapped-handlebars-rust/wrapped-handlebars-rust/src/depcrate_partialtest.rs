...
" ; let mut hbs = Registry :: new () ; hbs . register_template_string ("inner" , inner) . unwrap () ; hbs . register_template_string ("outer" , outer) . unwrap () ; let result = hbs . render ("outer" , & json ! ({ "inner_solo" : { "name" : "inner_solo" } , "inners" : [{ "name" : "hello" } , { "name" : "there" }] }) ,) . unwrap () ; assert_eq ! (result , r"                name: inner_solo
        ...
        name: hello
        name: there
") ; } # [test] fn test_partial_prevent_indent () { let outer = r"                {{> inner inner_solo}}
...
{{/each}}
        ...
        {{/each}}
" ; let inner = r"name: {{name}}
" ; let mut hbs = Registry :: new () ; hbs . set_prevent_indent (true) ; hbs . register_template_string ("inner" , inner) . unwrap () ; hbs . register_template_string ("outer" , outer) . unwrap () ; let result = hbs . render ("outer" , & json ! ({ "inner_solo" : { "name" : "inner_solo" } , "inners" : [{ "name" : "hello" } , { "name" : "there" }] }) ,) . unwrap () ; assert_eq ! (result , r"                name: inner_solo
        ...
        name: hello
        name: there
") ; } # [test] fn test_nested_partials () { let mut hb = Registry :: new () ; hb . register_template_string ("partial" , "{{> @partial-block}}") . unwrap () ; hb . register_template_string ("index" , r"{{#>partial}}
    Yo
    {{#>partial}}
    Yo 2
    {{/partial}}
{{/partial}}" ,) . unwrap () ; assert_eq ! (r"    Yo
    Yo 2
" , hb . render ("index" , & ()) . unwrap ()) ; hb . register_template_string ("partial2" , "{{> @partial-block}}") . unwrap () ; let r2 = hb . render_template (r"{{#> partial}}
{{#> partial2}}
:(
{{/partial2}}
{{/partial}}" , & () ,) . unwrap () ; assert_eq ! (":(\n" , r2) ; } # [test] fn test_partial_context_issue_495 () { let mut hb = Registry :: new () ; hb . register_template_string ("t1" , r#"{{~#*inline "displayName"~}}
Template:{{name}}
{{/inline}}
{{#each data as |name|}}
Name:{{name}}
{{>displayName name="aaaa"}}
{{/each}}"# ,) . unwrap () ; hb . register_template_string ("t2" , r#"{{~#*inline "displayName"~}}
Template:{{this}}
{{/inline}}
{{#each data as |name|}}
Name:{{name}}
{{>displayName}}
{{/each}}"# ,) . unwrap () ; let data = json ! ({ "data" : ["hudel" , "test"] }) ; assert_eq ! (r"Name:hudel
Template:aaaa
Name:test
Template:aaaa
" , hb . render ("t1" , & data) . unwrap ()) ; assert_eq ! (r"Name:hudel
Template:hudel
Name:test
Template:test
" , hb . render ("t2" , & data) . unwrap ()) ; } # [test] fn test_multiline_partial_indent () { let mut hb = Registry :: new () ; hb . register_template_string ("t1" , r#"{{#*inline "thepartial"}}
  inner first line
  inner second line
{{/inline}}
  {{> thepartial}}
outer third line"# ,) . unwrap () ; assert_eq ! (r"    inner first line
    inner second line
outer third line" , hb . render ("t1" , & ()) . unwrap ()) ; hb . register_template_string ("t2" , r#"{{#*inline "thepartial"}}inner first line
inner second line
{{/inline}}
  {{> thepartial}}
outer third line"# ,) . unwrap () ; assert_eq ! (r"  inner first line
  inner second line
outer third line" , hb . render ("t2" , & ()) . unwrap ()) ; hb . register_template_string ("t3" , r#"{{#*inline "thepartial"}}{{a}}{{/inline}}
  {{> thepartial}}
outer third line"# ,) . unwrap () ; assert_eq ! (r"
  inner first line
  inner second lineouter third line" , hb . render ("t3" , & json ! ({ "a" : "inner first line\ninner second line" })) . unwrap ()) ; hb . register_template_string ("t4" , r#"{{#*inline "thepartial"}}
  inner first line
  inner second line
{{/inline}}
  {{~> thepartial}}
outer third line"# ,) . unwrap () ; assert_eq ! (r"  inner first line
  inner second line
outer third line" , hb . render ("t4" , & ()) . unwrap ()) ; let mut hb2 = Registry :: new () ; hb2 . set_prevent_indent (true) ; hb2 . register_template_string ("t1" , r#"{{#*inline "thepartial"}}
  inner first line
  inner second line
{{/inline}}
  {{> thepartial}}
outer third line"# ,) . unwrap () ; assert_eq ! (r"    inner first line
  inner second line
outer third line" , hb2 . render ("t1" , & ()) . unwrap ()) ; } # [test] fn test_indent_level_on_nested_partials () { let nested_partial = "
<div>
    content
</div>
" ; let partial = "
...
" ; let mut hb = Registry :: new () ; hb . register_template_string ("nested_partial" , nested_partial . trim_start ()) . unwrap () ; hb . register_template_string ("partial" , partial . trim_start ()) . unwrap () ; hb . register_template_string ("partial_indented" , partial_indented . trim_start ()) . unwrap () ; let s = hb . render ("partial_indented" , & ()) . unwrap () ; assert_eq ! (& s , result . trim_start ()) ; } # [test] fn test_issue_534 () { let t1 = "{{title}}" ; let t2 = "{{#each modules}}{{> (lookup this \"module\") content name=0}}{{/each}}" ; let data = json ! ({ "modules" : [{ "module" : "t1" , "content" : { "title" : "foo" } } , { "module" : "t1" , "content" : { "title" : "bar" } } ,] }) ; let mut hbs = Registry :: new () ; hbs . register_template_string ("t1" , t1) . unwrap () ; hbs . register_template_string ("t2" , t2) . unwrap () ; assert_eq ! ("foobar" , hbs . render ("t2" , & data) . unwrap ()) ; } # [test] fn test_partial_not_found () { let t1 = "{{> bar}}" ; let hbs = Registry :: new () ; assert ! (hbs . render_template (t1 , & ()) . is_err ()) ; } # [test] fn test_issue_643_this_context () { let t1 = "{{this}}" ; let t2 = "{{> t1 \"hello world\"}}" ; let mut hbs = Registry :: new () ; hbs . register_template_string ("t1" , t1) . unwrap () ; hbs . register_template_string ("t2" , t2) . unwrap () ; assert_eq ! ("hello world" , hbs . render ("t2" , & ()) . unwrap ()) ; let t1 = "{{a}} {{[0]}} {{[1]}}" ; let t2 = "{{> t1 \"hello world\" a=1}}" ; let mut hbs = Registry :: new () ; hbs . register_template_string ("t1" , t1) . unwrap () ; hbs . register_template_string ("t2" , t2) . unwrap () ; assert_eq ! ("1 h e" , hbs . render ("t2" , & ()) . unwrap ()) ; let t1 = "{{#each this}}{{@key}}:{{this}},{{/each}}" ; let t2 = "{{> t1 a=1}}" ; let mut hbs = Registry :: new () ; hbs . register_template_string ("t1" , t1) . unwrap () ; hbs . register_template_string ("t2" , t2) . unwrap () ; assert_eq ! ("a:1," , hbs . render ("t2" , & ()) . unwrap ()) ; let t1 = "{{#each this}}{{@key}}:{{this}},{{/each}}" ; let t2 = "{{> t1 a=1}}" ; let mut hbs = Registry :: new () ; hbs . register_template_string ("t1" , t1) . unwrap () ; hbs . register_template_string ("t2" , t2) . unwrap () ; assert_eq ! ("a:1,b:2," , hbs . render ("t2" , & json ! ({ "b" : 2 })) . unwrap ()) ; let t1 = "{{#each this}}{{@key}}:{{this}},{{/each}}" ; let t2 = "{{> t1 b a=1}}" ; let mut hbs = Registry :: new () ; hbs . register_template_string ("t1" , t1) . unwrap () ; hbs . register_template_string ("t2" , t2) . unwrap () ; assert_eq ! ("a:1," , hbs . render ("t2" , & json ! ({ "b" : 2 })) . unwrap ()) ; } }
};
}
