// Generated by add_generated_rust_header! macro at submodules/split-decls-rs/src/eager_splitter/declaration_writer.rs:71 from submodules/split-decls-rs/src/eager_splitter/declaration_writer.rs at 2025-12-23T20:00:22.599978625+00:00
...
pub fn handle_single_crate_wrapping(
    ...
    let workspace_dependencies_output_map =
        collect_and_format_workspace_dependencies(global_config, output_dir, scan_root)?;
    let mut process_dep_group =
        |deps_source: Option<&Table>, output_map: &mut HashMap<String, Value>| -> Result<()> {
                        ...
                        if let Some(dep_table) = dep_value_clone.as_table_mut() {
                            if let Some(path_value) = dep_table.get("path") {
                                if let Some(path_str) = path_value.as_str() {
                                    ...
                                    let relative_path =
                                        path_diff::path_diff(output_dir, &absolute_dep_path)
                                            .context(format!(
                ...
                for (key, value) in bin_table.iter() {
                    if key == "path" {
                        if let Some(path_str) = value.as_str() {
                            ...
                            let relative_bin_path =
                                path_diff::path_diff(output_dir, &absolute_bin_path).context(
                                    format!(
