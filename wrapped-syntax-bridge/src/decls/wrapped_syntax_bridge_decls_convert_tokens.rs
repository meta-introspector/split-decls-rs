    ...
    C::Token: fmt::Debug,
{
    let mut builder = tt::TopSubtreeBuilder::new(
        tt::Delimiter::invisible_spanned(conv.call_site()),
    );
    while let Some((token, abs_range)) = conv.bump() {
        let tt = match token.as_leaf() {
            Some(
                ...
                let found_expected_delimiter = builder
                    .expected_delimiters()
                    .enumerate()
                    .find(|(_, delim)| match delim.kind {
                ...
                match token.kind(conv) {
                    ...
                    kind => {
                        macro_rules! make_ident {
                            () => {
                                tt::Ident { span : conv.span_for(abs_range), sym :
                                Symbol::intern(& token.to_text(conv)), is_raw :
                                tt::IdentIsRaw::No, } .into()
                            };
                        }
                        let leaf: tt::Leaf<_> = match kind {
                            ...
                            LIFETIME_IDENT => {
                                let apostrophe = tt::Leaf::from(tt::Punct {
                                    ...
                                    span: conv
                                        .span_for(
                                            TextRange::at(abs_range.start(), TextSize::of('\'')),
                                        ),
                                });
