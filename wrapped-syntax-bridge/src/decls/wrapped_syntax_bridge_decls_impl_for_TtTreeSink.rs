    ...
    Ctx: PartialEq,
{
    ...
    fn float_split(&mut self, has_pseudo_dot: bool) {
        ...
        match text.split_once('.') {
            Some((left, right)) => {
                ...
                self.token_map
                    .push(self.text_pos + TextSize::of(left) + TextSize::of("."), span);
                if has_pseudo_dot {
    ...
    fn token(&mut self, kind: SyntaxKind, mut n_tokens: u8) {
                    ...
                    Some(tt::TokenTree::Leaf(leaf)) => {
                        match leaf {
                            ...
                            tt::Leaf::Literal(lit) => {
                                ...
                                self.text_pos
                                    += TextSize::new((self.buf.len() - buf_l) as u32);
                                combined_span = match combined_span {
                                    None => Some(lit.span),
                                    Some(prev_span) => {
                                        Some(Self::merge_spans(prev_span, lit.span))
                                    }
                                };
                                self.cursor.bump();
                                continue 'tokens;
        ...
        self.token_map
            .push(self.text_pos, combined_span.expect("expected at least one token"));
