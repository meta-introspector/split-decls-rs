...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 3 {
        println!("Usage: {} <input_dir> <output_file>", args[0]);
        return Ok(());
    }
    ...
    fn scan_directory(dir: &str, signatures: &mut HashMap<String, u64>, processed: &mut usize) -> Result<(), Box<dyn std::error::Error>> {
        for entry in fs::read_dir(dir)? {
            let entry = entry?;
            let path = entry.path();
            
            if path.is_dir() {
                scan_directory(path.to_str().unwrap(), signatures, processed)?;
    ...
    scan_directory(input_dir, &mut signature_frequencies, &mut processed_files)?;
