pub fn try_match_macro_attr<'matcher, T: Tracker<'matcher>>(
    psess: &ParseSess,
    name: Ident,
    ...
    let args_parser = macro_rules_utils::parser_from_cx(psess, attr_args.clone(), T::recovery());
    let body_parser = macro_rules_utils::parser_from_cx(psess, attr_body.clone(), T::recovery());
    ...
    for (i, rule) in rules.iter().enumerate() {
        ...
        let mut named_matches = match result {
            mbe_macro_parser::ParseResult::Success(named_matches) => named_matches,
        ...
        match result {
            mbe_macro_parser::ParseResult::Success(body_named_matches) => {
