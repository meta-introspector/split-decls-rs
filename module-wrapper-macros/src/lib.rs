...
/// Usage:
/// ```rust
/// includemod!(my_module, "path/to/file.rs");
/// ```
#[proc_macro]
pub fn includemod(input: TokenStream) -> TokenStream {
    let input = input.to_string();
    let parts: Vec<&str> = input.split(',').map(|s| s.trim()).collect();
    
    if parts.len() != 2 {
        panic!("includemod! expects exactly 2 arguments: module_name, file_path");
    }
    
    let mod_name = parts[0];
    let file_path = parts[1].trim_matches('"');
    
    let mod_ident = syn::parse_str::<Ident>(mod_name).expect("Invalid module name");
