...
mod t {
    ...
    mod mock {
        ...
        ";
            ...
            assert_not_contains(
                &output,
                quote!(
                    self.Bar_expectations.checkpoint(); self.Bar_expectations
                    .checkpoint();
                ),
            );
            assert_contains(
                &output,
                quote!(
                    pub fn expect_bar(& mut self) -> & mut
                    __mock_MockFoo_Bar::__bar::Expectation < u32 >
                ),
            );
            assert_contains(
                &output,
                quote!(
                    pub fn expect_bar(& mut self) -> & mut
                    __mock_MockFoo_Bar::__bar::Expectation < i32 >
                ),
            );
        }
    }
    ...
    mod concretize_args {
        ...
        fn function_args() {
            check_concretize(
                quote!(
                    fn foo < F1 : Fn(u32) -> u32, F2 : FnMut(& mut u32) -> u32, F3 :
                    FnOnce(u32) -> u32, F4 : Fn() + Send > (f1 : F1, f2 : F2, f3 : F3, f4
                    : F4)
                ),
                &[
