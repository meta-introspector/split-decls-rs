...
#[doc = " replace generic arguments with concrete trait object arguments"]
#[doc = ""]
#[doc = " # Return"]
#[doc = ""]
#[doc = " * A Generics object with the concretized types removed"]
#[doc = " * An array of transformed argument types, suitable for matchers and"]
#[doc = "   returners"]
#[doc = " * An array of expressions that should be passed to the `call` function."]
fn concretize_args(
    gen: &Generics,
    sig: &Signature,
) -> (
    ...
    let outargs = args
        ...
        .collect();
    let call_exprs = args . iter () . filter_map (| arg | { match arg { FnArg :: Typed (pt) => { let mut pt2 = pt . clone () ; demutify_arg (& mut pt2) ; let pat = & pt2 . pat ; if pat_is_self (pat) { None } else if let Some ((_ , newbound)) = hm . get (& pt . ty) { if let Type :: Reference (tr) = & * pt . ty { if let Type :: Slice (_ts) = & * tr . elem { Some (quote ! (& (0 ..# pat . len ()) . map (| __mockall_i | &# pat [__mockall_i] as & (dyn # newbound)) . collect ::< Vec < _ >> ())) } else { Some (quote ! (# pat)) } } else if needs_muts . get (& pt . ty) . cloned () . unwrap_or (false) { Some (quote ! (& mut # pat)) } else { Some (quote ! (&# pat)) } } else { Some (quote ! (# pat)) } } , FnArg :: Receiver (_) => None , } }) . collect () ;
        ...
        if let FnArg::Typed(pt) = arg {
            if needs_muts.get(&pt.ty).cloned().unwrap_or(false) {
                if let Pat::Ident(pi) = &mut *pt.pat {
                    ...
                    compile_error (pt . pat . span () , "This Pat type is not yet supported by Mockall when used as an argument to a concretized function.")
                }
            }
        }
    }
