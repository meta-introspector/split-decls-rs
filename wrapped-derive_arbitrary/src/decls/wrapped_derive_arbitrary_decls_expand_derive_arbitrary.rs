...
fn expand_derive_arbitrary(input: syn::DeriveInput) -> Result<TokenStream> {
    ...
    let recursive_count = needs_recursive_count . then (| | { Some (quote ! { :: std :: thread_local ! { # [allow (non_upper_case_globals)] static # recursive_count : :: core :: cell :: Cell < u32 > = const { :: core :: cell :: Cell :: new (0) } ; } }) }) ;
    Ok(
        quote! { const _ : () = { # recursive_count # [automatically_derived] impl # impl_generics arbitrary :: Arbitrary <# lifetime_without_bounds > for # name # ty_generics # where_clause { # arbitrary_method # size_hint_method } } ; },
    )
}
