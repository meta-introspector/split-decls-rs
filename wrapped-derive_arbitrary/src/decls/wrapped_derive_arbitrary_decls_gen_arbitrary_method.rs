...
fn gen_arbitrary_method(
    ...
    fn arbitrary_structlike(
        ...
        let body = quote! { arbitrary :: details :: with_recursive_count (u , &# recursive_count , | mut u | { Ok (# ident # arbitrary) }) };
        let arbitrary_take_rest = construct_take_rest(fields)?;
        let take_rest_body = quote! { arbitrary :: details :: with_recursive_count (u , &# recursive_count , | mut u | { Ok (# ident # arbitrary_take_rest) }) };
        Ok(
            quote! { fn arbitrary (u : & mut arbitrary :: Unstructured <# lifetime >) -> arbitrary :: Result < Self > { # body } fn arbitrary_take_rest (mut u : arbitrary :: Unstructured <# lifetime >) -> arbitrary :: Result < Self > { # take_rest_body } },
        )
    }
    ...
    fn arbitrary_enum_method(
        ...
        let do_variants = quote! { Ok (match (u64 :: from (< u32 as arbitrary :: Arbitrary >:: arbitrary (# unstructured) ?) * # count) >> 32 { # (# variants ,) * _ => unreachable ! () }) };
        if needs_recursive_count {
            quote! { arbitrary :: details :: with_recursive_count (u , &# recursive_count , | mut u | { # do_variants }) }
        } else {
            do_variants
        }
    }
    fn arbitrary_enum(
        ...
        let variants_take_rest = enumerated_variants
            ...
            .collect::<Result<Vec<TokenStream>>>()?;
        (! variants . is_empty ()) . then (| | { let arbitrary = arbitrary_enum_method (recursive_count , quote ! { u } , & variants , needs_recursive_count ,) ; let arbitrary_take_rest = arbitrary_enum_method (recursive_count , quote ! { & mut u } , & variants_take_rest , needs_recursive_count ,) ; (quote ! { fn arbitrary (u : & mut arbitrary :: Unstructured <# lifetime >) -> arbitrary :: Result < Self > { # arbitrary } fn arbitrary_take_rest (mut u : arbitrary :: Unstructured <# lifetime >) -> arbitrary :: Result < Self > { # arbitrary_take_rest } } , needs_recursive_count ,) }) . ok_or_else (| | { Error :: new_spanned (enum_name , "Enum must have at least one variant, that is not skipped" ,) })
    }
