...
fn gen_arbitrary_method(
    ...
    fn arbitrary_variant(
        ...
        quote! {
            # index => # enum_name::# variant_name # ctor
        }
    }
    fn arbitrary_enum_method(
        ...
        let do_variants = quote! {
            Ok(match (u64::from(< u32 as arbitrary::Arbitrary >::arbitrary(#
            unstructured) ?) * # count) >> 32 { # (# variants,) * _ => unreachable!() })
        };
        ...
        (!variants.is_empty())
            .then(|| {
                ...
                (
                    quote! {
                        fn arbitrary(u : & mut arbitrary::Unstructured <# lifetime >) ->
                        arbitrary::Result < Self > { # arbitrary } fn
                        arbitrary_take_rest(mut u : arbitrary::Unstructured <# lifetime
                        >) -> arbitrary::Result < Self > { # arbitrary_take_rest }
                    },
                    needs_recursive_count,
                )
            })
