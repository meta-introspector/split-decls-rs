...
macro_rules! Depcrate_http_playground_sourceplayground_source {
() => {
// Module: crate::http::playground_source
// Provides: {"playground_source"}
// Dependencies: {}
# [doc = " Generate the page for GraphQL Playground"] # [doc = ""] # [doc = " # Example"] # [doc = ""] # [doc = " ```rust"] # [doc = " use async_graphql::http::*;"] # [doc = ""] # [doc = " playground_source(GraphQLPlaygroundConfig::new(\"http://localhost:8000\"));"] # [doc = " ```"] pub fn playground_source (config : GraphQLPlaygroundConfig) -> String { let title = config . title . unwrap_or ("GraphQL Playground") ; r##"
<!DOCTYPE html>
...
<body>
  ...
  "## . replace ("GRAPHQL_PLAYGROUND_CONFIG" , & match serde_json :: to_string (& config) { Ok (str) => str , Err (_) => "{}" . to_string () }) . replace ("%GRAPHQL_PLAYGROUND_TITLE%" , title) }
};
}
