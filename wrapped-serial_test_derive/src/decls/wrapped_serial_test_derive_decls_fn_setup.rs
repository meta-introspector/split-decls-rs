    ...
    if let Some(ret) = return_type {
        match asyncness {
            Some(_) => {
                ...
                quote! { # (# attrs) * # vis async fn # name () -> # ret { async fn # temp_fn () -> # ret # block # print_name # (# crate_ident) *::# fnname (vec ! [# (# names) ,*] , # path , # temp_fn ()) . await } }
            }
            None => {
                let fnname = format_ident!("{}_{}_core_with_return", prefix, kind);
                quote! { # (# attrs) * # vis fn # name () -> # ret { # print_name # (# crate_ident) *::# fnname (vec ! [# (# names) ,*] , # path , || # block) } }
            }
        }
    } else {
        match asyncness {
            Some(_) => {
                ...
                let temp_fn = format_ident!("_{}_internal", name);
                quote! { # (# attrs) * # vis async fn # name () { async fn # temp_fn () # block # print_name # (# crate_ident) *::# fnname (vec ! [# (# names) ,*] , # path , # temp_fn ()) . await ; } }
            }
            None => {
                let fnname = format_ident!("{}_{}_core", prefix, kind);
                quote! { # (# attrs) * # vis fn # name () { # print_name # (# crate_ident) *::# fnname (vec ! [# (# names) ,*] , # path , || # block) ; } }
            }
        }
    }
}
