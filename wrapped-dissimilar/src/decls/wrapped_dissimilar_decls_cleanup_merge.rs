...
fn cleanup_merge(solution: &mut Solution) {
    ...
    while !diffs.is_empty() {
            ...
            match this_diff {
                ...
                Diff::Equal(text, _) => {
                    ...
                    if count_both > 1 {
                            ...
                            if common_length != 0 {
                                if pointer > 0 {
                                    match &mut diffs[pointer - 1] {
                                        ...
                                        _ => {
                                            unreachable!("previous diff should have been an equality")
                                        }
                                    }
                                } else {
                                    diffs
                        ...
                        if let Some(Diff::Equal(prev_equal1, prev_equal2)) = diffs
                            .get_mut(pointer - 1)
                        {
        ...
        while let Some(&next_diff) = diffs.get(pointer + 1) {
            ...
            if let (Diff::Equal(prev_diff, _), Diff::Equal(next_diff, _)) = (
                prev_diff,
                next_diff,
            ) {
