...
impl DiagMessage {
    #[doc = " Given a `SubdiagMessage` which may contain a Fluent attribute, create a new"]
    #[doc = " `DiagMessage` that combines that attribute with the Fluent identifier of `self`."]
    #[doc = ""]
    #[doc = " - If the `SubdiagMessage` is non-translatable then return the message as a `DiagMessage`."]
    #[doc = " - If `self` is non-translatable then return `self`'s message."]
