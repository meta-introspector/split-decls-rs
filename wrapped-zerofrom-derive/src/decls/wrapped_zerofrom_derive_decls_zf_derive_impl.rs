...
fn zf_derive_impl(input: &DeriveInput) -> TokenStream2 {
    ...
    let may_borrow_attrs = match get_may_borrow_attr(&input.attrs) {
        Ok(mb) => mb,
        Err(span) => {
            return syn :: Error :: new (span , "#[zerofrom(may_borrow)] on the struct takes in a comma separated list of type parameters, like so: `#[zerofrom(may_borrow(A, B, C, D)]`" ,) . to_compile_error () ;
        }
    };
    ...
    if lts == 0 && !generics_may_borrow {
        ...
        let bounds: Vec<WherePredicate> = typarams
            ...
            .collect();
        quote! { impl <'zf , # (# tybounds) ,*> zerofrom :: ZeroFrom <'zf , # name <# (# typarams) ,*>> for # name <# (# typarams) ,*> where # (# bounds) ,* { fn zero_from (this : &'zf Self) -> Self { # clone } } }
    } else {
        if lts > 1 {
        ...
        let body = structure . each_variant (| vi | { vi . construct (| f , i | { let binding = format ! ("__binding_{i}") ; let field = Ident :: new (& binding , Span :: call_site ()) ; if has_attr (& f . attrs , "clone") { quote ! { # field . clone () } } else { let fty = replace_lifetime (& f . ty , custom_lt ("'zf")) ; let lifetime_ty = replace_lifetime_and_type (& f . ty , custom_lt ("'zf_inner") , & generics_env) ; let (has_ty , has_lt) = visitor :: check_type_for_parameters (& f . ty , & generics_env) ; if has_ty { if has_lt { zf_bounds . push (parse_quote ! (# fty : zerofrom :: ZeroFrom <'zf , # lifetime_ty >)) ; } else { zf_bounds . push (parse_quote ! (# fty : zerofrom :: ZeroFrom <'zf , # fty >)) ; } } if has_ty || has_lt { quote ! { <# fty as zerofrom :: ZeroFrom <'zf , # lifetime_ty >>:: zero_from (# field) } } else { quote ! { *# field } } } }) }) ;
        ...
        if generics_may_borrow {
            for typaram_c in &mut typarams_c {
                if let Some(Some(replacement)) = generics_env.get(typaram_c) {
                    ...
                    zf_bounds . push (parse_quote ! (# typaram_c : zerofrom :: ZeroFrom <'zf_inner , # typaram_t >)) ;
                    tybounds.push(parse_quote ! (# typaram_c));
                }
            }
        }
        quote! { impl <'zf , 'zf_inner , # (# tybounds) ,*> zerofrom :: ZeroFrom <'zf , # name <# maybe_zf_inner_lifetime # (# typarams_c) ,*>> for # name <# maybe_zf_lifetime # (# typarams) ,*> where # (# zf_bounds ,) * { fn zero_from (this : &'zf # name <# maybe_zf_inner_lifetime # (# typarams_c) ,*>) -> Self { match * this { # body } } } }
    }
}
