// Generated macro for can_read_safari_client_hello_with_ip_address_in_sni_extension (function)
macro_rules! Depcrate_msgs_message_testcan_read_safari_client_hello_with_ip_address_in_sni_extension {
() => {
// Module: crate::msgs::message_test
// Provides: {"can_read_safari_client_hello_with_ip_address_in_sni_extension"}
// Dependencies: {}
# [test] fn can_read_safari_client_hello_with_ip_address_in_sni_extension () { let _ = env_logger :: Builder :: new () . filter (None , log :: LevelFilter :: Trace) . try_init () ; let bytes = b"\
        ...
        \x01\x00\x00\x0a\x00\x0a\x00\x08\x00\x1d\x00\x17\x00\x18\x00\x19" ; let mut rd = Reader :: init (bytes) ; let m = PlainMessage :: read (& mut rd) . unwrap () ; println ! ("m = {m:?}") ; Message :: try_from (m) . unwrap () ; }
};
}
