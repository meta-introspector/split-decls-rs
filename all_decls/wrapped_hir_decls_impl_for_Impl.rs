...
impl Impl {
    ...
    pub fn all_for_type<'db>(
        ...
        let mut extend_with_impls = |impls: &[ImplId]| {
            result.extend(impls.iter().copied().map(Impl::from))
        };
        extend_with_impls(
            method_resolution::incoherent_inherent_impls(db, simplified_ty),
        );
        if let Some(module) = method_resolution::simplified_type_module(
            ...
            std::iter::successors(
                    module.containing_block(),
                    |block| { block.loc(db).module.containing_block() },
                )
                .filter_map(|block| TraitImpls::for_block(db, block).as_deref())
                .for_each(|impls| {
                    impls.for_self_ty(&simplified_ty, &mut extend_with_impls)
                });
    ...
    pub fn all_for_trait(db: &dyn HirDatabase, trait_: Trait) -> Vec<Impl> {
        ...
        let mut handle_impls = |impls: &TraitImpls| {
            impls
                .for_trait(
                    trait_.id,
                    |impls| all.extend(impls.iter().copied().map(Impl::from)),
                );
        };
