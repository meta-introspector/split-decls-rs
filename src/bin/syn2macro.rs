...
fn main() -> Result<()> {
    ...
    // Create security context - using enum variants
    let security = match security_level.as_str() {
        "strict" => split_decls_rs::syn2macro::SecurityContext::Strict(vec![
            AstOperation::ParseItem,
            AstOperation::TransformItem,
            AstOperation::GenerateCode,
        ]),
        _ => split_decls_rs::syn2macro::SecurityContext::Default,
    };
