...
use crate::rustc_hir::{Pat, PatKind};
use crate::rustc_middle::bug;
use crate::rustc_middle::lint::LevelAndSource;
use crate::rustc_middle::middle::privacy::EffectiveVisibilities;
use crate::rustc_middle::ty::layout::{LayoutError, LayoutOfHelpers, TyAndLayout};
use crate::rustc_middle::ty::print::{PrintError, PrintTraitRefExt as _, Printer, with_no_trimmed_paths};
use crate::rustc_middle::ty::{self, GenericArg, RegisteredTools, Ty, TyCtxt, TypingEnv, TypingMode};
use crate::rustc_session::lint::{FutureIncompatibleInfo, Lint, LintExpectationId, LintId};
use crate::rustc_session::{DynLintStore, Session};
use crate::rustc_span::edit_distance::find_best_match_for_names;
...
impl DynLintStore for LintStore {
    fn lint_groups_iter(&self) -> Box<dyn Iterator<Item = crate::rustc_session::LintGroup> + '_> {
        Box::new(self.get_lint_groups().map(|(name, lints, is_externally_loaded)| {
            crate::rustc_session::LintGroup { name, lints, is_externally_loaded }
        }))
    }
}
