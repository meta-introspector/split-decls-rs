...
use rustc_driver_impl::session_diagnostics::{
...
pub static DEFAULT_LOCALE_RESOURCES: &[&str] = &[
    // tidy-alphabetical-start
    rustc_driver_impl::DEFAULT_LOCALE_RESOURCE,
    ...
    for req in &sess.opts.prints {
        ...
        match req.kind {
            ...
            CrateRootLintLevels => {
                ...
                let crate_name = passes::get_crate_name(sess, attrs);
                let lint_store = rustc_driver_impl::unerased_lint_store(sess);
            ...
            CheckCfg => {
                ...
                for (name, expected_values) in &sess.psess.check_config.expecteds {
                    use rustc_driver_impl::config::ExpectedValues;
...
pub macro version($early_dcx: expr, $binary: literal, $matches: expr) {
    ...
    $rustc_driver_impl::version_at_macro_invocation(
        $early_dcx,
        $binary,
        $matches,
...
fn report_ice(
    ...
    let file = if let Some(path) = ice_path() {
        ...
        match rustc_driver_impl::fs::File::options().create(true).append(true).open(&path) {
            Ok(mut file) => {
