...
pub(super) fn type_decodable_derive(
    ...
    s.add_impl_generic(parse_quote! { #decoder_ty: ::crate::rustc_middle::ty::codec::TyDecoder<'tcx> });
...
pub(super) fn decodable_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    let decoder_ty = quote! { __D };
    s.add_impl_generic(parse_quote! { #decoder_ty: ::crate::rustc_span::SpanDecoder });
    s.add_bounds(synstructure::AddBounds::Generics);
...
fn decode_field(field: &syn::Field) -> proc_macro2::TokenStream {
    ...
    let decode_inner_method = if let syn::Type::Reference(_) = field.ty {
        quote! { ::crate::rustc_middle::ty::codec::RefDecodable::decode }
    } else {
...
pub(super) fn type_encodable_derive(
    ...
    s.add_impl_generic(parse_quote! { #encoder_ty: ::crate::rustc_middle::ty::codec::TyEncoder<'tcx> });
    s.add_bounds(synstructure::AddBounds::Fields);
...
pub(super) fn encodable_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    let encoder_ty = quote! { __E };
    s.add_impl_generic(parse_quote! { #encoder_ty: ::crate::rustc_span::SpanEncoder });
    s.add_bounds(synstructure::AddBounds::Generics);
