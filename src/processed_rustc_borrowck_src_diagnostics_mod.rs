...
impl UseSpans<'_> {
    ...
    pub(super) fn var_subdiag(
        self,
        err: &mut Diag<'_>,
        kind: Option<crate::rustc_middle::mir::BorrowKind>,
        ...
        if let UseSpans::ClosureUse { closure_kind, capture_kind_span, path_span, .. } = self {
            if capture_kind_span != path_span {
                err.subdiagnostic(match kind {
                    Some(kd) => match kd {
                        crate::rustc_middle::mir::BorrowKind::Shared
                        | crate::rustc_middle::mir::BorrowKind::Fake(_) => {
                            CaptureVarKind::Immut { kind_span: capture_kind_span }
                        }

                        crate::rustc_middle::mir::BorrowKind::Mut { .. } => {
        ...
        if let UseSpans::FnSelfUse { var_span, fn_call_span, fn_span, kind } = move_spans {
            ...
            match kind {
                ...
                CallKind::Normal { self_arg, desugaring, method_did, method_args } => {
                    ...
                    let parent_self_ty =
                        matches!(tcx.def_kind(parent_did), crate::rustc_hir::def::DefKind::Impl { .. })
