...
macro_rules! declare_lint {
    ...
    ($(#[$attr:meta])* $vis: vis $NAME: ident, $Level: ident, $desc: expr,
     ...
     $($v:ident),*) => (
        $(#[$attr])*
        $vis static $NAME: &$rustc_lint_defs::Lint = &$rustc_lint_defs::Lint {
            name: stringify!($NAME),
            default_level: $rustc_lint_defs::$Level,
            desc: $desc,
            is_externally_loaded: false,
            $($v: true,)*
            ...
            $(edition_lint_opts: Some(($rustc_lint_defs::Edition::$lint_edition, $rustc_lint_defs::$edition_level)),)?
...
macro_rules! declare_tool_lint {
    (
        ...
        $rustc_lint_defs::declare_tool_lint!{$(#[$attr])* $vis $tool::$NAME, $Level, $desc, false $(, @eval_always = $eval_always)? $(, @feature_gate = $gate;)?}
    );
    (
        $(#[$attr:meta])* $vis:vis $tool:ident ::$NAME:ident, $Level:ident, $desc:expr,
        ...
        $(, @feature_gate = $gate:ident;)?
         ...
         $rustc_lint_defs::declare_tool_lint!{$(#[$attr])* $vis $tool::$NAME, $Level, $desc, $rep  $(, @eval_always = $eval_always)? $(, @feature_gate = $gate;)?}
    );
    (
        $(#[$attr:meta])* $vis:vis $tool:ident ::$NAME:ident, $Level:ident, $desc:expr,
        $external:expr
...
macro_rules! impl_lint_pass {
    ($ty:ty => [$($lint:expr),* $(,)?]) => {
        impl $rustc_lint_defs::LintPass for $ty {
            fn name(&self) -> &'static str { stringify!($ty) }
            fn get_lints(&self) -> $rustc_lint_defs::LintVec { vec![$($lint),*] }
        }
        impl $ty {
            #[allow(unused)]
            pub fn lint_vec() -> $rustc_lint_defs::LintVec { vec![$($lint),*] }
        }
    };
}
