...
use crate::rustc_session::cstore::{CrateSource, ExternCrate};
             ...
             name: &mut crate::rustc_span::FileName| {
                ...
                for virtual_dir in virtual_source_base_dir.iter().flatten() {
                    if let Some(real_dir) = &real_source_base_dir
                        ...
                        && let crate::rustc_span::RealFileName::Remapped { local_path: _, virtual_name } =
                            old_name
                        && let Ok(rest) = virtual_name.strip_prefix(virtual_dir)
                    {
                        ...
                        let new_name = if applied {
                            ...
                            crate::rustc_span::RealFileName::LocalPath(new_path)
                        };
                        *old_name = new_name;
                    }
                }
             ...
             name: &mut crate::rustc_span::FileName| {
                    ...
                    && let crate::rustc_span::FileName::Real(old_name) = name
                {
                    let relative_path = match old_name {
                        crate::rustc_span::RealFileName::LocalPath(local) => {
                            local.strip_prefix(real_dir).ok()
                        }
                        crate::rustc_span::RealFileName::Remapped { virtual_name, .. } => {
                    ...
                    if let Some(rest) = relative_path.and_then(|p| p.strip_prefix(subdir).ok()) {
                        *old_name = crate::rustc_span::RealFileName::Remapped {
                            local_path: None,
