...
// AST visitor. Also see `crate::rustc_session::lint::builtin`, which contains the
        ...
        if cx.tcx.visibility(def_id).is_public() && !cx.effective_visibilities.is_reachable(def_id)
        {
            ...
            let new_vis = if let Some(ty::Visibility::Restricted(restricted_did)) =
                cx.effective_visibilities.effective_vis(def_id).map(|effective_vis| {
                    effective_vis.at_level(crate::rustc_middle::middle::privacy::Level::Reachable)
                })
        ...
        lang_features
            ...
            .for_each(|(name, span)| {
                if features.incomplete(name) {
                    let note = crate::rustc_feature::find_feature_issue(name, GateIssue::Language)
