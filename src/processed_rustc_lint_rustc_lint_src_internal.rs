...
impl LateLintPass<'_> for DefaultHashTypes {
    fn check_path(&mut self, cx: &LateContext<'_>, path: &hir::Path<'_>, hir_id: HirId) {
        let Res::Def(crate::rustc_hir::def::DefKind::Struct, def_id) = path.res else { return };
        if matches!(
...
impl<'tcx> LateLintPass<'tcx> for TyTyKind {
    fn check_path(
        &mut self,
        cx: &LateContext<'tcx>,
        path: &crate::rustc_hir::Path<'tcx>,
        _: crate::rustc_hir::HirId,
    ) {
...
impl<'tcx> LateLintPass<'tcx> for TypeIr {
    ...
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx hir::Item<'tcx>) {
        let crate::rustc_hir::ItemKind::Use(path, kind) = item.kind else { return };
        ...
        let (lo, hi, snippet) = match path.segments {
            ...
            [.., segment]
                if let Some(type_ns) = path.res.type_ns
                    && is_mod_inherent(type_ns)
                    && let crate::rustc_hir::UseKind::Single(ident) = kind =>
            {
                let (lo, snippet) =
...
impl<'tcx> LateLintPass<'tcx> for SymbolInternStringLiteral {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx crate::rustc_hir::Expr<'tcx>) {
        if let hir::ExprKind::Call(path, [arg]) = expr.kind
            ...
            && let crate::rustc_ast::LitKind::Str(_, _) = kind.node
        {
            cx.emit_span_lint(
