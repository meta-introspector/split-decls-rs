...
fn process_module_recursively(
    ...
    let mod_file = if mod_file1.exists() {
        ...
        if !dry_run {
            ...
            println!("Dry-run: Would write module not found error to {}", decl_file_path.display());
        }
        println!("  (Module not found error captured in {} for later LLM processing)", decl_file_path.display());
