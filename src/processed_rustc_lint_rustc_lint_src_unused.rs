...
trait UnusedDelimLint {
    ...
    fn check_expr(&mut self, cx: &EarlyContext<'_>, e: &ast::Expr) {
        ...
        let (value, ctx, followed_by_block, left_pos, right_pos, is_kw) = match e.kind {
            ...
            {
                let left = e.span.lo() + crate::rustc_span::BytePos(2);
            ...
            While(ref cond, ref block, ..)
                if !matches!(cond.kind, Let(..)) || Self::LINT_EXPR_IN_PATTERN_MATCHING_CTX =>
            {
                let left = e.span.lo() + crate::rustc_span::BytePos(5);
                let right = block.span.lo();
            ...
            Ret(Some(ref value)) => {
                let left = e.span.lo() + crate::rustc_span::BytePos(3);
                ...
                if !Self::is_expr_delims_necessary(inner, ctx, followed_by_block)
                    ...
                    && (ctx != UnusedDelimsCtx::LetScrutineeExpr
                        || !matches!(inner.kind, ast::ExprKind::Binary(
                                crate::rustc_span::source_map::Spanned { node, .. },
                                _,
                                _,
                            ) if node.is_lazy()))
...
impl EarlyLintPass for UnusedParens {
                    ...
                    if let ast::GenericBound::Trait(poly_trait_ref) = &bounds[i] {
                            ...
                            && !dyn2015_exception
                        {
                            ...
                            let spans = (!s.from_expansion()).then(|| {
                                (
                                    s.with_hi(s.lo() + crate::rustc_span::BytePos(1)),
                                    s.with_lo(s.hi() - crate::rustc_span::BytePos(1)),
                                )
                            });
    ...
    fn check_item_post(&mut self, _: &EarlyContext<'_>, _: &crate::rustc_ast::Item) {
