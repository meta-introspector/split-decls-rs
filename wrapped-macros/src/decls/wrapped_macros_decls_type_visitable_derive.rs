...
fn type_visitable_derive(
    ...
    s.bound_impl(
        quote!(
            ::rustc_type_ir::TypeVisitable <::hir_ty::next_solver::DbInterner <'db >>
        ),
        quote! {
            fn visit_with < __V : ::rustc_type_ir::TypeVisitor
            <::hir_ty::next_solver::DbInterner <'db >>> (& self, __visitor : & mut __V)
            -> __V::Result { match * self { # body_visit } < __V::Result as
