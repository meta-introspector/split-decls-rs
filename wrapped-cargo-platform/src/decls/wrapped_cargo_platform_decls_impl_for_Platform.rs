...
impl Platform {
    #[doc = " Returns whether the Platform matches the given target and cfg."]
    #[doc = ""]
    #[doc = " The named target and cfg values should be obtained from `rustc`."]
    ...
    pub fn check_cfg_attributes(&self, warnings: &mut Vec<String>) {
        fn check_cfg_expr(expr: &CfgExpr, warnings: &mut Vec<String>) {
            match * expr { CfgExpr :: Not (ref e) => check_cfg_expr (e , warnings) , CfgExpr :: All (ref e) | CfgExpr :: Any (ref e) => { for e in e { check_cfg_expr (e , warnings) ; } } CfgExpr :: Value (ref e) => match e { Cfg :: Name (name) => match name . as_str () { "test" | "debug_assertions" | "proc_macro" => warnings . push (format ! ("Found `{}` in `target.'cfg(...)'.dependencies`. \
                             ...
                             https://doc.rust-lang.org/cargo/reference/features.html")) } , } CfgExpr :: True | CfgExpr :: False => { } , }
        }
                ...
                CfgExpr::Value(ref e) => match e {
                    Cfg::Name(name) | Cfg::KeyPair(name, _) => {
                        if !name.raw && KEYWORDS.contains(&name.as_str()) {
                            warnings . push (format ! ("[{}] future-incompatibility: `cfg({e})` is deprecated as `{name}` is a keyword \
                                 ...
                                 |\n \
                                 | help: use raw-idents instead: `cfg(r#{name})`" , path . display ())) ;
                        }
                    }
                },
            }
