    ...
    pub fn parse_with_proc_macros(
        ...
        for entry in fixture {
            ...
            if let Some(range_or_offset) = range_or_offset {
                file_position = Some((
                    EditionedFileId::new(db, file_id, meta.edition),
                    range_or_offset,
                ));
            }
            ...
            if !meta.deps.is_empty() {
                assert!(
                    meta.krate.is_some(), "can't specify deps without naming the crate"
                )
            }
            if let Some(kind) = meta.introduce_new_source_root {
                ...
                let prev_root = match prev_kind {
                    SourceRootKind::Local => {
                        SourceRoot::new_local(mem::take(&mut file_set))
                    }
                    SourceRootKind::Library => {
                        SourceRoot::new_library(mem::take(&mut file_set))
                    }
                };
        ...
        let mini_core = mini_core
            .map(|mini_core| {
                ...
                fs.insert(
                    core_file,
                    VfsPath::new_virtual_path("/sysroot/core/lib.rs".to_owned()),
                );
                roots.push(SourceRoot::new_library(fs));
                sysroot_files.push(core_file);
                source_change
                    .change_file(core_file, Some(mini_core.source_code(minicore_raw)));
                let core_crate = crate_graph
                    .add_crate_root(
                        core_file,
                        Edition::CURRENT,
        ...
        if !proc_macro_names.is_empty() {
            ...
            let (proc_macro, source) = filter_test_proc_macros(
                &proc_macro_names,
                proc_macro_defs,
            );
            let mut fs = FileSet::default();
            ...
            let all_crates = crate_graph.iter().collect::<Vec<_>>();
            let proc_macros_crate = crate_graph
                .add_crate_root(
                    proc_lib_file,
                    Edition::CURRENT,
