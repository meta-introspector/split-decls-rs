...
impl SourceFile {
    ...
    #[doc = " This converts the `lines` field to contain `SourceFileLines::Lines` if needed and freezes"]
    #[doc = " it."]
    ...
    #[doc = " Returns the `BytePos` of the beginning of the current line."]
    ...
    #[doc = " Add externally loaded source."]
    #[doc = " If the hash of the input doesn't match or no input is supplied via None,"]
    #[doc = " it is interpreted as an error and the corresponding enum variant is set."]
    #[doc = " The return value signifies whether some kind of source is present."]
    ...
    #[doc = " Gets a line from the list of pre-computed line-beginnings."]
    #[doc = " The line number here is 0-based."]
    ...
    #[doc = " Finds the line containing the given position. The return value is the"]
    #[doc = " index into the `lines` array of this `SourceFile`, not the 1-based line"]
    #[doc = " number. If the source_file is empty or the position is located before the"]
    #[doc = " first line, `None` is returned."]
    ...
    #[doc = " Returns whether or not the file contains the given `SourceMap` byte"]
    #[doc = " position. The position one past the end of the file is considered to be"]
    #[doc = " contained by the file. This implies that files for which `is_empty`"]
    #[doc = " returns true still contain one byte position according to this function."]
    #[inline]
    ...
    #[doc = " Calculates the original byte position relative to the start of the file"]
    #[doc = " based on the given byte position."]
    ...
    #[doc = " Calculates a normalized byte position from a byte offset relative to the"]
    #[doc = " start of the file."]
    #[doc = ""]
    #[doc = " When we get an inline assembler error from LLVM during codegen, we"]
    #[doc = " import the expanded assembly code as a new `SourceFile`, which can then"]
    #[doc = " be used for error reporting with spans. However the byte offsets given"]
    #[doc = " to us by LLVM are relative to the start of the original buffer, not the"]
    #[doc = " normalized one. Hence we need to convert those offsets to the normalized"]
    #[doc = " form when constructing spans."]
    ...
    #[doc = " Converts an relative `RelativeBytePos` to a `CharPos` relative to the `SourceFile`."]
    ...
    #[doc = " Looks up the file's (1-based) line number and (0-based `CharPos`) column offset, for a"]
    #[doc = " given `RelativeBytePos`."]
    ...
    #[doc = " Looks up the file's (1-based) line number, (0-based `CharPos`) column offset, and (0-based)"]
    #[doc = " column offset when displayed, for a given `BytePos`."]
