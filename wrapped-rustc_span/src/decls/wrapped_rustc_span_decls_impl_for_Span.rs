...
impl Span {
    ...
    pub fn in_derive_expansion(self) -> bool {
        matches!(
            self.ctxt().outer_expn_data().kind, ExpnKind::Macro(MacroKind::Derive, _)
        )
    }
    ...
    pub fn can_be_used_for_suggestions(self) -> bool {
        !self.from_expansion()
            || (self.in_derive_expansion()
                && self.parent_callsite().map(|p| (p.lo(), p.hi()))
    ...
    pub fn find_ancestor_not_from_extern_macro(
        mut self,
        sm: &SourceMap,
    ) -> Option<Span> {
    ...
    fn prepare_to_combine(
        ...
        let a_is_callsite = a.ctxt.is_root()
            || a.ctxt == b.span().source_callsite().ctxt();
    ...
    pub fn within_macro(self, within: Span, sm: &SourceMap) -> Option<Span> {
        match Span::prepare_to_combine(self, within) {
            Ok(
                ...
                || self.hi() < self_.lo && !sm.is_imported(within) => {
