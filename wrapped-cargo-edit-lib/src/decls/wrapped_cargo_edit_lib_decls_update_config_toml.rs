...
pub fn update_config_toml(
    ...
    let mut doc = if config_toml_path.exists() {
        let contents = fs::read_to_string(config_toml_path)
            .context(
                format!("Failed to read existing config.toml: {:?}", config_toml_path),
            )?;
        contents
            .parse::<DocumentMut>()
            .context(
                format!("Failed to parse existing config.toml: {:?}", config_toml_path),
            )?
    } else {
    ...
    fs::write(config_toml_path, doc.to_string())
        .context(
            format!("Failed to write updated config.toml: {:?}", config_toml_path),
        )?;
