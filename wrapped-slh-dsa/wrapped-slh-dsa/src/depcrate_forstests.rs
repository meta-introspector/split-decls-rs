...
macro_rules! Depcrate_forstests {
() => {
// Module: crate::fors
// Provides: {"tests"}
// Dependencies: {}
# [cfg (test)] mod tests { use self :: address :: ForsTree ; use crate :: Shake128f ; use crate :: util :: macros :: test_parameter_sets ; use rand :: { Rng , RngCore , rng } ; use super :: * ; # [test] # [cfg (feature = "alloc")] # [allow (clippy :: too_many_lines)] fn fors_sign_kat () { use hex_literal :: hex ; let sk_seed = SkSeed (Array ([1 ; 16])) ; let pk_seed = PkSeed (Array ([2 ; 16])) ; let adrs = ForsTree :: new (3 , 5) ; let md = Array ([3 ; 25]) ; let sig = < Shake128f as ForsParams > :: fors_sign (& md , & sk_seed , & pk_seed , & adrs) ; let expected = hex ! ("2cac88fad4eeae791048fe07aa3544a9
        ...
        99b6ca2ce814190f602542a1bfa738ea") ; assert_eq ! (sig . to_vec () , expected) ; } fn test_sign_verify < Fors : ForsParams > () { let mut rng = rng () ; let sk_seed = SkSeed :: new (& mut rng) ; let pk_seed = PkSeed :: new (& mut rng) ; let mut msg = Array :: < u8 , Fors :: MD > :: default () ; rng . fill_bytes (msg . as_mut_slice ()) ; let idx_tree = rng . random_range (0 ..= (1u64 . wrapping_shl (Fors :: H :: U32 - Fors :: HPrime :: U32) . wrapping_sub (1)) ,) ; let idx_leaf = rng . random_range (0 .. (1 << (Fors :: HPrime :: USIZE))) ; let mut adrs = ForsTree :: new (idx_tree , idx_leaf) ; let mut pks = Array :: < Array < u8 , Fors :: N > , Fors :: K > :: default () ; for i in 0 .. Fors :: K :: U32 { adrs . tree_index . set (i) ; pks [i as usize] = Fors :: fors_node (& sk_seed , i , Fors :: A :: U32 , & pk_seed , & adrs) ; } let pk = Fors :: t (& pk_seed , & adrs . fors_roots () , & pks) ; let sig = Fors :: fors_sign (& msg , & sk_seed , & pk_seed , & adrs) ; let pk_recovered = Fors :: fors_pk_from_sig (& sig , & msg , & pk_seed , & adrs) ; assert_eq ! (pk , pk_recovered) ; } test_parameter_sets ! (test_sign_verify) ; fn test_sign_verify_failure < Fors : ForsParams > () { let mut rng = rng () ; let sk_seed = SkSeed :: new (& mut rng) ; let pk_seed = PkSeed :: new (& mut rng) ; let mut msg = Array :: < u8 , Fors :: MD > :: default () ; rng . fill_bytes (msg . as_mut_slice ()) ; let idx_tree = rng . random_range (0 ..= (1u64 . wrapping_shl (Fors :: H :: U32 - Fors :: HPrime :: U32) . wrapping_sub (1)) ,) ; let idx_leaf = rng . random_range (0 .. (1 << (Fors :: HPrime :: USIZE))) ; let mut adrs = ForsTree :: new (idx_tree , idx_leaf) ; let mut pks = Array :: < Array < u8 , Fors :: N > , Fors :: K > :: default () ; for i in 0 .. Fors :: K :: U32 { adrs . tree_index . set (i) ; pks [i as usize] = Fors :: fors_node (& sk_seed , i , Fors :: A :: U32 , & pk_seed , & adrs) ; } let pk = Fors :: t (& pk_seed , & adrs . fors_roots () , & pks) ; let sig = Fors :: fors_sign (& msg , & sk_seed , & pk_seed , & adrs) ; msg [0] ^= 0xff ; let pk_recovered = Fors :: fors_pk_from_sig (& sig , & msg , & pk_seed , & adrs) ; assert_ne ! (pk , pk_recovered , "Signature verification should fail with a modified message") ; } test_parameter_sets ! (test_sign_verify_failure) ; }
};
}
