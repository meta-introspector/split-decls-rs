# AST Trace: compiler/rustc_ast/src/util/comments/tests.rs

Generated 5 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=FUNCTION | NAME=test_block_doc_comment_1 | COMPLEXITY=5 | LINES=31

```rust
// SRC: compiler/rustc_ast/src/util/comments/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_ast/src/util/comments/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_ast/src/util/comments/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_ast/src/util/comments/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_ast/src/util/comments/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_ast/src/util/comments/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_ast/src/util/comments/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_ast/src/util/comments/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/compiler/rustc_ast/src/util/comments/tests.rs
#[allow(rustc::symbol_intern_string_literal)]

use crate::rustc_complete::create_default_session_globals_then;

use super::*;

#[test]
fn test_block_doc_comment_1() {
    create_default_session_globals_then(|| {
        let comment = "\n * Test \n **  Test\n *   Test\n";
        let stripped = beautify_doc_string(Symbol::intern(comment), CommentKind::Block);
        assert_eq!(stripped.as_str(), " Test \n*  Test\n   Test");
    })
}
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=test_block_doc_comment_2 | COMPLEXITY=3 | LINES=9

```rust
#[test]
fn test_block_doc_comment_2() {
    create_default_session_globals_then(|| {
        let comment = "\n * Test\n *  Test\n";
        let stripped = beautify_doc_string(Symbol::intern(comment), CommentKind::Block);
        assert_eq!(stripped.as_str(), " Test\n  Test");
    })
}
```

## Block 3
**Metadata**: AST_ID=3 | TYPE=FUNCTION | NAME=test_block_doc_comment_3 | COMPLEXITY=3 | LINES=9

```rust
#[test]
fn test_block_doc_comment_3() {
    create_default_session_globals_then(|| {
        let comment = "\n let a: *i32;\n *a = 5;\n";
        let stripped = beautify_doc_string(Symbol::intern(comment), CommentKind::Block);
        assert_eq!(stripped.as_str(), "let a: *i32;\n*a = 5;");
    })
}
```

## Block 4
**Metadata**: AST_ID=4 | TYPE=FUNCTION | NAME=test_line_doc_comment | COMPLEXITY=4 | LINES=14

```rust
#[test]
fn test_line_doc_comment() {
    create_default_session_globals_then(|| {
        let stripped = beautify_doc_string(Symbol::intern(" test"), CommentKind::Line);
        assert_eq!(stripped.as_str(), " test");
        let stripped = beautify_doc_string(Symbol::intern("! test"), CommentKind::Line);
        assert_eq!(stripped.as_str(), "! test");
        let stripped = beautify_doc_string(Symbol::intern("test"), CommentKind::Line);
        assert_eq!(stripped.as_str(), "test");
        let stripped = beautify_doc_string(Symbol::intern("!test"), CommentKind::Line);
        assert_eq!(stripped.as_str(), "!test");
    })
}
```

## Block 5
**Metadata**: AST_ID=5 | TYPE=FUNCTION | NAME=test_doc_blocks | COMPLEXITY=4 | LINES=18

```rust
#[test]
fn test_doc_blocks() {
    create_default_session_globals_then(|| {
        let stripped =
            beautify_doc_string(Symbol::intern(" # Returns\n     *\n     "), CommentKind::Block);
        assert_eq!(stripped.as_str(), " # Returns\n\n");

        let stripped = beautify_doc_string(
            Symbol::intern("\n     * # Returns\n     *\n     "),
            CommentKind::Block,
        );
        assert_eq!(stripped.as_str(), " # Returns\n\n");

        let stripped = beautify_doc_string(Symbol::intern("\n *     a\n "), CommentKind::Block);
        assert_eq!(stripped.as_str(), "     a\n");
    })
}
```

---
*Generated by AST tracing system*
