# AST Trace: compiler/rustc_public/src/unstable/convert/stable/mod.rs

Generated 6 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=FUNCTION | NAME=stable | COMPLEXITY=13 | LINES=45

```rust
// SRC: compiler/rustc_public/src/unstable/convert/stable/mod.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/unstable/convert/stable/mod.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/unstable/convert/stable/mod.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/unstable/convert/stable/mod.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/unstable/convert/stable/mod.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/unstable/convert/stable/mod.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/unstable/convert/stable/mod.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/unstable/convert/stable/mod.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/compiler/rustc_public/src/unstable/convert/stable/mod.rs
//! Conversion of internal Rust compiler items to stable ones.

use rustc_abi::FieldIdx;
use rustc_public_bridge::Tables;
use rustc_public_bridge::context::CompilerCtxt;

use super::Stable;
use crate::compiler_interface::BridgeTys;

// MOD DECLARATION: abi (from compiler/rustc_public/src/unstable/convert/stable/mod.rs)
// REPLACED: mod abi; -> use crate::abi::*;
use crate::abi::*;
// MOD DECLARATION: mir (from compiler/rustc_public/src/unstable/convert/stable/mod.rs)
// REPLACED: mod mir; -> use crate::mir::*;
use crate::mir::*;
// MOD DECLARATION: ty (from compiler/rustc_public/src/unstable/convert/stable/mod.rs)
// REPLACED: mod ty; -> use crate::ty::*;
use crate::ty::*;

impl<'tcx> Stable<'tcx> for rustc_hir::Safety {
    type T = crate::mir::Safety;
    fn stable(&self, _: &mut Tables<'_, BridgeTys>, _: &CompilerCtxt<'_, BridgeTys>) -> Self::T {
        match self {
            rustc_hir::Safety::Unsafe => crate::mir::Safety::Unsafe,
            rustc_hir::Safety::Safe => crate::mir::Safety::Safe,
        }
    }
}
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=stable | COMPLEXITY=5 | LINES=7

```rust
impl<'tcx> Stable<'tcx> for FieldIdx {
    type T = usize;
    fn stable(&self, _: &mut Tables<'_, BridgeTys>, _: &CompilerCtxt<'_, BridgeTys>) -> Self::T {
        self.as_usize()
    }
}
```

## Block 3
**Metadata**: AST_ID=3 | TYPE=FUNCTION | NAME=stable | COMPLEXITY=9 | LINES=12

```rust
impl<'tcx> Stable<'tcx> for rustc_hir::CoroutineSource {
    type T = crate::mir::CoroutineSource;
    fn stable(&self, _: &mut Tables<'_, BridgeTys>, _: &CompilerCtxt<'_, BridgeTys>) -> Self::T {
        use rustc_hir::CoroutineSource;
        match self {
            CoroutineSource::Block => crate::mir::CoroutineSource::Block,
            CoroutineSource::Closure => crate::mir::CoroutineSource::Closure,
            CoroutineSource::Fn => crate::mir::CoroutineSource::Fn,
        }
    }
}
```

## Block 4
**Metadata**: AST_ID=4 | TYPE=FUNCTION | NAME=stable | COMPLEXITY=16 | LINES=34

```rust
impl<'tcx> Stable<'tcx> for rustc_hir::CoroutineKind {
    type T = crate::mir::CoroutineKind;
    fn stable<'cx>(
        &self,
        tables: &mut Tables<'cx, BridgeTys>,
        cx: &CompilerCtxt<'cx, BridgeTys>,
    ) -> Self::T {
        use rustc_hir::{CoroutineDesugaring, CoroutineKind};
        match *self {
            CoroutineKind::Desugared(CoroutineDesugaring::Async, source) => {
                crate::mir::CoroutineKind::Desugared(
                    crate::mir::CoroutineDesugaring::Async,
                    source.stable(tables, cx),
                )
            }
            CoroutineKind::Desugared(CoroutineDesugaring::Gen, source) => {
                crate::mir::CoroutineKind::Desugared(
                    crate::mir::CoroutineDesugaring::Gen,
                    source.stable(tables, cx),
                )
            }
            CoroutineKind::Coroutine(movability) => {
                crate::mir::CoroutineKind::Coroutine(movability.stable(tables, cx))
            }
            CoroutineKind::Desugared(CoroutineDesugaring::AsyncGen, source) => {
                crate::mir::CoroutineKind::Desugared(
                    crate::mir::CoroutineDesugaring::AsyncGen,
                    source.stable(tables, cx),
                )
            }
        }
    }
}
```

## Block 5
**Metadata**: AST_ID=5 | TYPE=FUNCTION | NAME=stable | COMPLEXITY=5 | LINES=8

```rust
impl<'tcx> Stable<'tcx> for rustc_span::Symbol {
    type T = crate::Symbol;

    fn stable(&self, _: &mut Tables<'_, BridgeTys>, _: &CompilerCtxt<'_, BridgeTys>) -> Self::T {
        self.to_string()
    }
}
```

## Block 6
**Metadata**: AST_ID=6 | TYPE=FUNCTION | NAME=stable | COMPLEXITY=5 | LINES=12

```rust
impl<'tcx> Stable<'tcx> for rustc_span::Span {
    type T = crate::ty::Span;

    fn stable<'cx>(
        &self,
        tables: &mut Tables<'cx, BridgeTys>,
        _: &CompilerCtxt<'cx, BridgeTys>,
    ) -> Self::T {
        tables.create_span(*self)
    }
}
```

---
*Generated by AST tracing system*
