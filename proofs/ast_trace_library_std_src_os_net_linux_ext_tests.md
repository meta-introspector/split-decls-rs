# AST Trace: library/std/src/os/net/linux_ext/tests.rs

Generated 2 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=FUNCTION | NAME=quickack | COMPLEXITY=21 | LINES=42

```rust
// SRC: library/std/src/os/net/linux_ext/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/net/linux_ext/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/net/linux_ext/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/net/linux_ext/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/net/linux_ext/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/net/linux_ext/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/net/linux_ext/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/library/std/src/os/net/linux_ext/tests.rs
#[test]
fn quickack() {
    use crate::net::test::next_test_ip4;
    use crate::net::{TcpListener, TcpStream};
    use crate::os::net::linux_ext::tcp::TcpStreamExt;

    macro_rules! t {
        ($e:expr) => {
            match $e {
                Ok(t) => t,
                Err(e) => panic!("received error for `{}`: {}", stringify!($e), e),
            }
        };
    }

    let addr = next_test_ip4();
    let _listener = t!(TcpListener::bind(&addr));

    let stream = t!(TcpStream::connect(&("localhost", addr.port())));

    t!(stream.set_quickack(false));
    assert_eq!(false, t!(stream.quickack()));
    t!(stream.set_quickack(true));
    assert_eq!(true, t!(stream.quickack()));
    t!(stream.set_quickack(false));
    assert_eq!(false, t!(stream.quickack()));
}
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=deferaccept | COMPLEXITY=19 | LINES=25

```rust
#[test]
#[cfg(target_os = "linux")]
fn deferaccept() {
    use crate::net::test::next_test_ip4;
    use crate::net::{TcpListener, TcpStream};
    use crate::os::net::linux_ext::tcp::TcpStreamExt;

    macro_rules! t {
        ($e:expr) => {
            match $e {
                Ok(t) => t,
                Err(e) => panic!("received error for `{}`: {}", stringify!($e), e),
            }
        };
    }

    let addr = next_test_ip4();
    let _listener = t!(TcpListener::bind(&addr));
    let stream = t!(TcpStream::connect(&("localhost", addr.port())));
    stream.set_deferaccept(1).expect("set_deferaccept failed");
    assert_eq!(stream.deferaccept().unwrap(), 1);
    stream.set_deferaccept(0).expect("set_deferaccept failed");
    assert_eq!(stream.deferaccept().unwrap(), 0);
}
```

---
*Generated by AST tracing system*
