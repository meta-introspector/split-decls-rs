# AST Trace: compiler/rustc_public_bridge/src/context/helpers.rs

Generated 3 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=FUNCTION | NAME=new_foreign | COMPLEXITY=4 | LINES=30

```rust
// SRC: compiler/rustc_public_bridge/src/context/helpers.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public_bridge/src/context/helpers.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public_bridge/src/context/helpers.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public_bridge/src/context/helpers.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public_bridge/src/context/helpers.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public_bridge/src/context/helpers.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public_bridge/src/context/helpers.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public_bridge/src/context/helpers.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/compiler/rustc_public_bridge/src/context/helpers.rs
//! A set of traits that define a stable interface to rustc's internals.
//!
//! These traits abstract rustc's internal APIs, allowing rustc_public to maintain a stable
//! interface regardless of internal compiler changes.

use rustc_middle::mir::interpret::AllocRange;
use rustc_middle::ty;
use rustc_middle::ty::Ty;
use rustc_span::def_id::DefId;

pub trait TyHelpers<'tcx> {
    fn new_foreign(&self, def_id: DefId) -> Ty<'tcx>;
}
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=fully_monomorphized | COMPLEXITY=2 | LINES=4

```rust
pub trait TypingEnvHelpers<'tcx> {
    fn fully_monomorphized(&self) -> ty::TypingEnv<'tcx>;
}
```

## Block 3
**Metadata**: AST_ID=3 | TYPE=FUNCTION | NAME=alloc_range | COMPLEXITY=2 | LINES=4

```rust
pub trait AllocRangeHelpers<'tcx> {
    fn alloc_range(&self, offset: rustc_abi::Size, size: rustc_abi::Size) -> AllocRange;
}
```

---
*Generated by AST tracing system*
