# AST Trace: compiler/rustc_query_system/src/values.rs

Generated 2 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=FUNCTION | NAME=from_cycle_error | COMPLEXITY=3 | LINES=23

```rust
// SRC: compiler/rustc_query_system/src/values.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_query_system/src/values.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_query_system/src/values.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_query_system/src/values.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_query_system/src/values.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_query_system/src/values.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_query_system/src/values.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/compiler/rustc_query_system/src/values.rs
use rustc_span::ErrorGuaranteed;

use crate::dep_graph::DepContext;
use crate::query::CycleError;

pub trait Value<Tcx: DepContext>: Sized {
    fn from_cycle_error(tcx: Tcx, cycle_error: &CycleError, guar: ErrorGuaranteed) -> Self;
}
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=UNNAMED | COMPLEXITY=8 | LINES=13

```rust
impl<Tcx: DepContext, T> Value<Tcx> for T {
    default fn from_cycle_error(tcx: Tcx, cycle_error: &CycleError, _guar: ErrorGuaranteed) -> T {
        tcx.sess().dcx().abort_if_errors();
        // Ideally we would use `bug!` here. But bug! is only defined in rustc_middle, and it's
        // non-trivial to define it earlier.
        panic!(
            "<{} as Value>::from_cycle_error called without errors: {:#?}",
            std::any::type_name::<T>(),
            cycle_error.cycle,
        );
    }
}
```

---
*Generated by AST tracing system*
