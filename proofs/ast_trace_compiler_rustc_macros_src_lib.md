# AST Trace: compiler/rustc_macros/src/lib.rs

Generated 10 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=USE | NAME=UNNAMED | COMPLEXITY=8 | LINES=71

```rust
// SRC: compiler/rustc_macros/src/lib.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_macros/src/lib.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_macros/src/lib.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_macros/src/lib.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_macros/src/lib.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_macros/src/lib.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_macros/src/lib.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_macros/src/lib.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/compiler/rustc_macros/src/lib.rs
// tidy-alphabetical-start
#![allow(rustc::default_hash_types)]
#![feature(if_let_guard)]
#![feature(never_type)]
#![feature(proc_macro_diagnostic)]
#![feature(proc_macro_tracked_env)]
// tidy-alphabetical-end

use proc_macro::TokenStream;
use synstructure::decl_derive;

// MOD DECLARATION: current_version (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod current_version; -> use crate::current_version::*;
use crate::current_version::*;
// MOD DECLARATION: diagnostics (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod diagnostics; -> use crate::diagnostics::*;
use crate::diagnostics::*;
// MOD DECLARATION: extension (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod extension; -> use crate::extension::*;
use crate::extension::*;
// MOD DECLARATION: hash_stable (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod hash_stable; -> use crate::hash_stable::*;
use crate::hash_stable::*;
// MOD DECLARATION: lift (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod lift; -> use crate::lift::*;
use crate::lift::*;
// MOD DECLARATION: print_attribute (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod print_attribute; -> use crate::print_attribute::*;
use crate::print_attribute::*;
// MOD DECLARATION: query (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod query; -> use crate::query::*;
use crate::query::*;
// MOD DECLARATION: serialize (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod serialize; -> use crate::serialize::*;
use crate::serialize::*;
// MOD DECLARATION: symbols (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod symbols; -> use crate::symbols::*;
use crate::symbols::*;
// MOD DECLARATION: try_from (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod try_from; -> use crate::try_from::*;
use crate::try_from::*;
// MOD DECLARATION: type_foldable (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod type_foldable; -> use crate::type_foldable::*;
use crate::type_foldable::*;
// MOD DECLARATION: type_visitable (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod type_visitable; -> use crate::type_visitable::*;
use crate::type_visitable::*;
// MOD DECLARATION: visitable (from compiler/rustc_macros/src/lib.rs)
// REPLACED: mod visitable; -> use crate::visitable::*;
use crate::visitable::*;

// Reads the rust version (e.g. "1.75.0") from the CFG_RELEASE env var and
// produces a `RustcVersion` literal containing that version (e.g.
// `RustcVersion { major: 1, minor: 75, patch: 0 }`).
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=current_rustc_version | COMPLEXITY=2 | LINES=4

```rust
#[proc_macro]
pub fn current_rustc_version(input: TokenStream) -> TokenStream {
    current_version::current_version(input)
}
```

## Block 3
**Metadata**: AST_ID=3 | TYPE=FUNCTION | NAME=rustc_queries | COMPLEXITY=2 | LINES=5

```rust
#[proc_macro]
pub fn rustc_queries(input: TokenStream) -> TokenStream {
    query::rustc_queries(input)
}
```

## Block 4
**Metadata**: AST_ID=4 | TYPE=FUNCTION | NAME=symbols | COMPLEXITY=2 | LINES=5

```rust
#[proc_macro]
pub fn symbols(input: TokenStream) -> TokenStream {
    symbols::symbols(input.into()).into()
}
```

## Block 5
**Metadata**: AST_ID=5 | TYPE=FUNCTION | NAME=UNNAMED | COMPLEXITY=7 | LINES=8

```rust
/// Derive an extension trait for a given impl block. The trait name
/// goes into the parenthesized args of the macro, for greppability.
/// For example:
/// ```
/// use rustc_macros::extension;
/// #[extension(pub trait Foo)]
/// impl i32 { fn hello() {} }
```

## Block 6
**Metadata**: AST_ID=6 | TYPE=FUNCTION | NAME=UNNAMED | COMPLEXITY=2 | LINES=5

```rust
/// ```
///
/// expands to:
/// ```
/// pub trait Foo { fn hello(); }
```

## Block 7
**Metadata**: AST_ID=7 | TYPE=FUNCTION | NAME=UNNAMED | COMPLEXITY=5 | LINES=1

```rust
/// impl Foo for i32 { fn hello() {} }
```

## Block 8
**Metadata**: AST_ID=8 | TYPE=FUNCTION | NAME=extension | COMPLEXITY=2 | LINES=5

```rust
/// ```
#[proc_macro_attribute]
pub fn extension(attr: TokenStream, input: TokenStream) -> TokenStream {
    extension::extension(attr, input)
}
```

## Block 9
**Metadata**: AST_ID=9 | TYPE=STRUCT | NAME=UNNAMED | COMPLEXITY=35 | LINES=125

```rust
decl_derive!([HashStable, attributes(stable_hasher)] => hash_stable::hash_stable_derive);
decl_derive!(
    [HashStable_Generic, attributes(stable_hasher)] =>
    hash_stable::hash_stable_generic_derive
);
decl_derive!(
    [HashStable_NoContext] =>
    /// `HashStable` implementation that has no `HashStableContext` bound and
    /// which adds `where` bounds for `HashStable` based off of fields and not
    /// generics. This is suitable for use in crates like `rustc_type_ir`.
    hash_stable::hash_stable_no_context_derive
);

decl_derive!([Decodable_NoContext] => serialize::decodable_nocontext_derive);
decl_derive!([Encodable_NoContext] => serialize::encodable_nocontext_derive);
decl_derive!([Decodable] => serialize::decodable_derive);
decl_derive!([Encodable] => serialize::encodable_derive);
decl_derive!([TyDecodable] => serialize::type_decodable_derive);
decl_derive!([TyEncodable] => serialize::type_encodable_derive);
decl_derive!([MetadataDecodable] => serialize::meta_decodable_derive);
decl_derive!([MetadataEncodable] => serialize::meta_encodable_derive);
decl_derive!(
    [TypeFoldable, attributes(type_foldable)] =>
    /// Derives `TypeFoldable` for the annotated `struct` or `enum` (`union` is not supported).
    ///
    /// The fold will produce a value of the same struct or enum variant as the input, with
    /// each field respectively folded using the `TypeFoldable` implementation for its type.
    /// However, if a field of a struct or an enum variant is annotated with
    /// `#[type_foldable(identity)]` then that field will retain its incumbent value (and its
    /// type is not required to implement `TypeFoldable`).
    type_foldable::type_foldable_derive
);
decl_derive!(
    [TypeVisitable, attributes(type_visitable)] =>
    /// Derives `TypeVisitable` for the annotated `struct` or `enum` (`union` is not supported).
    ///
    /// Each field of the struct or enum variant will be visited in definition order, using the
    /// `TypeVisitable` implementation for its type. However, if a field of a struct or an enum
    /// variant is annotated with `#[type_visitable(ignore)]` then that field will not be
    /// visited (and its type is not required to implement `TypeVisitable`).
    type_visitable::type_visitable_derive
);
decl_derive!(
    [Walkable, attributes(visitable)] =>
    /// Derives `Walkable` for the annotated `struct` or `enum` (`union` is not supported).
    ///
    /// Each field of the struct or enum variant will be visited in definition order, using the
    /// `Walkable` implementation for its type. However, if a field of a struct or an enum
    /// variant is annotated with `#[visitable(ignore)]` then that field will not be
    /// visited (and its type is not required to implement `Walkable`).
    visitable::visitable_derive
);
decl_derive!([Lift, attributes(lift)] => lift::lift_derive);
decl_derive!(
    [Diagnostic, attributes(
        // struct attributes
        diag,
        help,
        help_once,
        note,
        note_once,
        warning,
        // field attributes
        skip_arg,
        primary_span,
        label,
        subdiagnostic,
        suggestion,
        suggestion_short,
        suggestion_hidden,
        suggestion_verbose)] => diagnostics::diagnostic_derive
);
decl_derive!(
    [LintDiagnostic, attributes(
        // struct attributes
        diag,
        help,
        help_once,
        note,
        note_once,
        warning,
        // field attributes
        skip_arg,
        primary_span,
        label,
        subdiagnostic,
        suggestion,
        suggestion_short,
        suggestion_hidden,
        suggestion_verbose)] => diagnostics::lint_diagnostic_derive
);
decl_derive!(
    [Subdiagnostic, attributes(
        // struct/variant attributes
        label,
        help,
        help_once,
        note,
        note_once,
        warning,
        subdiagnostic,
        suggestion,
        suggestion_short,
        suggestion_hidden,
        suggestion_verbose,
        multipart_suggestion,
        multipart_suggestion_short,
        multipart_suggestion_hidden,
        multipart_suggestion_verbose,
        // field attributes
        skip_arg,
        primary_span,
        suggestion_part,
        applicability)] => diagnostics::subdiagnostic_derive
);

decl_derive! {
    [TryFromU32] =>
    /// Derives `TryFrom<u32>` for the annotated `enum`, which must have no fields.
    /// Each variant maps to the value it would produce under an `as u32` cast.
    ///
    /// The error type is `u32`.
    try_from::try_from_u32
}
```

## Block 10
**Metadata**: AST_ID=10 | TYPE=BLOCK | NAME=UNNAMED | COMPLEXITY=4 | LINES=8

```rust
decl_derive! {
    [PrintAttribute] =>
    /// Derives `PrintAttribute` for `AttributeKind`.
    /// This macro is pretty specific to `rustc_hir::attrs` and likely not that useful in
    /// other places. It's deriving something close to `Debug` without printing some extraneous
    /// things like spans.
    print_attribute::print_attribute
}
```

---
*Generated by AST tracing system*
