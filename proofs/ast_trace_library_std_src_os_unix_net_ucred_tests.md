# AST Trace: library/std/src/os/unix/net/ucred/tests.rs

Generated 3 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=USE | NAME=UNNAMED | COMPLEXITY=3 | LINES=18

```rust
// SRC: library/std/src/os/unix/net/ucred/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/unix/net/ucred/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/unix/net/ucred/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/unix/net/ucred/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/unix/net/ucred/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/unix/net/ucred/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/unix/net/ucred/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: library/std/src/os/unix/net/ucred/tests.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/library/std/src/os/unix/net/ucred/tests.rs
use libc::{getegid, geteuid, getpid};
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=test_socket_pair | COMPLEXITY=16 | LINES=24

```rust
use crate::os::unix::net::UnixStream;

#[test]
#[cfg(any(
    target_os = "android",
    target_os = "linux",
    target_os = "dragonfly",
    target_os = "freebsd",
    target_os = "openbsd",
    target_vendor = "apple",
))]
fn test_socket_pair() {
    // Create two connected sockets and get their peer credentials. They should be equal.
    let (sock_a, sock_b) = UnixStream::pair().unwrap();
    let (cred_a, cred_b) = (sock_a.peer_cred().unwrap(), sock_b.peer_cred().unwrap());
    assert_eq!(cred_a, cred_b);

    // Check that the UID and GIDs match up.
    let uid = unsafe { geteuid() };
    let gid = unsafe { getegid() };
    assert_eq!(cred_a.uid, uid);
    assert_eq!(cred_a.gid, gid);
}
```

## Block 3
**Metadata**: AST_ID=3 | TYPE=FUNCTION | NAME=test_socket_pair_pids | COMPLEXITY=8 | LINES=13

```rust
#[test]
#[cfg(any(target_os = "linux", target_vendor = "apple"))]
fn test_socket_pair_pids(arg: Type) -> RetType {
    // Create two connected sockets and get their peer credentials.
    let (sock_a, sock_b) = UnixStream::pair().unwrap();
    let (cred_a, cred_b) = (sock_a.peer_cred().unwrap(), sock_b.peer_cred().unwrap());

    // On supported platforms (see the cfg above), the credentials should always include the PID.
    let pid = unsafe { getpid() };
    assert_eq!(cred_a.pid, Some(pid));
    assert_eq!(cred_b.pid, Some(pid));
}
```

---
*Generated by AST tracing system*
