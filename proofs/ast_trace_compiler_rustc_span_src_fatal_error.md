# AST Trace: compiler/rustc_span/src/fatal_error.rs

Generated 4 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=STRUCT | NAME=FatalError; | COMPLEXITY=8 | LINES=25

```rust
// SRC: compiler/rustc_span/src/fatal_error.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_span/src/fatal_error.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_span/src/fatal_error.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_span/src/fatal_error.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_span/src/fatal_error.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_span/src/fatal_error.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_span/src/fatal_error.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/compiler/rustc_span/src/fatal_error.rs
/// Used as a return value to signify a fatal error occurred.
#[derive(Copy, Clone, Debug)]
#[must_use]
pub struct FatalError;

pub use rustc_data_structures::FatalErrorMarker;

// Don't implement Send on FatalError. This makes it impossible to `panic_any!(FatalError)`.
// We don't want to invoke the panic handler and print a backtrace for fatal errors.
impl !Send for FatalError {}
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=raise | COMPLEXITY=3 | LINES=6

```rust
impl FatalError {
    pub fn raise(self) -> ! {
        std::panic::resume_unwind(Box::new(FatalErrorMarker))
    }
}
```

## Block 3
**Metadata**: AST_ID=3 | TYPE=FUNCTION | NAME=fmt | COMPLEXITY=5 | LINES=6

```rust
impl std::fmt::Display for FatalError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "fatal error")
    }
}
```

## Block 4
**Metadata**: AST_ID=4 | TYPE=IMPL | NAME=UNNAMED | COMPLEXITY=4 | LINES=2

```rust
impl std::error::Error for FatalError {}
```

---
*Generated by AST tracing system*
