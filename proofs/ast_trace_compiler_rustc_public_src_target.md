# AST Trace: compiler/rustc_public/src/target.rs

Generated 5 AST blocks from source file

## Block 1
**Metadata**: AST_ID=1 | TYPE=STRUCT | NAME=MachineInfo | COMPLEXITY=4 | LINES=27

```rust
// SRC: compiler/rustc_public/src/target.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/target.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/target.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/target.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/target.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/target.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: compiler/rustc_public/src/target.rs
// GENERATED BY: build.rs (split-decls-genesis)
// SRC: ../rust/compiler/rustc_public/src/target.rs
// Provide information about the machine that this is being compiled into.

use serde::Serialize;

use crate::compiler_interface::with;

/// The properties of the target machine being compiled into.
#[derive(Clone, PartialEq, Eq, Serialize)]
pub struct MachineInfo {
    pub endian: Endian,
    pub pointer_width: MachineSize,
}
```

## Block 2
**Metadata**: AST_ID=2 | TYPE=FUNCTION | NAME=target | COMPLEXITY=5 | LINES=14

```rust
impl MachineInfo {
    pub fn target() -> MachineInfo {
        with(|cx| cx.target_info())
    }

    pub fn target_endianness() -> Endian {
        with(|cx| cx.target_info().endian)
    }

    pub fn target_pointer_width() -> MachineSize {
        with(|cx| cx.target_info().pointer_width)
    }
}
```

## Block 3
**Metadata**: AST_ID=3 | TYPE=ENUM | NAME=UNNAMED | COMPLEXITY=2 | LINES=6

```rust
#[derive(Copy, Clone, PartialEq, Eq, Serialize)]
pub enum Endian {
    Little,
    Big,
}
```

## Block 4
**Metadata**: AST_ID=4 | TYPE=STRUCT | NAME=MachineSize | COMPLEXITY=2 | LINES=6

```rust
/// Represent the size of a component.
#[derive(Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Debug, Serialize)]
pub struct MachineSize {
    num_bits: usize,
}
```

## Block 5
**Metadata**: AST_ID=5 | TYPE=FUNCTION | NAME=bytes | COMPLEXITY=7 | LINES=22

```rust
impl MachineSize {
    #[inline(always)]
    pub fn bytes(self) -> usize {
        self.num_bits / 8
    }

    #[inline(always)]
    pub fn bits(self) -> usize {
        self.num_bits
    }

    #[inline(always)]
    pub fn from_bits(num_bits: usize) -> MachineSize {
        MachineSize { num_bits }
    }

    #[inline]
    pub fn unsigned_int_max(self) -> Option<u128> {
        (self.num_bits <= 128).then(|| u128::MAX >> (128 - self.bits()))
    }
}
```

---
*Generated by AST tracing system*
