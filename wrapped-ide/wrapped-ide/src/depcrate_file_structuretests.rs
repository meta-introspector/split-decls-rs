...
# [cfg (test)] mod tests { use expect_test :: { Expect , expect } ; use super :: * ; const DEFAULT_CONFIG : FileStructureConfig = FileStructureConfig { exclude_locals : true } ; fn check (# [rust_analyzer :: rust_fixture] ra_fixture : & str , expect : Expect) { check_with_config (ra_fixture , & DEFAULT_CONFIG , expect) ; } fn check_with_config (# [rust_analyzer :: rust_fixture] ra_fixture : & str , config : & FileStructureConfig , expect : Expect ,) { let file = SourceFile :: parse (ra_fixture , span :: Edition :: CURRENT) . ok () . unwrap () ; let structure = file_structure (& file , config) ; expect . assert_debug_eq (& structure) } # [test] fn test_negative_trait_bound () { let txt = r#"impl !Unpin for Test {}"# ; check (txt , expect ! [[r#"
        ...
        "#]] ,) ; } # [test] fn test_file_structure () { check (r#"
struct Foo {
    x: i32
}
...
"# , expect ! [[r#"
            ...
            "#]] ,) ; } # [test] fn test_file_structure_include_locals () { check_with_config (r#"
struct Foo {
    x: i32
}
...
"# , & FileStructureConfig { exclude_locals : false } , expect ! [[r#"
                [
                    StructureNode {
                        parent: None,
                        label: "Foo",
