// Generated macro for overlapping_not_allowed_leftmost_first (function)
macro_rules! Depcrate_testsoverlapping_not_allowed_leftmost_first {
() => {
// Module: crate::tests
// Provides: {"overlapping_not_allowed_leftmost_first"}
// Dependencies: {}
# [test] # [should_panic] fn overlapping_not_allowed_leftmost_first () { let fsm = AhoCorasick :: builder () . match_kind (MatchKind :: LeftmostFirst) . build (None :: < String >) . unwrap () ; assert_eq ! (fsm . find_overlapping_iter ("") . count () , 0) ; }
};
}
