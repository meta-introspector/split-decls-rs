...
mkuse!{use crate :: expression :: { Expression , FnCall , IdentifierType } ;}
mkuse!{use crate :: intrinsic :: Intrinsic ;}
mkuse!{use crate :: typekinds :: { ToRepr , TypeKind } ;}
mkuse!{use crate :: wildcards :: Wildcard ;}
mkuse!{use crate :: wildstring :: WildString ;}
mkitem!{const ZEROING_SUFFIX : & str = "_z" ;}
mkitem!{const MERGING_SUFFIX : & str = "_m" ;}
mkitem!{const DONT_CARE_SUFFIX : & str = "_x" ;}
mkitem!{mkenum!{#[derive (Debug , Clone , PartialEq , Eq , Serialize , Deserialize)] #[serde (untagged)] pub enum ZeroingMethod { #[doc = " Drop the specified argument and replace it with a zeroinitializer"] Drop { drop : WildString } , #[doc = " Apply zero selection to the specified variable when zeroing"] Select { select : WildString } , }}}
mkitem!{mkimpl!{impl PartialOrd for ZeroingMethod { fn partial_cmp (& self , other : & Self) -> Option < std :: cmp :: Ordering > { Some (self . cmp (other)) } }}}
mkitem!{mkimpl!{impl Ord for ZeroingMethod { fn cmp (& self , _ : & Self) -> std :: cmp :: Ordering { std :: cmp :: Ordering :: Equal } }}}
mkitem!{mkenum!{#[derive (Debug , Clone , Copy , Default , PartialEq , Eq , PartialOrd , Ord , Serialize , Deserialize)] pub enum DontCareMethod { #[default] Inferred , AsZeroing , AsMerging , }}}
mkitem!{mkstruct!{#[derive (Debug , Clone , Default , PartialEq , Eq , Deserialize , Serialize)] pub struct PredicationMethods { #[doc = " Zeroing method, if the zeroing predicate form is used"] #[serde (default)] pub zeroing_method : Option < ZeroingMethod > , #[doc = " Don't care method, if the don't care predicate form is used"] #[serde (default)] pub dont_care_method : DontCareMethod , }}}
