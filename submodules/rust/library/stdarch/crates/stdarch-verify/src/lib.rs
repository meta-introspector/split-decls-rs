...
macro_rules! find_instrs_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function find_instrs in module {}", module_path!());
    };
}

mkfn!{
    find_instrs_introspect!();
...
macro_rules! find_target_feature_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function find_target_feature in module {}", module_path!());
    };
}

mkfn!{
    find_target_feature_introspect!();
...
macro_rules! find_doc_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function find_doc in module {}", module_path!());
    };
}
...
macro_rules! find_required_const_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function find_required_const in module {}", module_path!());
    };
}

mkfn!{
    find_required_const_introspect!();
...
mkitem!{mkstruct!{struct RustcArgsRequiredConst { args : Vec < usize > , }}}
mkitem!{mkimpl!{impl syn :: parse :: Parse for RustcArgsRequiredConst { fn parse (input : syn :: parse :: ParseStream < '_ >) -> syn :: Result < Self > { let list = syn :: punctuated :: Punctuated :: < syn :: LitInt , Token ! [,] > :: parse_terminated (input) ? ; Ok (Self { args : list . into_iter () . map (| a | a . base10_parse :: < usize > ()) . collect :: < syn :: Result < _ > > () ? , }) } }}}