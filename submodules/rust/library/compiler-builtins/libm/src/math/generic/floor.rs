...
mkfn!{
    floor_introspect!();
    # [inline] pub fn floor < F : Float > (x : F) -> F { floor_status (x) . val }
}

macro_rules! floor_status_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function floor_status in module {}", module_path!());
    };
}

mkfn!{
    floor_status_introspect!();
    # [inline] pub fn floor_status < F : Float > (x : F) -> FpResult < F > { let zero = IntTy :: < F > :: ZERO ; let mut ix = x . to_bits () ; let e = x . exp_unbiased () ; if e >= F :: SIG_BITS as i32 { return FpResult :: ok (x) ; } let status ; let res = if e >= 0 { let m = F :: SIG_MASK >> e . unsigned () ; if ix & m == zero { return FpResult :: ok (x) ; } status = Status :: INEXACT ; if x . is_sign_negative () { ix += m ; } ix &= ! m ; F :: from_bits (ix) } else { if ix & F :: SIG_MASK == F :: Int :: ZERO { status = Status :: OK ; } else { status = Status :: INEXACT ; } if x . is_sign_positive () { F :: ZERO } else if ix << 1 != zero { F :: NEG_ONE } else { x } } ; FpResult :: new (res , status) }
}
mkmod!{tests, { 
                getname!(tests);
                getsrc!(tests);
...
mkfn!{
    ...
    # [test] fn sanity_check_f32 () { assert_eq ! (floor (0.5f32) , 0.0) ; assert_eq ! (floor (1.1f32) , 1.0) ; assert_eq ! (floor (2.9f32) , 2.0) ; }
}
...
mkfn!{
    ...
    # [test] fn spec_tests_f32 () { let cases = [(0.1 , 0.0 , Status :: INEXACT) , (- 0.1 , - 1.0 , Status :: INEXACT) , (0.9 , 0.0 , Status :: INEXACT) , (- 0.9 , - 1.0 , Status :: INEXACT) , (1.1 , 1.0 , Status :: INEXACT) , (- 1.1 , - 2.0 , Status :: INEXACT) , (1.9 , 1.0 , Status :: INEXACT) , (- 1.9 , - 2.0 , Status :: INEXACT) ,] ; spec_test :: < f32 > (& cases) ; }
}
...
mkfn!{
    ...
    # [test] fn sanity_check_f64 () { assert_eq ! (floor (1.1f64) , 1.0) ; assert_eq ! (floor (2.9f64) , 2.0) ; }
}
...
mkfn!{
    ...
    # [test] fn spec_tests_f64 () { let cases = [(0.1 , 0.0 , Status :: INEXACT) , (- 0.1 , - 1.0 , Status :: INEXACT) , (0.9 , 0.0 , Status :: INEXACT) , (- 0.9 , - 1.0 , Status :: INEXACT) , (1.1 , 1.0 , Status :: INEXACT) , (- 1.1 , - 2.0 , Status :: INEXACT) , (1.9 , 1.0 , Status :: INEXACT) , (- 1.9 , - 2.0 , Status :: INEXACT) ,] ; spec_test :: < f64 > (& cases) ; }
}
