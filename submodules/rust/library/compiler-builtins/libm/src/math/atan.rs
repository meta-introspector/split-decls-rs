...
mkuse!{use core :: f64 ;}
mkuse!{use super :: fabs ;}
mkitem!{const ATANHI : [f64 ; 4] = [4.63647609000806093515e-01 , 7.85398163397448278999e-01 , 9.82793723247329054082e-01 , 1.57079632679489655800e+00 ,] ;}
mkitem!{const ATANLO : [f64 ; 4] = [2.26987774529616870924e-17 , 3.06161699786838301793e-17 , 1.39033110312309984516e-17 , 6.12323399573676603587e-17 ,] ;}
mkitem!{const AT : [f64 ; 11] = [3.33333333333329318027e-01 , - 1.99999999998764832476e-01 , 1.42857142725034663711e-01 , - 1.11111104054623557880e-01 , 9.09088713343650656196e-02 , - 7.69187620504482999495e-02 , 6.66107313738753120669e-02 , - 5.83357013379057348645e-02 , 4.97687799461593236017e-02 , - 3.65315727442169155270e-02 , 1.62858201153657823623e-02 ,] ;}

macro_rules! atan_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function atan in module {}", module_path!());
    };
}
...
mkfn!{
    ...
    # [test] fn sanity_check () { for (input , answer) in [(3.0_f64 . sqrt () / 3.0 , f64 :: consts :: FRAC_PI_6) , (1.0 , f64 :: consts :: FRAC_PI_4) , (3.0_f64 . sqrt () , f64 :: consts :: FRAC_PI_3) , (- 3.0_f64 . sqrt () / 3.0 , - f64 :: consts :: FRAC_PI_6) , (- 1.0 , - f64 :: consts :: FRAC_PI_4) , (- 3.0_f64 . sqrt () , - f64 :: consts :: FRAC_PI_3) ,] . iter () { assert ! ((atan (* input) - answer) / answer < 1e-5 , "\natan({:.4}/16) = {:.4}, actual: {}" , input * 16.0 , answer , atan (* input)) ; } }
}

macro_rules! zero_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function zero in module {}", module_path!());
    };
}

mkfn!{
    zero_introspect!();
    # [test] fn zero () { assert_eq ! (atan (0.0) , 0.0) ; }
}

macro_rules! infinity_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function infinity in module {}", module_path!());
    };
}

mkfn!{
    infinity_introspect!();
    # [test] fn infinity () { assert_eq ! (atan (f64 :: INFINITY) , f64 :: consts :: FRAC_PI_2) ; }
}

macro_rules! minus_infinity_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function minus_infinity in module {}", module_path!());
    };
}

mkfn!{
    minus_infinity_introspect!();
    # [test] fn minus_infinity () { assert_eq ! (atan (f64 :: NEG_INFINITY) , - f64 :: consts :: FRAC_PI_2) ; }
}

macro_rules! nan_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function nan in module {}", module_path!());
    };
}

mkfn!{
    nan_introspect!();
    # [test] fn nan () { assert ! (atan (f64 :: NAN) . is_nan ()) ; }
} 
            }}