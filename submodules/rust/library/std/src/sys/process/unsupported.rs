...
mkitem!{mkstruct!{pub struct Command { program : OsString , args : Vec < OsString > , env : CommandEnv , cwd : Option < OsString > , stdin : Option < Stdio > , stdout : Option < Stdio > , stderr : Option < Stdio > , }}}
mkitem!{mkstruct!{pub struct StdioPipes { pub stdin : Option < AnonPipe > , pub stdout : Option < AnonPipe > , pub stderr : Option < AnonPipe > , }}}
mkitem!{mkenum!{# [derive (Debug)] pub enum Stdio { Inherit , Null , MakePipe , ParentStdout , ParentStderr , # [allow (dead_code)] InheritFile (File) , }}}
mkitem!{mkimpl!{impl Command { pub fn new (program : & OsStr) -> Command { Command { program : program . to_owned () , args : vec ! [program . to_owned ()] , env : Default :: default () , cwd : None , stdin : None , stdout : None , stderr : None , } } pub fn arg (& mut self , arg : & OsStr) { self . args . push (arg . to_owned ()) ; } pub fn env_mut (& mut self) -> & mut CommandEnv { & mut self . env } pub fn cwd (& mut self , dir : & OsStr) { self . cwd = Some (dir . to_owned ()) ; } pub fn stdin (& mut self , stdin : Stdio) { self . stdin = Some (stdin) ; } pub fn stdout (& mut self , stdout : Stdio) { self . stdout = Some (stdout) ; } pub fn stderr (& mut self , stderr : Stdio) { self . stderr = Some (stderr) ; } pub fn get_program (& self) -> & OsStr { & self . program } pub fn get_args (& self) -> CommandArgs < '_ > { let mut iter = self . args . iter () ; iter . next () ; CommandArgs { iter } } pub fn get_envs (& self) -> CommandEnvs < '_ > { self . env . iter () } pub fn get_current_dir (& self) -> Option < & Path > { self . cwd . as_ref () . map (| cs | Path :: new (cs)) } pub fn spawn (& mut self , _default : Stdio , _needs_stdin : bool ,) -> io :: Result < (Process , StdioPipes) > { unsupported () } }}}
...
mkfn!{
    output_introspect!();
    pub fn output (_cmd : & mut Command) -> io :: Result < (ExitStatus , Vec < u8 > , Vec < u8 >) > { unsupported () }
}
mkitem!{mkimpl!{impl From < AnonPipe > for Stdio { fn from (pipe : AnonPipe) -> Stdio { pipe . diverge () } }}}
mkitem!{mkimpl!{impl From < io :: Stdout > for Stdio { fn from (_ : io :: Stdout) -> Stdio { Stdio :: ParentStdout } }}}
mkitem!{mkimpl!{impl From < io :: Stderr > for Stdio { fn from (_ : io :: Stderr) -> Stdio { Stdio :: ParentStderr } }}}
mkitem!{mkimpl!{impl From < File > for Stdio { fn from (file : File) -> Stdio { Stdio :: InheritFile (file) } }}}
...
mkitem!{mkstruct!{# [derive (PartialEq , Eq , Clone , Copy , Debug , Default)] # [non_exhaustive] pub struct ExitStatus () ;}}
mkitem!{mkimpl!{impl ExitStatus { pub fn exit_ok (& self) -> Result < () , ExitStatusError > { Ok (()) } pub fn code (& self) -> Option < i32 > { Some (0) } }}}
mkitem!{mkimpl!{impl fmt :: Display for ExitStatus { fn fmt (& self , f : & mut fmt :: Formatter < '_ >) -> fmt :: Result { write ! (f , "<dummy exit status>") } }}}
mkitem!{mkstruct!{pub struct ExitStatusError (!) ;}}
mkitem!{mkimpl!{impl Clone for ExitStatusError { fn clone (& self) -> ExitStatusError { self . 0 } }}}
mkitem!{mkimpl!{impl Copy for ExitStatusError { }}}
mkitem!{mkimpl!{impl PartialEq for ExitStatusError { fn eq (& self , _other : & ExitStatusError) -> bool { self . 0 } }}}
mkitem!{mkimpl!{impl Eq for ExitStatusError { }}}
mkitem!{mkimpl!{impl fmt :: Debug for ExitStatusError { fn fmt (& self , _f : & mut fmt :: Formatter < '_ >) -> fmt :: Result { self . 0 } }}}
mkitem!{mkimpl!{impl Into < ExitStatus > for ExitStatusError { fn into (self) -> ExitStatus { self . 0 } }}}
mkitem!{mkimpl!{impl ExitStatusError { pub fn code (self) -> Option < NonZero < i32 > > { self . 0 } }}}
mkitem!{mkstruct!{# [derive (PartialEq , Eq , Clone , Copy , Debug)] pub struct ExitCode (u8) ;}}
mkitem!{mkimpl!{impl ExitCode { pub const SUCCESS : ExitCode = ExitCode (0) ; pub const FAILURE : ExitCode = ExitCode (1) ; pub fn as_i32 (& self) -> i32 { self . 0 as i32 } }}}
...
mkitem!{mkimpl!{impl < 'a > Iterator for CommandArgs < 'a > { type Item = & 'a OsStr ; fn next (& mut self) -> Option < & 'a OsStr > { self . iter . next () . map (| os | & * * os) } fn size_hint (& self) -> (usize , Option < usize >) { self . iter . size_hint () } }}}
