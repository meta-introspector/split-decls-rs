...
mkuse!{use crate :: os :: wasi :: io :: { AsFd , AsRawFd , BorrowedFd , FromRawFd , IntoRawFd , OwnedFd , RawFd } ;}
...
mkuse!{use crate :: { cmp , mem , str } ;}
mkitem!{# [allow (non_camel_case_types)] pub type wrlen_t = size_t ;}
...
mkfn!{
    cvt_r_introspect!();
    pub fn cvt_r < T , F > (mut f : F) -> crate :: io :: Result < T > where T : IsMinusOne , F : FnMut () -> T , { loop { match cvt (f ()) { Err (ref e) if e . is_interrupted () => { } other => return other , } } }
}
...
mkfn!{
    cvt_gai_introspect!();
    pub fn cvt_gai (err : c_int) -> io :: Result < () > { if err == 0 { return Ok (()) ; } if err == netc :: EAI_SYSTEM { return Err (io :: Error :: last_os_error ()) ; } let detail = unsafe { str :: from_utf8 (CStr :: from_ptr (netc :: gai_strerror (err)) . to_bytes ()) . unwrap () . to_owned () } ; Err (io :: Error :: new (io :: ErrorKind :: Uncategorized , & format ! ("failed to lookup address information: {detail}") [..] ,)) }
}
...
mkitem!{mkimpl!{impl FromRawFd for Socket { unsafe fn from_raw_fd (raw_fd : RawFd) -> Self { unsafe { Self (FromRawFd :: from_raw_fd (raw_fd)) } } }}}