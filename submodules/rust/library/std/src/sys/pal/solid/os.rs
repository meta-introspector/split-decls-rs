...
mkuse!{use super :: { error , itron , unsupported } ;}
mkuse!{use crate :: ffi :: { OsStr , OsString } ;}
mkuse!{use crate :: path :: { self , PathBuf } ;}
mkuse!{use crate :: { fmt , io } ;}
mkitem!{mkimpl!{impl itron :: error :: ItronError { #[inline] pub (crate) fn as_io_error (self) -> crate :: io :: Error { crate :: io :: Error :: from_raw_os_error (self . as_raw ()) } }}}
...
mkfn!{
    ...
    pub fn error_string (errno : i32) -> String { if let Some (name) = error :: error_name (errno) { name . to_owned () } else { format ! ("{errno}") } }
}
...
mkitem!{mkstruct!{pub struct SplitPaths < 'a > (& 'a !) ;}}
...
mkfn!{
    temp_dir_introspect!();
    pub fn temp_dir () -> PathBuf { panic ! ("no standard temporary directory on this platform") }
}
...
mkfn!{
    exit_introspect!();
    pub fn exit (code : i32) -> ! { rtabort ! ("exit({}) called" , code) ; }
}
