...
mkfn!{
    errno_introspect!();
    pub fn errno () -> i32 { unsafe { hermit_abi :: get_errno () } }
}
...
mkfn!{
    ...
    pub fn error_string (errno : i32) -> String { hermit_abi :: error_string (errno) . to_string () }
}
...
mkfn!{
    getcwd_introspect!();
    pub fn getcwd () -> io :: Result < PathBuf > { Ok (PathBuf :: from ("/")) }
}
...
mkitem!{mkimpl!{impl fmt :: Display for JoinPathsError { fn fmt (& self , f : & mut fmt :: Formatter < '_ >) -> fmt :: Result { "not supported on hermit yet" . fmt (f) } }}}
...
mkfn!{
    temp_dir_introspect!();
    pub fn temp_dir () -> PathBuf { PathBuf :: from ("/tmp") }
}
...
mkfn!{
    exit_introspect!();
    pub fn exit (code : i32) -> ! { unsafe { hermit_abi :: exit (code) } }
}
...
mkfn!{
    getpid_introspect!();
    pub fn getpid () -> u32 { unsafe { hermit_abi :: getpid () as u32 } }
}