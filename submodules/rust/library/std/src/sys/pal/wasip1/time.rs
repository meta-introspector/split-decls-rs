...
mkitem!{mkstruct!{# [derive (Copy , Clone , PartialEq , Eq , PartialOrd , Ord , Debug , Hash)] pub struct SystemTime (Duration) ;}}
mkitem!{pub const UNIX_EPOCH : SystemTime = SystemTime (Duration :: from_secs (0)) ;}

macro_rules! current_time_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function current_time in module {}", module_path!());
    };
}

mkfn!{
    current_time_introspect!();
    fn current_time (clock : wasi :: Clockid) -> Duration { let ts = unsafe { wasi :: clock_time_get (clock , 1 ,) . unwrap () } ; Duration :: new ((ts / 1_000_000_000) as u64 , (ts % 1_000_000_000) as u32) }
}
mkitem!{mkimpl!{impl Instant { pub fn now () -> Instant { Instant (current_time (wasi :: CLOCKID_MONOTONIC)) } pub fn checked_sub_instant (& self , other : & Instant) -> Option < Duration > { self . 0 . checked_sub (other . 0) } pub fn checked_add_duration (& self , other : & Duration) -> Option < Instant > { Some (Instant (self . 0 . checked_add (* other) ?)) } pub fn checked_sub_duration (& self , other : & Duration) -> Option < Instant > { Some (Instant (self . 0 . checked_sub (* other) ?)) } }}}
