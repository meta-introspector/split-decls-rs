...
mkuse!{use std :: fs :: OpenOptions ;}
mkuse!{use std :: io :: prelude :: * ;}
mkuse!{use rustc_middle :: ty :: { self , ClosureSizeProfileData , Instance , TyCtxt } ;}

macro_rules! dump_closure_profile_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function dump_closure_profile in module {}", module_path!());
    };
}

mkfn!{
    dump_closure_profile_introspect!();
