...
mkuse!{use super :: map :: SsoHashMap ;}
mkitem!{mkstruct!{#[doc = " Small-storage-optimized implementation of a set."] #[doc = ""] #[doc = " Stores elements in a small array up to a certain length"] #[doc = " and switches to `HashSet` when that length is exceeded."] #[derive (Clone)] pub struct SsoHashSet < T > { map : SsoHashMap < T , () > , }}}

macro_rules! entry_to_key_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function entry_to_key in module {}", module_path!());
    };
}

mkfn!{
    entry_to_key_introspect!();
