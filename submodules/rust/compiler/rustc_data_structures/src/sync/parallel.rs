...
mkuse!{use std :: panic :: { AssertUnwindSafe , catch_unwind , resume_unwind } ;}
mkuse!{use parking_lot :: Mutex ;}
mkuse!{use crate :: FatalErrorMarker ;}
mkuse!{use crate :: sync :: { DynSend , DynSync , FromDyn , IntoDynSyncSend , mode } ;}
mkitem!{mkstruct!{# [doc = " A guard used to hold panics that occur during a parallel section to later by unwound."] # [doc = " This is used for the parallel compiler to prevent fatal errors from non-deterministically"] # [doc = " hiding errors by ensuring that everything in the section has completed executing before"] # [doc = " continuing with unwinding. It's also used for the non-parallel code to ensure error message"] # [doc = " output match the parallel compiler for testing purposes."] pub struct ParallelGuard { panic : Mutex < Option < IntoDynSyncSend < Box < dyn Any + Send + 'static > > > > , }}}
mkitem!{mkimpl!{impl ParallelGuard { pub fn run < R > (& self , f : impl FnOnce () -> R) -> Option < R > { catch_unwind (AssertUnwindSafe (f)) . map_err (| err | { let mut panic = self . panic . lock () ; if panic . is_none () || ! (* err) . is :: < FatalErrorMarker > () { * panic = Some (IntoDynSyncSend (err)) ; } }) . ok () } }}}

macro_rules! parallel_guard_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function parallel_guard in module {}", module_path!());
    };
}

mkfn!{
    parallel_guard_introspect!();
...
macro_rules! serial_join_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function serial_join in module {}", module_path!());
    };
}

mkfn!{
    serial_join_introspect!();
...
macro_rules! par_slice_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function par_slice in module {}", module_path!());
    };
}

mkfn!{
    par_slice_introspect!();
...
macro_rules! par_for_each_in_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function par_for_each_in in module {}", module_path!());
    };
}

mkfn!{
    par_for_each_in_introspect!();
