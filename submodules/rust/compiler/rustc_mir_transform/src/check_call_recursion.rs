...
mkuse!{use rustc_data_structures :: graph :: iterate :: { NodeStatus , TriColorDepthFirstSearch , TriColorVisitor , } ;}
mkuse!{use rustc_hir :: LangItem ;}
mkuse!{use rustc_hir :: def :: DefKind ;}
mkuse!{use rustc_middle :: mir :: { self , BasicBlock , BasicBlocks , Body , Terminator , TerminatorKind } ;}
mkuse!{use rustc_middle :: ty :: { self , GenericArg , GenericArgs , Instance , Ty , TyCtxt } ;}
mkuse!{use rustc_session :: lint :: builtin :: UNCONDITIONAL_RECURSION ;}
mkuse!{use rustc_span :: Span ;}
mkuse!{use crate :: errors :: UnconditionalRecursion ;}
mkuse!{use crate :: pass_manager :: MirLint ;}
mkitem!{mkstruct!{pub (super) struct CheckCallRecursion ;}}
mkitem!{mkimpl!{impl < 'tcx > MirLint < 'tcx > for CheckCallRecursion { fn run_lint (& self , tcx : TyCtxt < 'tcx > , body : & Body < 'tcx >) { let def_id = body . source . def_id () . expect_local () ; if let DefKind :: Fn | DefKind :: AssocFn = tcx . def_kind (def_id) { let trait_args = match tcx . trait_of_assoc (def_id . to_def_id ()) { Some (trait_def_id) => { let trait_args_count = tcx . generics_of (trait_def_id) . count () ; & GenericArgs :: identity_for_item (tcx , def_id) [.. trait_args_count] } _ => & [] , } ; check_recursion (tcx , body , CallRecursion { trait_args }) } } }}}
mkitem!{mkstruct!{# [doc = " Requires drop elaboration to have been performed."] pub (super) struct CheckDropRecursion ;}}
mkitem!{mkimpl!{impl < 'tcx > MirLint < 'tcx > for CheckDropRecursion { fn run_lint (& self , tcx : TyCtxt < 'tcx > , body : & Body < 'tcx >) { let def_id = body . source . def_id () . expect_local () ; if let DefKind :: AssocFn = tcx . def_kind (def_id) && let Some (impl_id) = tcx . trait_impl_of_assoc (def_id . to_def_id ()) && let trait_ref = tcx . impl_trait_ref (impl_id) . unwrap () && tcx . is_lang_item (trait_ref . instantiate_identity () . def_id , LangItem :: Drop) && let sig = tcx . fn_sig (def_id) . instantiate_identity () && sig . inputs () . skip_binder () . len () == 1 { if let ty :: Ref (_ , dropped_ty , _) = tcx . liberate_late_bound_regions (def_id . to_def_id () , sig . input (0)) . kind () { check_recursion (tcx , body , RecursiveDrop { drop_for : * dropped_ty }) ; } } } }}}

macro_rules! check_recursion_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_recursion in module {}", module_path!());
    };
}

mkfn!{
    check_recursion_introspect!();
