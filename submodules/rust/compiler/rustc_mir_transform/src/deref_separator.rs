...
mkuse!{use rustc_middle :: mir :: visit :: NonUseContext :: VarDebugInfo ;}
mkuse!{use rustc_middle :: mir :: visit :: { MutVisitor , PlaceContext } ;}
mkuse!{use rustc_middle :: mir :: * ;}
mkuse!{use rustc_middle :: ty :: TyCtxt ;}
mkuse!{use crate :: patch :: MirPatch ;}
mkitem!{mkstruct!{pub (super) struct Derefer ;}}
mkitem!{mkstruct!{struct DerefChecker < 'a , 'tcx > { tcx : TyCtxt < 'tcx > , patcher : MirPatch < 'tcx > , local_decls : & 'a LocalDecls < 'tcx > , }}}
...
macro_rules! deref_finder_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function deref_finder in module {}", module_path!());
    };
}

mkfn!{
    deref_finder_introspect!();
    pub (super) fn deref_finder < 'tcx > (tcx : TyCtxt < 'tcx > , body : & mut Body < 'tcx >) { let patch = MirPatch :: new (body) ; let mut checker = DerefChecker { tcx , patcher : patch , local_decls : & body . local_decls } ; for (bb , data) in body . basic_blocks . as_mut_preserves_cfg () . iter_enumerated_mut () { checker . visit_basic_block_data (bb , data) ; } checker . patcher . apply (body) ; }
}
mkitem!{mkimpl!{impl < 'tcx > crate :: MirPass < 'tcx > for Derefer { fn run_pass (& self , tcx : TyCtxt < 'tcx > , body : & mut Body < 'tcx >) { deref_finder (tcx , body) ; } fn is_required (& self) -> bool { true } }}}