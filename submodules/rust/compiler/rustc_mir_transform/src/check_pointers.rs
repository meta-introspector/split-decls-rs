...
mkitem!{mkstruct!{# [doc = " Details of a pointer check, the condition on which we decide whether to"] # [doc = " fail the assert and an [AssertKind] that defines the behavior on failure."] pub (crate) struct PointerCheck < 'tcx > { pub (crate) cond : Operand < 'tcx > , pub (crate) assert_kind : Box < AssertKind < Operand < 'tcx > > > , }}}
mkitem!{mkenum!{# [doc = " When checking for borrows of field projections (`&(*ptr).a`), we might want"] # [doc = " to check for the field type (type of `.a` in the example). This enum defines"] # [doc = " the variations (pass the pointer [Ty] or the field [Ty])."] # [derive (Copy , Clone)] pub (crate) enum BorrowedFieldProjectionMode { FollowProjections , NoFollowProjections , }}}

macro_rules! check_pointers_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_pointers in module {}", module_path!());
    };
}

mkfn!{
    check_pointers_introspect!();
