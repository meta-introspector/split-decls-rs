...
mkuse!{use rustc_middle :: ty :: { self , GenericArgsRef , Instance , Ty , TypeVisitableExt } ;}
mkuse!{use rustc_session :: Session ;}
mkuse!{use rustc_session :: config :: { self , DebugInfo } ;}
mkuse!{use rustc_span :: { BytePos , Pos , SourceFile , SourceFileAndLine , SourceFileHash , Span , StableSourceFileId , Symbol , } ;}
mkuse!{use rustc_target :: callconv :: FnAbi ;}
mkuse!{use rustc_target :: spec :: DebuginfoKind ;}
mkuse!{use smallvec :: SmallVec ;}
mkuse!{use tracing :: debug ;}
mkuse!{use self :: metadata :: { UNKNOWN_COLUMN_NUMBER , UNKNOWN_LINE_NUMBER , file_metadata , spanned_type_di_node , type_di_node , } ;}
mkuse!{use self :: namespace :: mangled_name_of_instance ;}
mkuse!{use self :: utils :: { DIB , create_DIArray , is_node_local_to_unit } ;}
mkuse!{use crate :: builder :: Builder ;}
mkuse!{use crate :: common :: { AsCCharPtr , CodegenCx } ;}
mkuse!{use crate :: llvm ;}
mkuse!{use crate :: llvm :: debuginfo :: { DIArray , DIBuilderBox , DIFile , DIFlags , DILexicalBlock , DILocation , DISPFlags , DIScope , DITemplateTypeParameter , DIType , DIVariable , } ;}
mkuse!{use crate :: value :: Value ;}
mkmod!{create_scope_map, { 
                getname!(create_scope_map);
                getsrc!(create_scope_map);
                getpath!(create_scope_map);
                get_deps!(create_scope_map);
                get_crates!(create_scope_map);
                mkinclude!(create_scope_map);
...
mkmod!{dwarf_const, { 
                ...
                getsrc!(dwarf_const);
                getpath!(dwarf_const);
                get_deps!(dwarf_const);
                get_crates!(dwarf_const);
                mkinclude!(dwarf_const);
