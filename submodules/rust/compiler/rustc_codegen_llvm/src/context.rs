...
mkuse!{use std :: ffi :: { CStr , c_char , c_uint } ;}
mkuse!{use std :: marker :: PhantomData ;}
mkuse!{use std :: ops :: { Deref , DerefMut } ;}
mkuse!{use std :: str ;}
mkuse!{use rustc_abi :: { HasDataLayout , Size , TargetDataLayout , VariantIdx } ;}
mkuse!{use rustc_codegen_ssa :: back :: versioned_llvm_target ;}
mkuse!{use rustc_codegen_ssa :: base :: { wants_msvc_seh , wants_wasm_eh } ;}
...
mkuse!{use rustc_middle :: middle :: codegen_fn_attrs :: PatchableFunctionEntry ;}
mkuse!{use rustc_middle :: mir :: mono :: CodegenUnit ;}
mkuse!{use rustc_middle :: ty :: layout :: { FnAbiError , FnAbiOfHelpers , FnAbiRequest , HasTypingEnv , LayoutError , LayoutOfHelpers , } ;}
mkuse!{use rustc_middle :: ty :: { self , Instance , Ty , TyCtxt } ;}
mkuse!{use rustc_middle :: { bug , span_bug } ;}
mkuse!{use rustc_session :: Session ;}
mkuse!{use rustc_session :: config :: { BranchProtection , CFGuard , CFProtection , CrateType , DebugInfo , FunctionReturn , PAuthKey , PacRet , } ;}
...
macro_rules! create_module_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function create_module in module {}", module_path!());
    };
}

mkfn!{
    create_module_introspect!();
