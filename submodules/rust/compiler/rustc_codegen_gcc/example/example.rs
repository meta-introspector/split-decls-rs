...
mkfn!{
    bcd_introspect!();
    fn bcd (b : bool , a : u8) -> u8 { if b { a * 2 } else { a * 3 } }
}
...
mkfn!{
    ...
    fn indirect_call () { let f : fn () = call ; f () ; }
}
mkitem!{mkenum!{enum BoolOption { Some (bool) , None , }}}
...
mkfn!{
    ...
    fn option_unwrap_or (o : BoolOption , d : bool) -> bool { match o { BoolOption :: Some (b) => b , BoolOption :: None => d , } }
}
...
mkfn!{
    ...
    fn return_str () -> & 'static str { "hello world" }
}
...
mkfn!{
    ...
    fn cast_ref_to_raw_ptr (abc : & u8) -> * const u8 { abc as * const u8 }
}
...
mkfn!{
    ...
    fn cmp_raw_ptr (a : * const u8 , b : * const u8) -> bool { a == b }
}
...
mkfn!{
    int_cast_introspect!();
    fn int_cast (a : u16 , b : i16) -> (u8 , u16 , u32 , usize , i8 , i16 , i32 , isize , u8 , u32) { (a as u8 , a as u16 , a as u32 , a as usize , a as i8 , a as i16 , a as i32 , a as isize , b as u8 , b as u32 ,) }
}
...
mkfn!{
    ...
    fn char_cast (c : char) -> u8 { c as u8 }
}
...
mkfn!{
    ...
    fn debug_tuple () -> DebugTuple { DebugTuple (()) }
}
...
mkfn!{
    size_of_introspect!();
    fn size_of < T > () -> usize { intrinsics :: size_of :: < T > () }
}
...
mkfn!{
    ...
    fn use_size_of () -> usize { size_of :: < u64 > () }
}
...
mkfn!{
    ...
    unsafe fn use_copy_intrinsic (src : * const u8 , dst : * mut u8) { intrinsics :: copy :: < u8 > (src , dst , 1) ; }
}
...
mkfn!{
    ...
    unsafe fn use_copy_intrinsic_ref (src : * const u8 , dst : * mut u8) { let copy2 = & intrinsics :: copy :: < u8 > ; copy2 (src , dst , 1) ; }
}
mkitem!{const ABC : u8 = 6 * 7 ;}
...
mkitem!{mkstruct!{struct IsNotEmpty ;}}
...
mkfn!{
    eq_char_introspect!();
    fn eq_char (a : char , b : char) -> bool { a == b }
}
...
mkfn!{
    ...
    unsafe fn transmute (c : char) -> u32 { intrinsics :: transmute (c) }
}
...
mkfn!{
    ...
    unsafe fn deref_str_ptr (s : * const str) -> & 'static str { & * s }
}
...
mkfn!{
    ...
    fn use_array (arr : [u8 ; 3]) -> u8 { arr [1] }
}
...
mkfn!{
    ...
    fn array_as_slice (arr : & [u8 ; 3]) -> & [u8] { arr }
}
...
mkfn!{
    ...
    unsafe fn use_ctlz_nonzero (a : u16) -> u32 { intrinsics :: ctlz_nonzero (a) }
}
...
mkfn!{
    ...
    fn ptr_as_usize (ptr : * const u8) -> usize { ptr as usize }
}
...
mkfn!{
    ...
    fn float_cast (a : f32 , b : f64) -> (f64 , f32) { (a as f64 , b as f32) }
}
...
mkfn!{
    ...
    fn int_to_float (a : u8 , b : i32) -> (f64 , f32) { (a as f64 , b as f32) }
}
...
mkfn!{
    ...
    fn some_promoted_tuple () -> & 'static (& 'static str , & 'static str) { & ("abc" , "some") }
}
...
mkfn!{
    ...
    fn str_wrapper_get (w : & StrWrapper) -> & str { & w . s }
}
...
mkfn!{
    ...
    fn i16_as_i8 (a : i16) -> i8 { a as i8 }
}
mkitem!{mkstruct!{struct Unsized (u8 , str) ;}}
...
mkfn!{
    ...
    fn get_sized_field_ref_from_unsized_type (u : & Unsized) -> & u8 { & u . 0 }
}
...
mkfn!{
    ...
    fn get_unsized_field_ref_from_unsized_type (u : & Unsized) -> & str { & u . 1 }
}
