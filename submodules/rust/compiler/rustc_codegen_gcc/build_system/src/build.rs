...
mkuse!{use crate :: config :: { Channel , ConfigInfo } ;}
mkuse!{use crate :: utils :: { create_dir , get_sysroot_dir , run_command , run_command_with_output_and_env , walk_dir , } ;}
mkitem!{mkstruct!{# [derive (Default)] struct BuildArg { flags : Vec < String > , config_info : ConfigInfo , build_sysroot : bool , }}}
mkitem!{mkimpl!{impl BuildArg { # [doc = " Creates a new `BuildArg` instance by parsing command-line arguments."] fn new () -> Result < Option < Self > , String > { let mut build_arg = Self :: default () ; let mut args = std :: env :: args () . skip (2) ; while let Some (arg) = args . next () { match arg . as_str () { "--sysroot" => { build_arg . build_sysroot = true ; } "--help" => { Self :: usage () ; return Ok (None) ; } arg => { if ! build_arg . config_info . parse_argument (arg , & mut args) ? { return Err (format ! ("Unknown argument `{arg}`")) ; } } } } Ok (Some (build_arg)) } fn usage () { println ! (r#"
`build` command help:

    --sysroot              : Build with sysroot"#) ; ConfigInfo :: show_usage () ; println ! ("    --help                 : Show this help") ; } }}}

macro_rules! cleanup_sysroot_previous_build_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function cleanup_sysroot_previous_build in module {}", module_path!());
    };
}

mkfn!{
    cleanup_sysroot_previous_build_introspect!();
...
macro_rules! build_codegen_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function build_codegen in module {}", module_path!());
    };
}

mkfn!{
    build_codegen_introspect!();
