mkuse!{use rustc_middle :: mir :: { self , BasicBlock , Location , traversal } ;}
mkuse!{use super :: { Analysis , Direction , Results } ;}

macro_rules! visit_results_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function visit_results in module {}", module_path!());
    };
}

mkfn!{
    visit_results_introspect!();
...
macro_rules! visit_reachable_results_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function visit_reachable_results in module {}", module_path!());
    };
}

mkfn!{
    visit_reachable_results_introspect!();
