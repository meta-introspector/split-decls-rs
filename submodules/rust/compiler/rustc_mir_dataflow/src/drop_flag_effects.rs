...
mkuse!{use rustc_middle :: mir :: { self , Body , Location , Terminator , TerminatorKind } ;}
mkuse!{use smallvec :: SmallVec ;}
mkuse!{use tracing :: debug ;}
mkuse!{use super :: move_paths :: { InitKind , LookupResult , MoveData , MovePathIndex } ;}
mkitem!{mkenum!{# [doc = " The value of an inserted drop flag."] # [derive (Debug , PartialEq , Eq , Copy , Clone)] pub enum DropFlagState { # [doc = " The tracked value is initialized and needs to be dropped when leaving its scope."] Present , # [doc = " The tracked value is uninitialized or was moved out of and does not need to be dropped when"] # [doc = " leaving its scope."] Absent , }}}
mkitem!{mkimpl!{impl DropFlagState { pub fn value (self) -> bool { match self { DropFlagState :: Present => true , DropFlagState :: Absent => false , } } }}}

macro_rules! move_path_children_matching_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function move_path_children_matching in module {}", module_path!());
    };
}

mkfn!{
    move_path_children_matching_introspect!();
...
macro_rules! on_lookup_result_bits_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function on_lookup_result_bits in module {}", module_path!());
    };
}

mkfn!{
    on_lookup_result_bits_introspect!();
...
macro_rules! on_all_children_bits_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function on_all_children_bits in module {}", module_path!());
    };
}

mkfn!{
    on_all_children_bits_introspect!();
...
macro_rules! drop_flag_effects_for_function_entry_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function drop_flag_effects_for_function_entry in module {}", module_path!());
    };
}

mkfn!{
    drop_flag_effects_for_function_entry_introspect!();
...
macro_rules! drop_flag_effects_for_location_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function drop_flag_effects_for_location in module {}", module_path!());
    };
}

mkfn!{
    drop_flag_effects_for_location_introspect!();
...
macro_rules! for_location_inits_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function for_location_inits in module {}", module_path!());
    };
}

mkfn!{
    for_location_inits_introspect!();
...
mkitem!{mkenum!{# [doc = " Indicates which variants are inactive at a `SwitchInt` edge by listing their `VariantIdx`s or"] # [doc = " specifying the single active variant's `VariantIdx`."] pub (crate) enum InactiveVariants { Inactives (SmallVec < [VariantIdx ; 4] >) , Active (VariantIdx) , }}}
mkitem!{mkimpl!{impl InactiveVariants { fn contains (& self , variant_idx : VariantIdx) -> bool { match self { InactiveVariants :: Inactives (inactives) => inactives . contains (& variant_idx) , InactiveVariants :: Active (active) => variant_idx != * active , } } }}}

macro_rules! on_all_inactive_variants_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function on_all_inactive_variants in module {}", module_path!());
    };
}

mkfn!{
    on_all_inactive_variants_introspect!();
