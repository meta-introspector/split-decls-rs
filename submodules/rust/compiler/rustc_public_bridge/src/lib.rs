...
mkuse!{use bridge :: * ;}
mkuse!{use context :: CompilerCtxt ;}
mkuse!{use rustc_data_structures :: fx :: { self , FxIndexMap } ;}
mkuse!{use rustc_middle :: mir ;}
mkuse!{use rustc_middle :: mir :: interpret :: AllocId ;}
mkuse!{use rustc_middle :: ty :: { self , Ty , TyCtxt } ;}
mkuse!{use rustc_span :: Span ;}
mkuse!{use rustc_span :: def_id :: { CrateNum , DefId , LOCAL_CRATE } ;}
mkmod!{alloc, { 
                getname!(alloc);
                getsrc!(alloc);
                getpath!(alloc);
...
mkitem!{mktrait!{pub trait IndexedVal { fn to_val (index : usize) -> Self ; fn to_index (& self) -> usize ; }}}
mkitem!{mkstruct!{# [doc = " Similar to rustc's `FxIndexMap`, `IndexMap` with extra"] # [doc = " safety features added."] pub struct IndexMap < K , V > { index_map : fx :: FxIndexMap < K , V > , }}}
mkitem!{mkimpl!{impl < K , V > Default for IndexMap < K , V > { fn default () -> Self { Self { index_map : FxIndexMap :: default () } } }}}
mkitem!{mkimpl!{impl < K : PartialEq + Hash + Eq , V : Copy + Debug + PartialEq + IndexedVal > IndexMap < K , V > { pub fn create_or_fetch (& mut self , key : K) -> V { let len = self . index_map . len () ; let v = self . index_map . entry (key) . or_insert (V :: to_val (len)) ; * v } }}}
mkitem!{mkimpl!{impl < K : PartialEq + Hash + Eq , V : Copy + Debug + PartialEq + IndexedVal > Index < V > for IndexMap < K , V > { type Output = K ; fn index (& self , index : V) -> & Self :: Output { let (k , v) = self . index_map . get_index (index . to_index ()) . unwrap () ; assert_eq ! (* v , index , "Provided value doesn't match with indexed value") ; k } }}}

macro_rules! filter_def_ids_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function filter_def_ids in module {}", module_path!());
    };
}

mkfn!{
    filter_def_ids_introspect!();
