...
mkuse!{use std :: mem ;}
mkuse!{use hashbrown :: hash_table :: Entry ;}
mkuse!{use rustc_data_structures :: fingerprint :: Fingerprint ;}
mkuse!{use rustc_data_structures :: sharded :: { self , Sharded } ;}
mkuse!{use rustc_data_structures :: stack :: ensure_sufficient_stack ;}
mkuse!{use rustc_data_structures :: { outline , sync } ;}
mkuse!{use rustc_errors :: { Diag , FatalError , StashKey } ;}
mkuse!{use rustc_span :: { DUMMY_SP , Span } ;}
mkuse!{use tracing :: instrument ;}
mkuse!{use super :: { QueryConfig , QueryStackFrameExtra } ;}
mkuse!{use crate :: HandleCycleError ;}
mkuse!{use crate :: dep_graph :: { DepContext , DepGraphData , DepNode , DepNodeIndex , DepNodeParams } ;}
mkuse!{use crate :: ich :: StableHashingContext ;}
mkuse!{use crate :: query :: caches :: QueryCache ;}
mkuse!{use crate :: query :: job :: { QueryInfo , QueryJob , QueryJobId , QueryJobInfo , QueryLatch , report_cycle } ;}
mkuse!{use crate :: query :: { QueryContext , QueryMap , QueryStackFrame , SerializedDepNodeIndex } ;}

macro_rules! equivalent_key_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function equivalent_key in module {}", module_path!());
    };
}

mkfn!{
    equivalent_key_introspect!();
    # [inline] fn equivalent_key < K : Eq , V > (k : & K) -> impl Fn (& (K , V)) -> bool + '_ { move | x | x . 0 == * k }
}
mkitem!{mkstruct!{pub struct QueryState < K , I > { active : Sharded < hashbrown :: HashTable < (K , QueryResult < I >) > > , }}}
mkitem!{mkenum!{# [doc = " Indicates the state of a query for a given key in a query map."] enum QueryResult < I > { # [doc = " An already executing query. The query job can be used to await for its completion."] Started (QueryJob < I >) , # [doc = " The query panicked. Queries trying to wait on this will raise a fatal error which will"] # [doc = " silently panic."] Poisoned , }}}
mkitem!{mkimpl!{impl < I > QueryResult < I > { # [doc = " Unwraps the query job expecting that it has started."] fn expect_job (self) -> QueryJob < I > { match self { Self :: Started (job) => job , Self :: Poisoned => { panic ! ("job for query failed to start and was poisoned") } } } }}}
mkitem!{mkimpl!{impl < K , I > QueryState < K , I > where K : Eq + Hash + Copy + Debug , { pub fn all_inactive (& self) -> bool { self . active . lock_shards () . all (| shard | shard . is_empty ()) } pub fn try_collect_active_jobs < Qcx : Copy > (& self , qcx : Qcx , make_query : fn (Qcx , K) -> QueryStackFrame < I > , jobs : & mut QueryMap < I > ,) -> Option < () > { let mut active = Vec :: new () ; for shard in self . active . try_lock_shards () { for (k , v) in shard ? . iter () { if let QueryResult :: Started (ref job) = * v { active . push ((* k , (* job) . clone ())) ; } } } for (key , job) in active { let query = make_query (qcx , key) ; jobs . insert (job . id , QueryJobInfo { query , job }) ; } Some (()) } }}}
mkitem!{mkimpl!{impl < K , I > Default for QueryState < K , I > { fn default () -> QueryState < K , I > { QueryState { active : Default :: default () } } }}}
mkitem!{mkstruct!{# [doc = " A type representing the responsibility to execute the job in the `job` field."] # [doc = " This will poison the relevant query if dropped."] struct JobOwner < 'tcx , K , I > where K : Eq + Hash + Copy , { state : & 'tcx QueryState < K , I > , key : K , }}}

macro_rules! mk_cycle_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function mk_cycle in module {}", module_path!());
    };
}
...
macro_rules! handle_cycle_error_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function handle_cycle_error in module {}", module_path!());
    };
}

mkfn!{
    handle_cycle_error_introspect!();
...
macro_rules! cycle_error_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function cycle_error in module {}", module_path!());
    };
}

mkfn!{
    cycle_error_introspect!();
...
macro_rules! wait_for_query_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function wait_for_query in module {}", module_path!());
    };
}

mkfn!{
    wait_for_query_introspect!();
...
macro_rules! try_execute_query_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function try_execute_query in module {}", module_path!());
    };
}

mkfn!{
    try_execute_query_introspect!();
...
macro_rules! execute_job_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function execute_job in module {}", module_path!());
    };
}

mkfn!{
    execute_job_introspect!();
...
macro_rules! execute_job_non_incr_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function execute_job_non_incr in module {}", module_path!());
    };
}

mkfn!{
    execute_job_non_incr_introspect!();
...
macro_rules! execute_job_incr_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function execute_job_incr in module {}", module_path!());
    };
}

mkfn!{
    execute_job_incr_introspect!();
...
macro_rules! try_load_from_disk_and_cache_in_memory_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function try_load_from_disk_and_cache_in_memory in module {}", module_path!());
    };
}

mkfn!{
    try_load_from_disk_and_cache_in_memory_introspect!();
...
macro_rules! incremental_verify_ich_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function incremental_verify_ich in module {}", module_path!());
    };
}

mkfn!{
    incremental_verify_ich_introspect!();
...
macro_rules! incremental_verify_ich_not_green_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function incremental_verify_ich_not_green in module {}", module_path!());
    };
}

mkfn!{
    incremental_verify_ich_not_green_introspect!();
...
macro_rules! incremental_verify_ich_failed_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function incremental_verify_ich_failed in module {}", module_path!());
    };
}

mkfn!{
    incremental_verify_ich_failed_introspect!();
...
macro_rules! ensure_must_run_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function ensure_must_run in module {}", module_path!());
    };
}

mkfn!{
    ensure_must_run_introspect!();
...
mkitem!{mkenum!{# [derive (Debug)] pub enum QueryMode { Get , Ensure { check_cache : bool } , }}}

macro_rules! get_query_non_incr_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function get_query_non_incr in module {}", module_path!());
    };
}

mkfn!{
    get_query_non_incr_introspect!();
...
macro_rules! get_query_incr_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function get_query_incr in module {}", module_path!());
    };
}

mkfn!{
    get_query_incr_introspect!();
...
macro_rules! force_query_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function force_query in module {}", module_path!());
    };
}

mkfn!{
    force_query_introspect!();
