...
macro_rules! enabled_names_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function enabled_names in module {}", module_path!());
    };
}

mkfn!{
    enabled_names_introspect!();
...
macro_rules! extern_abi_enabled_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function extern_abi_enabled in module {}", module_path!());
    };
}

mkfn!{
    extern_abi_enabled_introspect!();
...
macro_rules! gate_unstable_abi_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function gate_unstable_abi in module {}", module_path!());
    };
}

mkfn!{
    gate_unstable_abi_introspect!();
...
mkitem!{mkstruct!{pub struct UnstableAbi { abi : ExternAbi , feature : Symbol , explain : GateReason , }}}
mkitem!{mkenum!{enum GateReason { Experimental , ImplDetail , }}}
mkitem!{mkimpl!{impl fmt :: Display for UnstableAbi { fn fmt (& self , f : & mut fmt :: Formatter < '_ >) -> fmt :: Result { let Self { abi , .. } = self ; match self . explain { GateReason :: Experimental => { write ! (f , "the extern {abi} ABI is experimental and subject to change") } GateReason :: ImplDetail => { write ! (f , "the extern {abi} ABI is an implementation detail and perma-unstable") } } } }}}

macro_rules! extern_abi_stability_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function extern_abi_stability in module {}", module_path!());
    };
}

mkfn!{
    extern_abi_stability_introspect!();
