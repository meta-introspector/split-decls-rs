...
macro_rules! nested_bodies_within_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function nested_bodies_within in module {}", module_path!());
    };
}

mkfn!{
    nested_bodies_within_introspect!();
...
mkfn!{
    provide_introspect!();
    pub (super) fn provide (providers : & mut Providers) { * providers = Providers { nested_bodies_within , .. * providers } ; }
}