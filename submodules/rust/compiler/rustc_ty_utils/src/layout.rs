...
mkuse!{use rustc_abi :: Integer :: { I8 , I32 } ;}
mkuse!{use rustc_abi :: Primitive :: { self , Float , Int , Pointer } ;}
mkuse!{use rustc_abi :: { AddressSpace , BackendRepr , FIRST_VARIANT , FieldIdx , FieldsShape , HasDataLayout , Layout , LayoutCalculatorError , LayoutData , Niche , ReprOptions , Scalar , Size , StructKind , TagEncoding , VariantIdx , Variants , WrappingRange , } ;}
mkuse!{use rustc_hashes :: Hash64 ;}
mkuse!{use rustc_index :: IndexVec ;}
mkuse!{use rustc_middle :: bug ;}
mkuse!{use rustc_middle :: query :: Providers ;}
mkuse!{use rustc_middle :: ty :: layout :: { FloatExt , HasTyCtxt , IntegerExt , LayoutCx , LayoutError , LayoutOf , TyAndLayout , } ;}
mkuse!{use rustc_middle :: ty :: print :: with_no_trimmed_paths ;}
mkuse!{use rustc_middle :: ty :: { self , AdtDef , CoroutineArgsExt , EarlyBinder , PseudoCanonicalInput , Ty , TyCtxt , TypeVisitableExt , } ;}
mkuse!{use rustc_session :: { DataTypeKind , FieldInfo , FieldKind , SizeKind , VariantInfo } ;}
mkuse!{use rustc_span :: { Symbol , sym } ;}
mkuse!{use tracing :: { debug , instrument } ;}
mkuse!{use { rustc_abi as abi , rustc_hir as hir } ;}
mkuse!{use crate :: errors :: { NonPrimitiveSimdType , OversizedSimdType , ZeroLengthSimdType } ;}
mkmod!{invariant, { 
                getname!(invariant);
                getsrc!(invariant);
                getpath!(invariant);
                get_deps!(invariant);
                get_crates!(invariant);
                mkinclude!(invariant);
...
mkfn!{
    provide_introspect!();
    pub (crate) fn provide (providers : & mut Providers) { * providers = Providers { layout_of , .. * providers } ; }
}
...
macro_rules! map_error_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function map_error in module {}", module_path!());
    };
}

mkfn!{
    map_error_introspect!();
...
macro_rules! extract_const_value_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function extract_const_value in module {}", module_path!());
    };
}

mkfn!{
    extract_const_value_introspect!();
...
macro_rules! layout_of_uncached_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function layout_of_uncached in module {}", module_path!());
    };
}

mkfn!{
    layout_of_uncached_introspect!();
...
macro_rules! record_layout_for_printing_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function record_layout_for_printing in module {}", module_path!());
    };
}

mkfn!{
    record_layout_for_printing_introspect!();
...
macro_rules! variant_info_for_adt_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function variant_info_for_adt in module {}", module_path!());
    };
}

mkfn!{
    variant_info_for_adt_introspect!();
    fn variant_info_for_adt < 'tcx > (cx : & LayoutCx < 'tcx > , layout : TyAndLayout < 'tcx > , adt_def : AdtDef < 'tcx > ,) -> (Vec < VariantInfo > , Option < Size >) { let build_variant_info = | n : Option < Symbol > , flds : & [Symbol] , layout : TyAndLayout < 'tcx > | { let mut min_size = Size :: ZERO ; let field_info : Vec < _ > = flds . iter () . enumerate () . map (| (i , & name) | { let field_layout = layout . field (cx , i) ; let offset = layout . fields . offset (i) ; min_size = min_size . max (offset + field_layout . size) ; FieldInfo { kind : FieldKind :: AdtField , name , offset : offset . bytes () , size : field_layout . size . bytes () , align : field_layout . align . abi . bytes () , type_name : None , } }) . collect () ; VariantInfo { name : n , kind : if layout . is_unsized () { SizeKind :: Min } else { SizeKind :: Exact } , align : layout . align . abi . bytes () , size : if min_size . bytes () == 0 { layout . size . bytes () } else { min_size . bytes () } , fields : field_info , } } ; match layout . variants { Variants :: Empty => (vec ! [] , None) , Variants :: Single { index } => { debug ! ("print-type-size `{:#?}` variant {}" , layout , adt_def . variant (index) . name) ; let variant_def = & adt_def . variant (index) ; let fields : Vec < _ > = variant_def . fields . iter () . map (| f | f . name) . collect () ; (vec ! [build_variant_info (Some (variant_def . name) , & fields , layout)] , None) } Variants :: Multiple { tag , ref tag_encoding , .. } => { debug ! ("print-type-size `{:#?}` adt general variants def {}" , layout . ty , adt_def . variants () . len ()) ; let variant_infos : Vec < _ > = adt_def . variants () . iter_enumerated () . map (| (i , variant_def) | { let fields : Vec < _ > = variant_def . fields . iter () . map (| f | f . name) . collect () ; build_variant_info (Some (variant_def . name) , & fields , layout . for_variant (cx , i)) }) . collect () ; (variant_infos , match tag_encoding { TagEncoding :: Direct => Some (tag . size (cx)) , _ => None , } ,) } } }
}

macro_rules! variant_info_for_coroutine_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function variant_info_for_coroutine in module {}", module_path!());
    };
}

mkfn!{
    variant_info_for_coroutine_introspect!();
    fn variant_info_for_coroutine < 'tcx > (cx : & LayoutCx < 'tcx > , layout : TyAndLayout < 'tcx > , def_id : DefId , args : ty :: GenericArgsRef < 'tcx > ,) -> (Vec < VariantInfo > , Option < Size >) { use itertools :: Itertools ; let Variants :: Multiple { tag , ref tag_encoding , tag_field , .. } = layout . variants else { return (vec ! [] , None) ; } ; let coroutine = cx . tcx () . coroutine_layout (def_id , args) . unwrap () ; let upvar_names = cx . tcx () . closure_saved_names_of_captured_variables (def_id) ; let mut upvars_size = Size :: ZERO ; let upvar_fields : Vec < _ > = args . as_coroutine () . upvar_tys () . iter () . zip_eq (upvar_names) . enumerate () . map (| (field_idx , (_ , name)) | { let field_layout = layout . field (cx , field_idx) ; let offset = layout . fields . offset (field_idx) ; upvars_size = upvars_size . max (offset + field_layout . size) ; FieldInfo { kind : FieldKind :: Upvar , name : * name , offset : offset . bytes () , size : field_layout . size . bytes () , align : field_layout . align . abi . bytes () , type_name : None , } }) . collect () ; let mut variant_infos : Vec < _ > = coroutine . variant_fields . iter_enumerated () . map (| (variant_idx , variant_def) | { let variant_layout = layout . for_variant (cx , variant_idx) ; let mut variant_size = Size :: ZERO ; let fields = variant_def . iter () . enumerate () . map (| (field_idx , local) | { let field_name = coroutine . field_names [* local] ; let field_layout = variant_layout . field (cx , field_idx) ; let offset = variant_layout . fields . offset (field_idx) ; variant_size = variant_size . max (offset + field_layout . size) ; FieldInfo { kind : FieldKind :: CoroutineLocal , name : field_name . unwrap_or_else (| | { Symbol :: intern (& format ! (".coroutine_field{}" , local . as_usize ())) }) , offset : offset . bytes () , size : field_layout . size . bytes () , align : field_layout . align . abi . bytes () , type_name : (field_name . is_none () || field_name == Some (sym :: __awaitee)) . then (| | Symbol :: intern (& field_layout . ty . to_string ())) , } }) . chain (upvar_fields . iter () . copied ()) . collect () ; if variant_size == Size :: ZERO { variant_size = upvars_size ; } if layout . fields . offset (tag_field . as_usize ()) >= variant_size { variant_size += match tag_encoding { TagEncoding :: Direct => tag . size (cx) , _ => Size :: ZERO , } ; } VariantInfo { name : Some (Symbol :: intern (& ty :: CoroutineArgs :: variant_name (variant_idx))) , kind : SizeKind :: Exact , size : variant_size . bytes () , align : variant_layout . align . abi . bytes () , fields , } }) . collect () ; let end_states = variant_infos . drain (1 ..= 2) ; let end_states : Vec < _ > = end_states . collect () ; variant_infos . extend (end_states) ; (variant_infos , match tag_encoding { TagEncoding :: Direct => Some (tag . size (cx)) , _ => None , } ,) }
}