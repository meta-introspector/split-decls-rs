...
mkitem!{mkimpl!{impl < T : Copy > Copy for Option < T > { }}}
...
mkitem!{mkimpl!{unsafe impl Sync for f32 { }}}
mkitem!{mkimpl!{unsafe impl < 'a , T : PointeeSized > Sync for & 'a T { }}}
mkitem!{mkimpl!{unsafe impl < T : Sync , const N : usize > Sync for [T ; N] { }}}
...
mkitem!{mkimpl!{impl PartialEq for u128 { fn eq (& self , other : & u128) -> bool { (* self) == (* other) } fn ne (& self , other : & u128) -> bool { (* self) != (* other) } }}}
...
mkitem!{mkimpl!{impl < T : PartialEq > PartialEq for Option < T > { fn eq (& self , other : & Self) -> bool { match (self , other) { (Some (lhs) , Some (rhs)) => * lhs == * rhs , (None , None) => true , _ => false , } } fn ne (& self , other : & Self) -> bool { match (self , other) { (Some (lhs) , Some (rhs)) => * lhs != * rhs , (None , None) => false , _ => true , } } }}}
mkitem!{mktrait!{# [lang = "shl"] pub trait Shl < RHS = Self > { type Output ; # [must_use] fn shl (self , rhs : RHS) -> Self :: Output ; }}}
mkitem!{mkimpl!{impl Shl for u128 { type Output = u128 ; fn shl (self , rhs : u128) -> u128 { self << rhs } }}}
...
mkfn!{
    ...
    # [lang = "panic_cannot_unwind"] # [track_caller] fn panic_cannot_unwind () -> ! { unsafe { libc :: puts ("panic in a function that cannot unwind\n\0" as * const str as * const i8) ; intrinsics :: abort () ; } }
}
...
mkfn!{
    ...
    # [lang = "eh_personality"] fn eh_personality (_version : i32 , _actions : i32 , _exception_class : u64 , _exception_object : * mut () , _context : * mut () ,) -> i32 { loop { } }
}
...
mkfn!{
    ...
    # [lang = "panic_in_cleanup"] fn panic_in_cleanup () -> ! { loop { } }
}
mkitem!{# [cfg (all (unix , not (target_vendor = "apple")))] # [link (name = "gcc_s")] extern "C" { fn _Unwind_Resume (exc : * mut ()) -> ! ; }}
...
mkitem!{mkstruct!{# [lang = "owned_box"] pub struct Box < T : ? Sized , A = Global > (Unique < T > , A) ;}}
mkitem!{mkimpl!{impl < T : ? Sized + Unsize < U > , U : ? Sized > CoerceUnsized < Box < U > > for Box < T > { }}}
...
mkitem!{mkimpl!{impl < T : ? Sized , A > Drop for Box < T , A > { fn drop (& mut self) { unsafe { libc :: free (self . 0 . pointer . 0 as * mut u8) ; } } }}}
mkitem!{mkimpl!{impl < T : ? Sized > Deref for Box < T > { type Target = T ; fn deref (& self) -> & Self :: Target { & * * self } }}}
...
mkitem!{# [rustc_intrinsic] pub const fn needs_drop < T : ?:: Sized > () -> bool ;}
...
mkmod!{libc, { 
                ...
                get_crates!(libc);
                mkinclude!(libc);
                mkitem!{# [cfg_attr (unix , link (name = "c"))] # [cfg_attr (target_env = "msvc" , link (name = "legacy_stdio_definitions"))] extern "C" { pub fn printf (format : * const i8 , ...) -> i32 ; }}
mkitem!{# [cfg_attr (unix , link (name = "c"))] # [cfg_attr (target_env = "msvc" , link (name = "msvcrt"))] extern "C" { pub fn puts (s : * const i8) -> i32 ; pub fn malloc (size : usize) -> * mut u8 ; pub fn free (ptr : * mut u8) ; pub fn memcpy (dst : * mut u8 , src : * const u8 , size : usize) ; pub fn memmove (dst : * mut u8 , src : * const u8 , size : usize) ; pub fn strncpy (dst : * mut u8 , src : * const u8 , size : usize) ; }} 
            }}
...
mkitem!{# [rustc_builtin_macro] # [rustc_macro_transparency = "semitransparent"] pub macro asm () { }}
mkitem!{# [rustc_builtin_macro] # [rustc_macro_transparency = "semitransparent"] pub macro global_asm () { }}
mkitem!{# [rustc_builtin_macro] # [rustc_macro_transparency = "semitransparent"] pub macro naked_asm () { }}
...
mkfn!{
    get_tls_introspect!();
    # [no_mangle] # [cfg (not (all (windows , target_env = "gnu")))] pub fn get_tls () -> u8 { # [thread_local] static A : u8 = 42 ; A }
}