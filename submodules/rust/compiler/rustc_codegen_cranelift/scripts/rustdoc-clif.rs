...
fn main() {
    ...
    if let Some(name) = option_option_env!("BUILTIN_BACKEND") {
    ...
    let rustdoc = if let Some(rustdoc) = option_option_env!("RUSTDOC") {
        rustdoc
    } else {
        // Ensure that the right toolchain is used
        env::set_var("RUSTUP_TOOLCHAIN", option_option_env!("TOOLCHAIN_NAME").expect("TOOLCHAIN_NAME"));
        "rustdoc"
    };
