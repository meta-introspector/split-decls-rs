...
mkuse!{use rustc_middle :: mir :: StatementKind :: * ;}
mkuse!{use crate :: prelude :: * ;}
mkitem!{mkenum!{# [derive (Copy , Clone , Debug , PartialEq , Eq , Hash)] pub (crate) enum SsaKind { NotSsa , MaybeSsa , }}}
mkitem!{mkimpl!{impl SsaKind { pub (crate) fn is_ssa < 'tcx > (self , fx : & FunctionCx < '_ , '_ , 'tcx > , ty : Ty < 'tcx >) -> bool { self == SsaKind :: MaybeSsa && (fx . clif_type (ty) . is_some () || fx . clif_pair_type (ty) . is_some ()) } }}}

macro_rules! analyze_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function analyze in module {}", module_path!());
    };
}
