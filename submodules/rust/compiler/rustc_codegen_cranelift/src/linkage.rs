...
mkuse!{use rustc_middle :: mir :: mono :: { MonoItem , Visibility } ;}
mkuse!{use crate :: prelude :: * ;}

macro_rules! get_clif_linkage_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function get_clif_linkage in module {}", module_path!());
    };
}

mkfn!{
    get_clif_linkage_introspect!();
...
macro_rules! get_static_linkage_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function get_static_linkage in module {}", module_path!());
    };
}

mkfn!{
    get_static_linkage_introspect!();
