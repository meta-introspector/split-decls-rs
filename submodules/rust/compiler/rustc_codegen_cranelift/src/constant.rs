...
mkuse!{use cranelift_module :: * ;}
mkuse!{use rustc_data_structures :: fx :: FxHashSet ;}
mkuse!{use rustc_middle :: middle :: codegen_fn_attrs :: CodegenFnAttrFlags ;}
mkuse!{use rustc_middle :: mir :: interpret :: { AllocId , GlobalAlloc , Scalar , read_target_uint } ;}
mkuse!{use rustc_middle :: ty :: { ExistentialTraitRef , ScalarInt } ;}
mkuse!{use crate :: prelude :: * ;}
mkitem!{mkstruct!{pub (crate) struct ConstantCx { todo : Vec < TodoItem > , anon_allocs : FxHashMap < AllocId , DataId > , }}}
mkitem!{mkenum!{# [derive (Copy , Clone , Debug , Eq , PartialEq , Hash)] enum TodoItem { Alloc (AllocId) , Static (DefId) , }}}
mkitem!{mkimpl!{impl ConstantCx { pub (crate) fn new () -> Self { ConstantCx { todo : vec ! [] , anon_allocs : FxHashMap :: default () } } pub (crate) fn finalize (mut self , tcx : TyCtxt < '_ > , module : & mut dyn Module) { define_all_allocs (tcx , module , & mut self) ; } }}}

macro_rules! codegen_static_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_static in module {}", module_path!());
    };
}

mkfn!{
    codegen_static_introspect!();
...
macro_rules! codegen_tls_ref_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_tls_ref in module {}", module_path!());
    };
}

mkfn!{
    codegen_tls_ref_introspect!();
...
macro_rules! eval_mir_constant_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function eval_mir_constant in module {}", module_path!());
    };
}

mkfn!{
    eval_mir_constant_introspect!();
...
macro_rules! codegen_constant_operand_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_constant_operand in module {}", module_path!());
    };
}

mkfn!{
    codegen_constant_operand_introspect!();
...
macro_rules! data_id_for_alloc_id_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function data_id_for_alloc_id in module {}", module_path!());
    };
}

mkfn!{
    data_id_for_alloc_id_introspect!();
...
macro_rules! data_id_for_vtable_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function data_id_for_vtable in module {}", module_path!());
    };
}

mkfn!{
    data_id_for_vtable_introspect!();
...
macro_rules! data_id_for_static_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function data_id_for_static in module {}", module_path!());
    };
}

mkfn!{
    data_id_for_static_introspect!();
...
macro_rules! define_all_allocs_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function define_all_allocs in module {}", module_path!());
    };
}

mkfn!{
    define_all_allocs_introspect!();
...
macro_rules! mir_operand_get_const_val_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function mir_operand_get_const_val in module {}", module_path!());
    };
}

mkfn!{
    mir_operand_get_const_val_introspect!();
