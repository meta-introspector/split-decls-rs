mkuse!{use cranelift_codegen :: ir :: ArgumentPurpose ;}
mkuse!{use rustc_abi :: { Reg , RegKind } ;}
mkuse!{use rustc_target :: callconv :: { ArgAbi , ArgAttributes , ArgExtension as RustcArgExtension , CastTarget , PassMode , } ;}
mkuse!{use smallvec :: { SmallVec , smallvec } ;}
mkuse!{use crate :: prelude :: * ;}
mkuse!{use crate :: value_and_place :: assert_assignable ;}
mkitem!{mktrait!{pub (super) trait ArgAbiExt < 'tcx > { fn get_abi_param (& self , tcx : TyCtxt < 'tcx >) -> SmallVec < [AbiParam ; 2] > ; fn get_abi_return (& self , tcx : TyCtxt < 'tcx >) -> (Option < AbiParam > , Vec < AbiParam >) ; }}}

macro_rules! reg_to_abi_param_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function reg_to_abi_param in module {}", module_path!());
    };
}

mkfn!{
    reg_to_abi_param_introspect!();
...
macro_rules! apply_attrs_to_abi_param_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function apply_attrs_to_abi_param in module {}", module_path!());
    };
}

mkfn!{
    apply_attrs_to_abi_param_introspect!();
...
macro_rules! cast_target_to_abi_params_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function cast_target_to_abi_params in module {}", module_path!());
    };
}

mkfn!{
    cast_target_to_abi_params_introspect!();
...
mkitem!{mkimpl!{impl < 'tcx > ArgAbiExt < 'tcx > for ArgAbi < 'tcx , Ty < 'tcx > > { fn get_abi_param (& self , tcx : TyCtxt < 'tcx >) -> SmallVec < [AbiParam ; 2] > { match self . mode { PassMode :: Ignore => smallvec ! [] , PassMode :: Direct (attrs) => match self . layout . backend_repr { BackendRepr :: Scalar (scalar) => smallvec ! [apply_attrs_to_abi_param (AbiParam :: new (scalar_to_clif_type (tcx , scalar)) , attrs)] , BackendRepr :: SimdVector { .. } => { let vector_ty = crate :: intrinsics :: clif_vector_type (tcx , self . layout) ; smallvec ! [AbiParam :: new (vector_ty)] } _ => unreachable ! ("{:?}" , self . layout . backend_repr) , } , PassMode :: Pair (attrs_a , attrs_b) => match self . layout . backend_repr { BackendRepr :: ScalarPair (a , b) => { let a = scalar_to_clif_type (tcx , a) ; let b = scalar_to_clif_type (tcx , b) ; smallvec ! [apply_attrs_to_abi_param (AbiParam :: new (a) , attrs_a) , apply_attrs_to_abi_param (AbiParam :: new (b) , attrs_b) ,] } _ => unreachable ! ("{:?}" , self . layout . backend_repr) , } , PassMode :: Cast { ref cast , pad_i32 } => { assert ! (! pad_i32 , "padding support not yet implemented") ; cast_target_to_abi_params (cast) . into_iter () . map (| (_ , param) | param) . collect () } PassMode :: Indirect { attrs , meta_attrs : None , on_stack } => { if on_stack { let size = self . layout . size . align_to (tcx . data_layout . pointer_align () . abi) ; let size = u32 :: try_from (size . bytes ()) . unwrap () ; smallvec ! [apply_attrs_to_abi_param (AbiParam :: special (pointer_ty (tcx) , ArgumentPurpose :: StructArgument (size) ,) , attrs)] } else { smallvec ! [apply_attrs_to_abi_param (AbiParam :: new (pointer_ty (tcx)) , attrs)] } } PassMode :: Indirect { attrs , meta_attrs : Some (meta_attrs) , on_stack } => { assert ! (! on_stack) ; smallvec ! [apply_attrs_to_abi_param (AbiParam :: new (pointer_ty (tcx)) , attrs) , apply_attrs_to_abi_param (AbiParam :: new (pointer_ty (tcx)) , meta_attrs) ,] } } } fn get_abi_return (& self , tcx : TyCtxt < 'tcx >) -> (Option < AbiParam > , Vec < AbiParam >) { match self . mode { PassMode :: Ignore => (None , vec ! []) , PassMode :: Direct (attrs) => match self . layout . backend_repr { BackendRepr :: Scalar (scalar) => (None , vec ! [apply_attrs_to_abi_param (AbiParam :: new (scalar_to_clif_type (tcx , scalar)) , attrs ,)] ,) , BackendRepr :: SimdVector { .. } => { let vector_ty = crate :: intrinsics :: clif_vector_type (tcx , self . layout) ; (None , vec ! [apply_attrs_to_abi_param (AbiParam :: new (vector_ty) , attrs)]) } _ => unreachable ! ("{:?}" , self . layout . backend_repr) , } , PassMode :: Pair (attrs_a , attrs_b) => match self . layout . backend_repr { BackendRepr :: ScalarPair (a , b) => { let a = scalar_to_clif_type (tcx , a) ; let b = scalar_to_clif_type (tcx , b) ; (None , vec ! [apply_attrs_to_abi_param (AbiParam :: new (a) , attrs_a) , apply_attrs_to_abi_param (AbiParam :: new (b) , attrs_b) ,] ,) } _ => unreachable ! ("{:?}" , self . layout . backend_repr) , } , PassMode :: Cast { ref cast , .. } => (None , cast_target_to_abi_params (cast) . into_iter () . map (| (_ , param) | param) . collect () ,) , PassMode :: Indirect { attrs , meta_attrs : None , on_stack } => { assert ! (! on_stack) ; (Some (apply_attrs_to_abi_param (AbiParam :: special (pointer_ty (tcx) , ArgumentPurpose :: StructReturn) , attrs ,)) , vec ! [] ,) } PassMode :: Indirect { attrs : _ , meta_attrs : Some (_) , on_stack : _ } => { unreachable ! ("unsized return value") } } } }}}

macro_rules! to_casted_value_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function to_casted_value in module {}", module_path!());
    };
}

mkfn!{
    to_casted_value_introspect!();
...
macro_rules! from_casted_value_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function from_casted_value in module {}", module_path!());
    };
}

mkfn!{
    from_casted_value_introspect!();
...
macro_rules! adjust_arg_for_abi_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function adjust_arg_for_abi in module {}", module_path!());
    };
}

mkfn!{
    adjust_arg_for_abi_introspect!();
