mkuse!{use crate :: codegen_f16_f128 ;}
mkuse!{use crate :: prelude :: * ;}

macro_rules! bin_op_to_intcc_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function bin_op_to_intcc in module {}", module_path!());
    };
}

mkfn!{
    bin_op_to_intcc_introspect!();
...
macro_rules! codegen_three_way_compare_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_three_way_compare in module {}", module_path!());
    };
}

mkfn!{
    codegen_three_way_compare_introspect!();
...
macro_rules! codegen_compare_bin_op_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_compare_bin_op in module {}", module_path!());
    };
}

mkfn!{
    codegen_compare_bin_op_introspect!();
...
macro_rules! codegen_binop_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_binop in module {}", module_path!());
    };
}

mkfn!{
    codegen_binop_introspect!();
...
macro_rules! codegen_bool_binop_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_bool_binop in module {}", module_path!());
    };
}

mkfn!{
    codegen_bool_binop_introspect!();
...
macro_rules! codegen_int_binop_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_int_binop in module {}", module_path!());
    };
}

mkfn!{
    codegen_int_binop_introspect!();
    pub (crate) fn codegen_int_binop < 'tcx > (fx : & mut FunctionCx < '_ , '_ , 'tcx > , bin_op : BinOp , in_lhs : CValue < 'tcx > , in_rhs : CValue < 'tcx > ,) -> CValue < 'tcx > { if ! matches ! (bin_op , BinOp :: Shl | BinOp :: ShlUnchecked | BinOp :: Shr | BinOp :: ShrUnchecked) { assert_eq ! (in_lhs . layout () . ty , in_rhs . layout () . ty , "int binop requires lhs and rhs of same type") ; } if let Some (res) = crate :: codegen_i128 :: maybe_codegen (fx , bin_op , in_lhs , in_rhs) { return res ; } let signed = type_sign (in_lhs . layout () . ty) ; let lhs = in_lhs . load_scalar (fx) ; let rhs = in_rhs . load_scalar (fx) ; let b = fx . bcx . ins () ; let val = match bin_op { BinOp :: Add | BinOp :: AddUnchecked => b . iadd (lhs , rhs) , BinOp :: Sub | BinOp :: SubUnchecked => b . isub (lhs , rhs) , BinOp :: Mul | BinOp :: MulUnchecked => b . imul (lhs , rhs) , BinOp :: Div => { if signed { b . sdiv (lhs , rhs) } else { b . udiv (lhs , rhs) } } BinOp :: Rem => { if signed { b . srem (lhs , rhs) } else { b . urem (lhs , rhs) } } BinOp :: BitXor => b . bxor (lhs , rhs) , BinOp :: BitAnd => b . band (lhs , rhs) , BinOp :: BitOr => b . bor (lhs , rhs) , BinOp :: Shl | BinOp :: ShlUnchecked => b . ishl (lhs , rhs) , BinOp :: Shr | BinOp :: ShrUnchecked => { if signed { b . sshr (lhs , rhs) } else { b . ushr (lhs , rhs) } } BinOp :: Offset => unreachable ! ("Offset is not an integer operation") , BinOp :: AddWithOverflow | BinOp :: SubWithOverflow | BinOp :: MulWithOverflow => { unreachable ! ("Overflow binops handled by `codegen_checked_int_binop`") } BinOp :: Eq | BinOp :: Ne | BinOp :: Lt | BinOp :: Le | BinOp :: Gt | BinOp :: Ge | BinOp :: Cmp => { unreachable ! ("{:?}({:?}, {:?})" , bin_op , in_lhs . layout () . ty , in_rhs . layout () . ty) ; } } ; CValue :: by_val (val , in_lhs . layout ()) }
}

macro_rules! codegen_checked_int_binop_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_checked_int_binop in module {}", module_path!());
    };
}

mkfn!{
    codegen_checked_int_binop_introspect!();
    pub (crate) fn codegen_checked_int_binop < 'tcx > (fx : & mut FunctionCx < '_ , '_ , 'tcx > , bin_op : BinOp , in_lhs : CValue < 'tcx > , in_rhs : CValue < 'tcx > ,) -> CValue < 'tcx > { let lhs = in_lhs . load_scalar (fx) ; let rhs = in_rhs . load_scalar (fx) ; let signed = type_sign (in_lhs . layout () . ty) ; let (res , has_overflow) = match bin_op { BinOp :: Add => { let val = fx . bcx . ins () . iadd (lhs , rhs) ; let has_overflow = if ! signed { fx . bcx . ins () . icmp (IntCC :: UnsignedLessThan , val , lhs) } else { let rhs_is_negative = fx . bcx . ins () . icmp_imm (IntCC :: SignedLessThan , rhs , 0) ; let slt = fx . bcx . ins () . icmp (IntCC :: SignedLessThan , val , lhs) ; fx . bcx . ins () . bxor (rhs_is_negative , slt) } ; (val , has_overflow) } BinOp :: Sub => { let val = fx . bcx . ins () . isub (lhs , rhs) ; let has_overflow = if ! signed { fx . bcx . ins () . icmp (IntCC :: UnsignedGreaterThan , val , lhs) } else { let rhs_is_negative = fx . bcx . ins () . icmp_imm (IntCC :: SignedLessThan , rhs , 0) ; let sgt = fx . bcx . ins () . icmp (IntCC :: SignedGreaterThan , val , lhs) ; fx . bcx . ins () . bxor (rhs_is_negative , sgt) } ; (val , has_overflow) } BinOp :: Mul => { if let Some (res) = crate :: codegen_i128 :: maybe_codegen_mul_checked (fx , in_lhs , in_rhs) { return res ; } let ty = fx . bcx . func . dfg . value_type (lhs) ; match ty { types :: I8 | types :: I16 | types :: I32 if ! signed => { let lhs = fx . bcx . ins () . uextend (ty . double_width () . unwrap () , lhs) ; let rhs = fx . bcx . ins () . uextend (ty . double_width () . unwrap () , rhs) ; let val = fx . bcx . ins () . imul (lhs , rhs) ; let has_overflow = fx . bcx . ins () . icmp_imm (IntCC :: UnsignedGreaterThan , val , (1 << ty . bits ()) - 1 ,) ; let val = fx . bcx . ins () . ireduce (ty , val) ; (val , has_overflow) } types :: I8 | types :: I16 | types :: I32 if signed => { let lhs = fx . bcx . ins () . sextend (ty . double_width () . unwrap () , lhs) ; let rhs = fx . bcx . ins () . sextend (ty . double_width () . unwrap () , rhs) ; let val = fx . bcx . ins () . imul (lhs , rhs) ; let has_underflow = fx . bcx . ins () . icmp_imm (IntCC :: SignedLessThan , val , - (1 << (ty . bits () - 1))) ; let has_overflow = fx . bcx . ins () . icmp_imm (IntCC :: SignedGreaterThan , val , (1 << (ty . bits () - 1)) - 1 ,) ; let val = fx . bcx . ins () . ireduce (ty , val) ; (val , fx . bcx . ins () . bor (has_underflow , has_overflow)) } types :: I64 => { let val = fx . bcx . ins () . imul (lhs , rhs) ; let has_overflow = if ! signed { let val_hi = fx . bcx . ins () . umulhi (lhs , rhs) ; fx . bcx . ins () . icmp_imm (IntCC :: NotEqual , val_hi , 0) } else { let val_hi = fx . bcx . ins () . smulhi (lhs , rhs) ; let val_sign = fx . bcx . ins () . sshr_imm (val , i64 :: from (ty . bits () - 1)) ; let xor = fx . bcx . ins () . bxor (val_hi , val_sign) ; fx . bcx . ins () . icmp_imm (IntCC :: NotEqual , xor , 0) } ; (val , has_overflow) } types :: I128 => { unreachable ! ("i128 should have been handled by codegen_i128::maybe_codegen") } _ => unreachable ! ("invalid non-integer type {}" , ty) , } } _ => bug ! ("binop {:?} on checked int/uint lhs: {:?} rhs: {:?}" , bin_op , in_lhs , in_rhs) , } ; let out_layout = fx . layout_of (Ty :: new_tup (fx . tcx , & [in_lhs . layout () . ty , fx . tcx . types . bool])) ; CValue :: by_val_pair (res , has_overflow , out_layout) }
}

macro_rules! codegen_saturating_int_binop_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_saturating_int_binop in module {}", module_path!());
    };
}

mkfn!{
    codegen_saturating_int_binop_introspect!();
...
macro_rules! codegen_float_binop_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_float_binop in module {}", module_path!());
    };
}

mkfn!{
    codegen_float_binop_introspect!();
...
macro_rules! codegen_float_max_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_float_max in module {}", module_path!());
    };
}

mkfn!{
    codegen_float_max_introspect!();
