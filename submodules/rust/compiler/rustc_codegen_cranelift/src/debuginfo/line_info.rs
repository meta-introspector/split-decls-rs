...
mkuse!{use std :: path :: { Component , Path } ;}
mkuse!{use cranelift_codegen :: MachSrcLoc ;}
mkuse!{use cranelift_codegen :: binemit :: CodeOffset ;}
mkuse!{use gimli :: write :: { AttributeValue , FileId , FileInfo , LineProgram , LineString , LineStringTable } ;}
mkuse!{use rustc_span :: { FileName , Pos , SourceFile , SourceFileAndLine , SourceFileHash , SourceFileHashAlgorithm , hygiene , } ;}
mkuse!{use crate :: debuginfo :: FunctionDebugContext ;}
mkuse!{use crate :: debuginfo :: emit :: address_for_func ;}
mkuse!{use crate :: prelude :: * ;}

macro_rules! split_path_dir_and_file_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function split_path_dir_and_file in module {}", module_path!());
    };
}

mkfn!{
    split_path_dir_and_file_introspect!();
...
macro_rules! osstr_as_utf8_bytes_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function osstr_as_utf8_bytes in module {}", module_path!());
    };
}

mkfn!{
    osstr_as_utf8_bytes_introspect!();
...
mkitem!{const MD5_LEN : usize = 16 ;}

macro_rules! make_file_info_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function make_file_info in module {}", module_path!());
    };
}

mkfn!{
    make_file_info_introspect!();
