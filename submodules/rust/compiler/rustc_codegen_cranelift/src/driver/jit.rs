...
mkuse!{use std :: os :: raw :: { c_char , c_int } ;}
mkuse!{use cranelift_jit :: { JITBuilder , JITModule } ;}
mkuse!{use rustc_codegen_ssa :: CrateInfo ;}
...
macro_rules! create_jit_module_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function create_jit_module in module {}", module_path!());
    };
}

mkfn!{
    create_jit_module_introspect!();
...
macro_rules! dep_symbol_lookup_fn_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function dep_symbol_lookup_fn in module {}", module_path!());
    };
}

mkfn!{
    dep_symbol_lookup_fn_introspect!();
