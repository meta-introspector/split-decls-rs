...
mkuse!{use std :: io :: BufWriter ;}
mkuse!{use std :: path :: { Path , PathBuf } ;}
mkuse!{use std :: sync :: Arc ;}
mkuse!{use std :: thread :: JoinHandle ;}
mkuse!{use cranelift_object :: { ObjectBuilder , ObjectModule } ;}
mkuse!{use rustc_codegen_ssa :: assert_module_sources :: CguReuse ;}
mkuse!{use rustc_codegen_ssa :: back :: link :: ensure_removed ;}
mkuse!{use rustc_codegen_ssa :: base :: determine_cgu_reuse ;}
mkuse!{use rustc_codegen_ssa :: { CodegenResults , CompiledModule , CrateInfo , ModuleKind , errors as ssa_errors , } ;}
...
mkuse!{use rustc_hir :: attrs :: Linkage as RLinkage ;}
...
mkuse!{use rustc_middle :: mir :: mono :: { CodegenUnit , MonoItem , MonoItemData , Visibility } ;}
mkuse!{use rustc_session :: Session ;}
mkuse!{use rustc_session :: config :: { DebugInfo , OutFileName , OutputFilenames , OutputType } ;}
mkuse!{use crate :: CodegenCx ;}
mkuse!{use crate :: base :: CodegenedFunction ;}
mkuse!{use crate :: concurrency_limiter :: { ConcurrencyLimiter , ConcurrencyLimiterToken } ;}
mkuse!{use crate :: debuginfo :: TypeDebugContext ;}
mkuse!{use crate :: global_asm :: { GlobalAsmConfig , GlobalAsmContext } ;}
mkuse!{use crate :: prelude :: * ;}
mkuse!{use crate :: unwind_module :: UnwindModule ;}

macro_rules! disable_incr_cache_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function disable_incr_cache in module {}", module_path!());
    };
}

mkfn!{
    disable_incr_cache_introspect!();
    fn disable_incr_cache () -> bool { env :: var ("CG_CLIF_DISABLE_INCR_CACHE") . as_deref () == Ok ("1") }
}
mkitem!{mkstruct!{struct ModuleCodegenResult { module_regular : CompiledModule , module_global_asm : Option < CompiledModule > , existing_work_product : Option < (WorkProductId , WorkProduct) > , }}}
mkitem!{mkenum!{enum OngoingModuleCodegen { Sync (Result < ModuleCodegenResult , String >) , Async (JoinHandle < Result < ModuleCodegenResult , String > >) , }}}
mkitem!{mkimpl!{impl < HCX > HashStable < HCX > for OngoingModuleCodegen { fn hash_stable (& self , _ : & mut HCX , _ : & mut StableHasher) { } }}}
mkitem!{mkstruct!{pub (crate) struct OngoingCodegen { modules : Vec < OngoingModuleCodegen > , allocator_module : Option < CompiledModule > , crate_info : CrateInfo , concurrency_limiter : ConcurrencyLimiter , }}}
mkitem!{mkimpl!{impl OngoingCodegen { pub (crate) fn join (self , sess : & Session , outputs : & OutputFilenames ,) -> (CodegenResults , FxIndexMap < WorkProductId , WorkProduct >) { let mut work_products = FxIndexMap :: default () ; let mut modules = vec ! [] ; let disable_incr_cache = disable_incr_cache () ; for module_codegen in self . modules { let module_codegen_result = match module_codegen { OngoingModuleCodegen :: Sync (module_codegen_result) => module_codegen_result , OngoingModuleCodegen :: Async (join_handle) => match join_handle . join () { Ok (module_codegen_result) => module_codegen_result , Err (panic) => std :: panic :: resume_unwind (panic) , } , } ; let module_codegen_result = match module_codegen_result { Ok (module_codegen_result) => module_codegen_result , Err (err) => sess . dcx () . fatal (err) , } ; let ModuleCodegenResult { module_regular , module_global_asm , existing_work_product } = module_codegen_result ; if let Some ((work_product_id , work_product)) = existing_work_product { work_products . insert (work_product_id , work_product) ; } else { let work_product = if disable_incr_cache { None } else if let Some (module_global_asm) = & module_global_asm { rustc_incremental :: copy_cgu_workproduct_to_incr_comp_cache_dir (sess , & module_regular . name , & [("o" , & module_regular . object . as_ref () . unwrap ()) , ("asm.o" , & module_global_asm . object . as_ref () . unwrap ()) ,] , & [] ,) } else { rustc_incremental :: copy_cgu_workproduct_to_incr_comp_cache_dir (sess , & module_regular . name , & [("o" , & module_regular . object . as_ref () . unwrap ())] , & [] ,) } ; if let Some ((work_product_id , work_product)) = work_product { work_products . insert (work_product_id , work_product) ; } } modules . push (module_regular) ; if let Some (module_global_asm) = module_global_asm { modules . push (module_global_asm) ; } } self . concurrency_limiter . finished () ; sess . dcx () . abort_if_errors () ; let codegen_results = CodegenResults { modules , allocator_module : self . allocator_module , crate_info : self . crate_info , } ; produce_final_output_artifacts (sess , & codegen_results , outputs) ; (codegen_results , work_products) } }}}
...
macro_rules! emit_cgu_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function emit_cgu in module {}", module_path!());
    };
}
...
macro_rules! emit_module_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function emit_module in module {}", module_path!());
    };
}

mkfn!{
    emit_module_introspect!();
...
macro_rules! reuse_workproduct_for_cgu_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function reuse_workproduct_for_cgu in module {}", module_path!());
    };
}

mkfn!{
    reuse_workproduct_for_cgu_introspect!();
    fn reuse_workproduct_for_cgu (tcx : TyCtxt < '_ > , cgu : & CodegenUnit < '_ > ,) -> Result < ModuleCodegenResult , String > { let work_product = cgu . previous_work_product (tcx) ; let obj_out_regular = tcx . output_filenames (()) . temp_path_for_cgu (OutputType :: Object , cgu . name () . as_str () , tcx . sess . invocation_temp . as_deref () ,) ; let source_file_regular = rustc_incremental :: in_incr_comp_dir_sess (& tcx . sess , & work_product . saved_files . get ("o") . expect ("no saved object file in work product") ,) ; if let Err (err) = rustc_fs_util :: link_or_copy (& source_file_regular , & obj_out_regular) { return Err (format ! ("unable to copy {} to {}: {}" , source_file_regular . display () , obj_out_regular . display () , err)) ; } let obj_out_global_asm = crate :: global_asm :: add_file_stem_postfix (obj_out_regular . clone () , ".asm") ; let source_file_global_asm = if let Some (asm_o) = work_product . saved_files . get ("asm.o") { let source_file_global_asm = rustc_incremental :: in_incr_comp_dir_sess (& tcx . sess , asm_o) ; if let Err (err) = rustc_fs_util :: link_or_copy (& source_file_global_asm , & obj_out_global_asm) { return Err (format ! ("unable to copy {} to {}: {}" , source_file_global_asm . display () , obj_out_global_asm . display () , err)) ; } Some (source_file_global_asm) } else { None } ; Ok (ModuleCodegenResult { module_regular : CompiledModule { name : cgu . name () . to_string () , kind : ModuleKind :: Regular , object : Some (obj_out_regular) , dwarf_object : None , bytecode : None , assembly : None , llvm_ir : None , links_from_incr_cache : vec ! [source_file_regular] , } , module_global_asm : source_file_global_asm . map (| source_file | CompiledModule { name : cgu . name () . to_string () , kind : ModuleKind :: Regular , object : Some (obj_out_global_asm) , dwarf_object : None , bytecode : None , assembly : None , llvm_ir : None , links_from_incr_cache : vec ! [source_file] , }) , existing_work_product : Some ((cgu . work_product_id () , work_product)) , }) }
}

macro_rules! codegen_cgu_content_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_cgu_content in module {}", module_path!());
    };
}

mkfn!{
    codegen_cgu_content_introspect!();
    fn codegen_cgu_content (tcx : TyCtxt < '_ > , module : & mut dyn Module , cgu_name : rustc_span :: Symbol ,) -> (CodegenCx , Vec < CodegenedFunction >) { let _timer = tcx . prof . generic_activity_with_arg ("codegen cgu" , cgu_name . as_str ()) ; let cgu = tcx . codegen_unit (cgu_name) ; let mono_items = cgu . items_in_deterministic_order (tcx) ; let mut cx = crate :: CodegenCx :: new (tcx , module . isa () , tcx . sess . opts . debuginfo != DebugInfo :: None , cgu_name ,) ; let mut type_dbg = TypeDebugContext :: default () ; super :: predefine_mono_items (tcx , module , & mono_items) ; let mut codegened_functions = vec ! [] ; for (mono_item , item_data) in mono_items { match mono_item { MonoItem :: Fn (instance) => { let flags = tcx . codegen_instance_attrs (instance . def) . flags ; if flags . contains (CodegenFnAttrFlags :: NAKED) { rustc_codegen_ssa :: mir :: naked_asm :: codegen_naked_asm (& mut GlobalAsmContext { tcx , global_asm : & mut cx . global_asm } , instance , MonoItemData { linkage : RLinkage :: External , visibility : if item_data . linkage == RLinkage :: Internal { Visibility :: Hidden } else { item_data . visibility } , .. item_data } ,) ; continue ; } let codegened_function = crate :: base :: codegen_fn (tcx , & mut cx , & mut type_dbg , Function :: new () , module , instance ,) ; codegened_functions . push (codegened_function) ; } MonoItem :: Static (def_id) => { let data_id = crate :: constant :: codegen_static (tcx , module , def_id) ; if let Some (debug_context) = & mut cx . debug_context { debug_context . define_static (tcx , & mut type_dbg , def_id , data_id) ; } } MonoItem :: GlobalAsm (item_id) => { rustc_codegen_ssa :: base :: codegen_global_asm (& mut GlobalAsmContext { tcx , global_asm : & mut cx . global_asm } , item_id ,) ; } } } crate :: main_shim :: maybe_create_entry_wrapper (tcx , module , false , cgu . is_primary ()) ; (cx , codegened_functions) }
}

macro_rules! module_codegen_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function module_codegen in module {}", module_path!());
    };
}

mkfn!{
    module_codegen_introspect!();
...
macro_rules! emit_allocator_module_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function emit_allocator_module in module {}", module_path!());
    };
}

mkfn!{
    emit_allocator_module_introspect!();
...
macro_rules! run_aot_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function run_aot in module {}", module_path!());
    };
}

mkfn!{
    run_aot_introspect!();
    pub (crate) fn run_aot (tcx : TyCtxt < '_ >) -> Box < OngoingCodegen > { let target_cpu = match tcx . sess . opts . cg . target_cpu { Some (ref name) => name , None => tcx . sess . target . cpu . as_ref () , } . to_owned () ; let cgus = if tcx . sess . opts . output_types . should_codegen () { tcx . collect_and_partition_mono_items (()) . codegen_units } else { return Box :: new (OngoingCodegen { modules : vec ! [] , allocator_module : None , crate_info : CrateInfo :: new (tcx , target_cpu) , concurrency_limiter : ConcurrencyLimiter :: new (0) , }) ; } ; if tcx . dep_graph . is_fully_enabled () { for cgu in cgus { tcx . ensure_ok () . codegen_unit (cgu . name ()) ; } } let cgu_reuse = tcx . sess . time ("find_cgu_reuse" , | | { cgus . iter () . map (| cgu | determine_cgu_reuse (tcx , & cgu)) . collect :: < Vec < _ > > () }) ; rustc_codegen_ssa :: assert_module_sources :: assert_module_sources (tcx , & | cgu_reuse_tracker | { for (i , cgu) in cgus . iter () . enumerate () { let cgu_reuse = cgu_reuse [i] ; cgu_reuse_tracker . set_actual_reuse (cgu . name () . as_str () , cgu_reuse) ; } }) ; let global_asm_config = Arc :: new (crate :: global_asm :: GlobalAsmConfig :: new (tcx)) ; let disable_incr_cache = disable_incr_cache () ; let (todo_cgus , done_cgus) = cgus . into_iter () . enumerate () . partition :: < Vec < _ > , _ > (| & (i , _) | match cgu_reuse [i] { _ if disable_incr_cache => true , CguReuse :: No => true , CguReuse :: PreLto | CguReuse :: PostLto => false , }) ; let concurrency_limiter = IntoDynSyncSend (ConcurrencyLimiter :: new (todo_cgus . len ())) ; let modules : Vec < _ > = tcx . sess . time ("codegen mono items" , | | { let modules : Vec < _ > = par_map (todo_cgus , | (_ , cgu) | { let dep_node = cgu . codegen_dep_node (tcx) ; let (module , _) = tcx . dep_graph . with_task (dep_node , tcx , (global_asm_config . clone () , cgu . name () , concurrency_limiter . acquire (tcx . dcx ())) , module_codegen , Some (rustc_middle :: dep_graph :: hash_result) ,) ; IntoDynSyncSend (module) }) ; modules . into_iter () . map (| module | module . 0) . chain (done_cgus . into_iter () . map (| (_ , cgu) | { OngoingModuleCodegen :: Sync (reuse_workproduct_for_cgu (tcx , cgu)) })) . collect () }) ; let allocator_module = emit_allocator_module (tcx) ; Box :: new (OngoingCodegen { modules , allocator_module , crate_info : CrateInfo :: new (tcx , target_cpu) , concurrency_limiter : concurrency_limiter . 0 , }) }
}