...
mkuse!{use rustc_codegen_ssa :: traits :: { AsmCodegenMethods , GlobalAsmOperandRef } ;}
mkuse!{use rustc_middle :: ty :: TyCtxt ;}
mkuse!{use rustc_middle :: ty :: layout :: { FnAbiError , FnAbiOfHelpers , FnAbiRequest , HasTyCtxt , HasTypingEnv , LayoutError , LayoutOfHelpers , } ;}
mkuse!{use rustc_session :: config :: { OutputFilenames , OutputType } ;}
mkuse!{use rustc_target :: asm :: InlineAsmArch ;}
mkuse!{use crate :: prelude :: * ;}
mkitem!{mkstruct!{pub (crate) struct GlobalAsmContext < 'a , 'tcx > { pub tcx : TyCtxt < 'tcx > , pub global_asm : & 'a mut String , }}}
mkitem!{mkimpl!{impl < 'tcx > AsmCodegenMethods < 'tcx > for GlobalAsmContext < '_ , 'tcx > { fn codegen_global_asm (& mut self , template : & [InlineAsmTemplatePiece] , operands : & [GlobalAsmOperandRef < 'tcx >] , options : InlineAsmOptions , _line_spans : & [Span] ,) { codegen_global_asm_inner (self . tcx , self . global_asm , template , operands , options) ; } fn mangled_name (& self , instance : Instance < 'tcx >) -> String { let symbol_name = self . tcx . symbol_name (instance) . name . to_owned () ; if self . tcx . sess . target . is_like_darwin { format ! ("_{symbol_name}") } else { symbol_name } } }}}
mkitem!{mkimpl!{impl < 'tcx > LayoutOfHelpers < 'tcx > for GlobalAsmContext < '_ , 'tcx > { # [inline] fn handle_layout_err (& self , err : LayoutError < 'tcx > , span : Span , ty : Ty < 'tcx >) -> ! { if let LayoutError :: SizeOverflow (_) | LayoutError :: ReferencesError (_) = err { self . tcx . sess . dcx () . span_fatal (span , err . to_string ()) } else { self . tcx . sess . dcx () . span_fatal (span , format ! ("failed to get layout for `{}`: {}" , ty , err)) } } }}}
mkitem!{mkimpl!{impl < 'tcx > FnAbiOfHelpers < 'tcx > for GlobalAsmContext < '_ , 'tcx > { # [inline] fn handle_fn_abi_err (& self , err : FnAbiError < 'tcx > , span : Span , fn_abi_request : FnAbiRequest < 'tcx > ,) -> ! { FullyMonomorphizedLayoutCx (self . tcx) . handle_fn_abi_err (err , span , fn_abi_request) } }}}
mkitem!{mkimpl!{impl < 'tcx > HasTyCtxt < 'tcx > for GlobalAsmContext < '_ , 'tcx > { fn tcx < 'b > (& 'b self) -> TyCtxt < 'tcx > { self . tcx } }}}
mkitem!{mkimpl!{impl < 'tcx > rustc_abi :: HasDataLayout for GlobalAsmContext < '_ , 'tcx > { fn data_layout (& self) -> & rustc_abi :: TargetDataLayout { & self . tcx . data_layout } }}}
mkitem!{mkimpl!{impl < 'tcx > HasTypingEnv < 'tcx > for GlobalAsmContext < '_ , 'tcx > { fn typing_env (& self) -> ty :: TypingEnv < 'tcx > { ty :: TypingEnv :: fully_monomorphized () } }}}

macro_rules! codegen_global_asm_inner_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function codegen_global_asm_inner in module {}", module_path!());
    };
}

mkfn!{
    codegen_global_asm_inner_introspect!();
...
macro_rules! add_file_stem_postfix_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function add_file_stem_postfix in module {}", module_path!());
    };
}

mkfn!{
    add_file_stem_postfix_introspect!();
