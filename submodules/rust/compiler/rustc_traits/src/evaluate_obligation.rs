...
mkuse!{use rustc_trait_selection :: traits :: query :: CanonicalPredicateGoal ;}
mkuse!{use rustc_trait_selection :: traits :: { EvaluationResult , Obligation , ObligationCause , OverflowError , SelectionContext , TraitQueryMode , sizedness_fast_path , } ;}
...
mkfn!{
    provide_introspect!();
    pub (crate) fn provide (p : & mut Providers) { * p = Providers { evaluate_obligation , .. * p } ; }
}

macro_rules! evaluate_obligation_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function evaluate_obligation in module {}", module_path!());
    };
}

mkfn!{
    evaluate_obligation_introspect!();
