...
mkuse!{use rustc_hir :: definitions :: { DefPathData , DefPathDataName , DisambiguatedDefPathData } ;}
mkuse!{use rustc_hir :: { CoroutineDesugaring , CoroutineKind , CoroutineSource , Mutability } ;}
mkuse!{use rustc_middle :: bug ;}
mkuse!{use rustc_middle :: ty :: layout :: { IntegerExt , TyAndLayout } ;}
mkuse!{use rustc_middle :: ty :: { self , ExistentialProjection , GenericArgKind , GenericArgsRef , Ty , TyCtxt } ;}
mkuse!{use smallvec :: SmallVec ;}
mkuse!{use crate :: debuginfo :: wants_c_like_enum_debuginfo ;}

macro_rules! compute_debuginfo_type_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function compute_debuginfo_type_name in module {}", module_path!());
    };
}

mkfn!{
    compute_debuginfo_type_name_introspect!();
...
macro_rules! push_debuginfo_type_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_debuginfo_type_name in module {}", module_path!());
    };
}

mkfn!{
    push_debuginfo_type_name_introspect!();
...
mkitem!{mkenum!{pub enum VTableNameKind { GlobalVariable , Type , }}}

macro_rules! compute_debuginfo_vtable_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function compute_debuginfo_vtable_name in module {}", module_path!());
    };
}

mkfn!{
    compute_debuginfo_vtable_name_introspect!();
    # [doc = " Computes a name for the global variable storing a vtable (or the type of that global variable)."] # [doc = ""] # [doc = " The name is of the form:"] # [doc = ""] # [doc = " `<path::to::SomeType as path::to::SomeTrait>::{vtable}`"] # [doc = ""] # [doc = " or, when generating C++-like names:"] # [doc = ""] # [doc = " `impl$<path::to::SomeType, path::to::SomeTrait>::vtable$`"] # [doc = ""] # [doc = " If `kind` is `VTableNameKind::Type` then the last component is `{vtable_ty}` instead of just"] # [doc = " `{vtable}`, so that the type and the corresponding global variable get assigned different"] # [doc = " names."] pub fn compute_debuginfo_vtable_name < 'tcx > (tcx : TyCtxt < 'tcx > , t : Ty < 'tcx > , trait_ref : Option < ty :: ExistentialTraitRef < 'tcx > > , kind : VTableNameKind ,) -> String { let cpp_like_debuginfo = cpp_like_debuginfo (tcx) ; let mut vtable_name = String :: with_capacity (64) ; if cpp_like_debuginfo { vtable_name . push_str ("impl$<") ; } else { vtable_name . push ('<') ; } let mut visited = FxHashSet :: default () ; push_debuginfo_type_name (tcx , t , true , & mut vtable_name , & mut visited) ; if cpp_like_debuginfo { vtable_name . push_str (", ") ; } else { vtable_name . push_str (" as ") ; } if let Some (trait_ref) = trait_ref { let trait_ref = tcx . normalize_erasing_regions (ty :: TypingEnv :: fully_monomorphized () , trait_ref) ; push_item_name (tcx , trait_ref . def_id , true , & mut vtable_name) ; visited . clear () ; push_generic_params_internal (tcx , trait_ref . args , & mut vtable_name , & mut visited) ; } else { vtable_name . push ('_') ; } push_close_angle_bracket (cpp_like_debuginfo , & mut vtable_name) ; let suffix = match (cpp_like_debuginfo , kind) { (true , VTableNameKind :: GlobalVariable) => "::vtable$" , (false , VTableNameKind :: GlobalVariable) => "::{vtable}" , (true , VTableNameKind :: Type) => "::vtable_type$" , (false , VTableNameKind :: Type) => "::{vtable_type}" , } ; vtable_name . reserve_exact (suffix . len ()) ; vtable_name . push_str (suffix) ; vtable_name }
}

macro_rules! push_item_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_item_name in module {}", module_path!());
    };
}

mkfn!{
    push_item_name_introspect!();
...
macro_rules! coroutine_kind_label_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function coroutine_kind_label in module {}", module_path!());
    };
}

mkfn!{
    coroutine_kind_label_introspect!();
...
macro_rules! push_disambiguated_special_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_disambiguated_special_name in module {}", module_path!());
    };
}

mkfn!{
    push_disambiguated_special_name_introspect!();
...
macro_rules! push_unqualified_item_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_unqualified_item_name in module {}", module_path!());
    };
}

mkfn!{
    push_unqualified_item_name_introspect!();
...
macro_rules! push_generic_params_internal_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_generic_params_internal in module {}", module_path!());
    };
}

mkfn!{
    push_generic_params_internal_introspect!();
...
macro_rules! push_const_param_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_const_param in module {}", module_path!());
    };
}

mkfn!{
    push_const_param_introspect!();
...
macro_rules! push_generic_params_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_generic_params in module {}", module_path!());
    };
}

mkfn!{
    push_generic_params_introspect!();
...
macro_rules! push_closure_or_coroutine_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_closure_or_coroutine_name in module {}", module_path!());
    };
}

mkfn!{
    push_closure_or_coroutine_name_introspect!();
...
macro_rules! push_close_angle_bracket_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_close_angle_bracket in module {}", module_path!());
    };
}

mkfn!{
    push_close_angle_bracket_introspect!();
...
macro_rules! pop_close_angle_bracket_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function pop_close_angle_bracket in module {}", module_path!());
    };
}

mkfn!{
    pop_close_angle_bracket_introspect!();
...
macro_rules! push_arg_separator_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function push_arg_separator in module {}", module_path!());
    };
}

mkfn!{
    push_arg_separator_introspect!();
    fn push_arg_separator (cpp_like_debuginfo : bool , output : & mut String) { if cpp_like_debuginfo { output . push (',') ; } else { output . push_str (", ") ; } ; }
}

macro_rules! pop_arg_separator_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function pop_arg_separator in module {}", module_path!());
    };
}

mkfn!{
    pop_arg_separator_introspect!();
    fn pop_arg_separator (output : & mut String) { if output . ends_with (' ') { output . pop () ; } assert ! (output . ends_with (',')) ; output . pop () ; }
}

macro_rules! cpp_like_debuginfo_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function cpp_like_debuginfo in module {}", module_path!());
    };
}

mkfn!{
    cpp_like_debuginfo_introspect!();
