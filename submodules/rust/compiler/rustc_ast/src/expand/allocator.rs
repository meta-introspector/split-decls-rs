...
mkitem!{mkenum!{# [derive (Clone , Debug , Copy , Eq , PartialEq , HashStable_Generic)] pub enum AllocatorKind { Global , Default , }}}

macro_rules! global_fn_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function global_fn_name in module {}", module_path!());
    };
}

mkfn!{
    global_fn_name_introspect!();
    pub fn global_fn_name (base : Symbol) -> String { format ! ("__rust_{base}") }
}

macro_rules! default_fn_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function default_fn_name in module {}", module_path!());
    };
}

mkfn!{
    default_fn_name_introspect!();
    pub fn default_fn_name (base : Symbol) -> String { format ! ("__rdl_{base}") }
}

macro_rules! alloc_error_handler_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function alloc_error_handler_name in module {}", module_path!());
    };
}

mkfn!{
    alloc_error_handler_name_introspect!();
