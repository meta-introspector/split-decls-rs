...
mkuse!{use rustc_ast_pretty :: pprust as pprust_ast ;}
mkuse!{use rustc_middle :: bug ;}
mkuse!{use rustc_middle :: mir :: { write_mir_graphviz , write_mir_pretty } ;}
mkuse!{use rustc_middle :: ty :: { self , TyCtxt } ;}
mkuse!{use rustc_mir_build :: thir :: print :: { thir_flat , thir_tree } ;}
mkuse!{use rustc_public :: rustc_internal :: pretty :: write_smir_pretty ;}
mkuse!{use rustc_session :: Session ;}
mkuse!{use rustc_session :: config :: { OutFileName , PpHirMode , PpMode , PpSourceMode } ;}
mkuse!{use rustc_span :: { FileName , Ident } ;}
mkuse!{use tracing :: debug ;}
mkuse!{use { rustc_ast as ast , rustc_hir_pretty as pprust_hir } ;}
mkuse!{pub use self :: PpMode :: * ;}
mkuse!{pub use self :: PpSourceMode :: * ;}
mkitem!{mkstruct!{struct AstNoAnn ;}}
mkitem!{mkimpl!{impl pprust_ast :: PpAnn for AstNoAnn { }}}
mkitem!{mkstruct!{struct AstIdentifiedAnn ;}}
mkitem!{mkimpl!{impl pprust_ast :: PpAnn for AstIdentifiedAnn { fn pre (& self , s : & mut pprust_ast :: State < '_ > , node : pprust_ast :: AnnNode < '_ >) { if let pprust_ast :: AnnNode :: Expr (_) = node { s . popen () ; } } fn post (& self , s : & mut pprust_ast :: State < '_ > , node : pprust_ast :: AnnNode < '_ >) { match node { pprust_ast :: AnnNode :: Crate (_) | pprust_ast :: AnnNode :: Ident (_) | pprust_ast :: AnnNode :: Name (_) => { } pprust_ast :: AnnNode :: Item (item) => { s . s . space () ; s . synth_comment (item . id . to_string ()) } pprust_ast :: AnnNode :: SubItem (id) => { s . s . space () ; s . synth_comment (id . to_string ()) } pprust_ast :: AnnNode :: Block (blk) => { s . s . space () ; s . synth_comment (format ! ("block {}" , blk . id)) } pprust_ast :: AnnNode :: Expr (expr) => { s . s . space () ; s . synth_comment (expr . id . to_string ()) ; s . pclose () } pprust_ast :: AnnNode :: Pat (pat) => { s . s . space () ; s . synth_comment (format ! ("pat {}" , pat . id)) ; } } } }}}
mkitem!{mkstruct!{struct HirIdentifiedAnn < 'tcx > { tcx : TyCtxt < 'tcx > , }}}
mkitem!{mkimpl!{impl < 'tcx > pprust_hir :: PpAnn for HirIdentifiedAnn < 'tcx > { fn nested (& self , state : & mut pprust_hir :: State < '_ > , nested : pprust_hir :: Nested) { self . tcx . nested (state , nested) } fn pre (& self , s : & mut pprust_hir :: State < '_ > , node : pprust_hir :: AnnNode < '_ >) { if let pprust_hir :: AnnNode :: Expr (_) = node { s . popen () ; } } fn post (& self , s : & mut pprust_hir :: State < '_ > , node : pprust_hir :: AnnNode < '_ >) { match node { pprust_hir :: AnnNode :: Name (_) => { } pprust_hir :: AnnNode :: Item (item) => { s . s . space () ; s . synth_comment (format ! ("hir_id: {}" , item . hir_id ())) ; } pprust_hir :: AnnNode :: SubItem (id) => { s . s . space () ; s . synth_comment (id . to_string ()) ; } pprust_hir :: AnnNode :: Block (blk) => { s . s . space () ; s . synth_comment (format ! ("block hir_id: {}" , blk . hir_id)) ; } pprust_hir :: AnnNode :: Expr (expr) => { s . s . space () ; s . synth_comment (format ! ("expr hir_id: {}" , expr . hir_id)) ; s . pclose () ; } pprust_hir :: AnnNode :: Pat (pat) => { s . s . space () ; s . synth_comment (format ! ("pat hir_id: {}" , pat . hir_id)) ; } pprust_hir :: AnnNode :: TyPat (pat) => { s . s . space () ; s . synth_comment (format ! ("ty pat hir_id: {}" , pat . hir_id)) ; } pprust_hir :: AnnNode :: Arm (arm) => { s . s . space () ; s . synth_comment (format ! ("arm hir_id: {}" , arm . hir_id)) ; } } } }}}
mkitem!{mkstruct!{struct AstHygieneAnn < 'a > { sess : & 'a Session , }}}
mkitem!{mkimpl!{impl < 'a > pprust_ast :: PpAnn for AstHygieneAnn < 'a > { fn post (& self , s : & mut pprust_ast :: State < '_ > , node : pprust_ast :: AnnNode < '_ >) { match node { pprust_ast :: AnnNode :: Ident (& Ident { name , span }) => { s . s . space () ; s . synth_comment (format ! ("{}{:?}" , name . as_u32 () , span . ctxt ())) } pprust_ast :: AnnNode :: Name (& name) => { s . s . space () ; s . synth_comment (name . as_u32 () . to_string ()) } pprust_ast :: AnnNode :: Crate (_) => { s . s . hardbreak () ; let verbose = self . sess . verbose_internals () ; s . synth_comment (rustc_span :: hygiene :: debug_hygiene_data (verbose)) ; s . s . hardbreak_if_not_bol () ; } _ => { } } } }}}
mkitem!{mkstruct!{struct HirTypedAnn < 'tcx > { tcx : TyCtxt < 'tcx > , maybe_typeck_results : Cell < Option < & 'tcx ty :: TypeckResults < 'tcx > > > , }}}
mkitem!{mkimpl!{impl < 'tcx > pprust_hir :: PpAnn for HirTypedAnn < 'tcx > { fn nested (& self , state : & mut pprust_hir :: State < '_ > , nested : pprust_hir :: Nested) { let old_maybe_typeck_results = self . maybe_typeck_results . get () ; if let pprust_hir :: Nested :: Body (id) = nested { self . maybe_typeck_results . set (Some (self . tcx . typeck_body (id))) ; } self . tcx . nested (state , nested) ; self . maybe_typeck_results . set (old_maybe_typeck_results) ; } fn pre (& self , s : & mut pprust_hir :: State < '_ > , node : pprust_hir :: AnnNode < '_ >) { if let pprust_hir :: AnnNode :: Expr (_) = node { s . popen () ; } } fn post (& self , s : & mut pprust_hir :: State < '_ > , node : pprust_hir :: AnnNode < '_ >) { if let pprust_hir :: AnnNode :: Expr (expr) = node { let typeck_results = self . maybe_typeck_results . get () . or_else (| | { self . tcx . hir_maybe_body_owned_by (expr . hir_id . owner . def_id) . map (| body_id | self . tcx . typeck_body (body_id . id ())) }) ; if let Some (typeck_results) = typeck_results { s . s . space () ; s . s . word ("as") ; s . s . space () ; s . s . word (typeck_results . expr_ty (expr) . to_string ()) ; } s . pclose () ; } } }}}

macro_rules! get_source_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function get_source in module {}", module_path!());
    };
}

mkfn!{
    get_source_introspect!();
