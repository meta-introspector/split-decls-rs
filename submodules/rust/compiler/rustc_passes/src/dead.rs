...
mkuse!{use std :: mem ;}
mkuse!{use hir :: def_id :: { LocalDefIdMap , LocalDefIdSet } ;}
...
mkuse!{use rustc_hir :: def_id :: { DefId , LocalDefId , LocalModDefId } ;}
mkuse!{use rustc_hir :: intravisit :: { self , Visitor } ;}
mkuse!{use rustc_hir :: { self as hir , Node , PatKind , QPath } ;}
mkuse!{use rustc_middle :: middle :: codegen_fn_attrs :: CodegenFnAttrFlags ;}
mkuse!{use rustc_middle :: middle :: privacy :: Level ;}
mkuse!{use rustc_middle :: query :: Providers ;}
mkuse!{use rustc_middle :: ty :: { self , AssocTag , TyCtxt } ;}
mkuse!{use rustc_middle :: { bug , span_bug } ;}
mkuse!{use rustc_session :: lint :: builtin :: DEAD_CODE ;}
mkuse!{use rustc_session :: lint :: { self , LintExpectationId } ;}
mkuse!{use rustc_span :: { Symbol , kw , sym } ;}
mkuse!{use crate :: errors :: { ChangeFields , IgnoredDerivedImpls , MultipleDeadCodes , ParentInfo , UselessAssignment , } ;}

macro_rules! should_explore_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function should_explore in module {}", module_path!());
    };
}

mkfn!{
    should_explore_introspect!();
...
macro_rules! maybe_record_as_seed_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function maybe_record_as_seed in module {}", module_path!());
    };
}

mkfn!{
    maybe_record_as_seed_introspect!();
...
macro_rules! create_and_seed_worklist_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function create_and_seed_worklist in module {}", module_path!());
    };
}

mkfn!{
    create_and_seed_worklist_introspect!();
...
macro_rules! live_symbols_and_ignored_derived_traits_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function live_symbols_and_ignored_derived_traits in module {}", module_path!());
    };
}

mkfn!{
    live_symbols_and_ignored_derived_traits_introspect!();
...
mkfn!{
    provide_introspect!();
    pub (crate) fn provide (providers : & mut Providers) { * providers = Providers { live_symbols_and_ignored_derived_traits , check_mod_deathness , .. * providers } ; }
}