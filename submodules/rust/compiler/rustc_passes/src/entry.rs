...
mkuse!{use rustc_ast :: entry :: EntryPointType ;}
mkuse!{use rustc_errors :: codes :: * ;}
mkuse!{use rustc_hir :: def :: DefKind ;}
mkuse!{use rustc_hir :: def_id :: { CRATE_DEF_ID , DefId , LOCAL_CRATE , LocalDefId } ;}
mkuse!{use rustc_hir :: { CRATE_HIR_ID , ItemId , Node } ;}
mkuse!{use rustc_middle :: query :: Providers ;}
mkuse!{use rustc_middle :: ty :: TyCtxt ;}
mkuse!{use rustc_session :: RemapFileNameExt ;}
mkuse!{use rustc_session :: config :: { CrateType , EntryFnType , RemapPathScopeComponents , sigpipe } ;}
mkuse!{use rustc_span :: { Span , Symbol , sym } ;}
mkuse!{use crate :: errors :: { AttrOnlyInFunctions , ExternMain , MultipleRustcMain , NoMainErr } ;}
mkitem!{mkstruct!{struct EntryContext < 'tcx > { tcx : TyCtxt < 'tcx > , # [doc = " The function has the `#[rustc_main]` attribute."] rustc_main_fn : Option < (LocalDefId , Span) > , # [doc = " The functions that one might think are `main` but aren't, e.g."] # [doc = " main functions not defined at the top level. For diagnostics."] non_main_fns : Vec < Span > , }}}
...
macro_rules! attr_span_by_symbol_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function attr_span_by_symbol in module {}", module_path!());
    };
}

mkfn!{
    attr_span_by_symbol_introspect!();
...
macro_rules! check_and_search_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_and_search_item in module {}", module_path!());
    };
}

mkfn!{
    check_and_search_item_introspect!();
...
macro_rules! configure_main_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function configure_main in module {}", module_path!());
    };
}

mkfn!{
    configure_main_introspect!();
...
macro_rules! sigpipe_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function sigpipe in module {}", module_path!());
    };
}
...
macro_rules! no_main_err_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function no_main_err in module {}", module_path!());
    };
}

mkfn!{
    no_main_err_introspect!();
...
mkfn!{
    provide_introspect!();
    pub fn provide (providers : & mut Providers) { * providers = Providers { entry_fn , .. * providers } ; }
}