...
mkuse!{use rustc_ast :: visit ;}
mkuse!{use rustc_data_structures :: fx :: FxHashSet ;}
mkuse!{use rustc_hir :: lang_items :: { self , LangItem } ;}
mkuse!{use rustc_hir :: weak_lang_items :: WEAK_LANG_ITEMS ;}
mkuse!{use rustc_middle :: middle :: lang_items :: required ;}
mkuse!{use rustc_middle :: ty :: TyCtxt ;}
mkuse!{use rustc_session :: config :: CrateType ;}
mkuse!{use crate :: errors :: { MissingLangItem , MissingPanicHandler , PanicUnwindWithoutStd , UnknownExternLangItem , } ;}
...
mkitem!{mkstruct!{struct WeakLangItemVisitor < 'a , 'tcx > { tcx : TyCtxt < 'tcx > , items : & 'a mut lang_items :: LanguageItems , }}}
mkitem!{mkimpl!{impl < 'ast > visit :: Visitor < 'ast > for WeakLangItemVisitor < '_ , '_ > { fn visit_foreign_item (& mut self , i : & 'ast ast :: ForeignItem) { if let Some ((lang_item , _)) = lang_items :: extract (& i . attrs) { if let Some (item) = LangItem :: from_name (lang_item) && item . is_weak () { if self . items . get (item) . is_none () { self . items . missing . push (item) ; } } else { self . tcx . dcx () . emit_err (UnknownExternLangItem { span : i . span , lang_item }) ; } } } }}}

macro_rules! verify_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function verify in module {}", module_path!());
    };
}
