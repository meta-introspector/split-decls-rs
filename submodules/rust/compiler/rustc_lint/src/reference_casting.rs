...
mkitem!{mkenum!{# [derive (Debug , Clone , Copy , PartialEq , Eq)] enum PatternKind { Borrow { mutbl : Mutability } , Assign , }}}

macro_rules! borrow_or_assign_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function borrow_or_assign in module {}", module_path!());
    };
}

mkfn!{
    borrow_or_assign_introspect!();
...
macro_rules! is_cast_from_ref_to_mut_ptr_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function is_cast_from_ref_to_mut_ptr in module {}", module_path!());
    };
}

mkfn!{
    is_cast_from_ref_to_mut_ptr_introspect!();
...
macro_rules! is_cast_to_bigger_memory_layout_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function is_cast_to_bigger_memory_layout in module {}", module_path!());
    };
}

mkfn!{
    is_cast_to_bigger_memory_layout_introspect!();
