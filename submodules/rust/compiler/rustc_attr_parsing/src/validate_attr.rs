...
mkuse!{use rustc_ast :: tokenstream :: DelimSpan ;}
mkuse!{use rustc_ast :: { self as ast , AttrArgs , Attribute , DelimArgs , MetaItem , MetaItemInner , MetaItemKind , NodeId , Path , Safety , } ;}
mkuse!{use rustc_errors :: { Applicability , DiagCtxtHandle , FatalError , PResult } ;}
mkuse!{use rustc_feature :: { AttributeSafety , AttributeTemplate , BUILTIN_ATTRIBUTE_MAP , BuiltinAttribute } ;}
mkuse!{use rustc_parse :: parse_in ;}
mkuse!{use rustc_session :: errors :: report_lit_error ;}
mkuse!{use rustc_session :: lint :: BuiltinLintDiag ;}
mkuse!{use rustc_session :: lint :: builtin :: { ILL_FORMED_ATTRIBUTE_INPUT , UNSAFE_ATTR_OUTSIDE_UNSAFE } ;}
mkuse!{use rustc_session :: parse :: ParseSess ;}
mkuse!{use rustc_span :: { Span , Symbol , sym } ;}
mkuse!{use crate :: { AttributeParser , Late , session_diagnostics as errors } ;}

macro_rules! check_attr_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_attr in module {}", module_path!());
    };
}

mkfn!{
    check_attr_introspect!();
...
macro_rules! parse_meta_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function parse_meta in module {}", module_path!());
    };
}

mkfn!{
    parse_meta_introspect!();
...
macro_rules! check_meta_bad_delim_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_meta_bad_delim in module {}", module_path!());
    };
}

mkfn!{
    check_meta_bad_delim_introspect!();
...
macro_rules! is_attr_template_compatible_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function is_attr_template_compatible in module {}", module_path!());
    };
}

mkfn!{
    is_attr_template_compatible_introspect!();
...
macro_rules! check_attribute_safety_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_attribute_safety in module {}", module_path!());
    };
}

mkfn!{
    check_attribute_safety_introspect!();
...
macro_rules! deny_builtin_meta_unsafety_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function deny_builtin_meta_unsafety in module {}", module_path!());
    };
}

mkfn!{
    deny_builtin_meta_unsafety_introspect!();
...
macro_rules! check_builtin_meta_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_builtin_meta_item in module {}", module_path!());
    };
}

mkfn!{
    check_builtin_meta_item_introspect!();
...
macro_rules! emit_malformed_attribute_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function emit_malformed_attribute in module {}", module_path!());
    };
}

mkfn!{
    emit_malformed_attribute_introspect!();
...
macro_rules! emit_fatal_malformed_builtin_attribute_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function emit_fatal_malformed_builtin_attribute in module {}", module_path!());
    };
}

mkfn!{
    emit_fatal_malformed_builtin_attribute_introspect!();
