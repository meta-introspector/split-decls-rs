...
mkuse!{use rustc_hir :: attrs :: MacroUseArgs ;}
mkuse!{use super :: prelude :: * ;}
mkuse!{use crate :: session_diagnostics :: IllFormedAttributeInputLint ;}
mkitem!{mkstruct!{pub (crate) struct MacroEscapeParser ;}}
mkitem!{mkimpl!{impl < S : Stage > NoArgsAttributeParser < S > for MacroEscapeParser { const PATH : & [Symbol] = & [sym :: macro_escape] ; const ON_DUPLICATE : OnDuplicate < S > = OnDuplicate :: Warn ; const ALLOWED_TARGETS : AllowedTargets = MACRO_USE_ALLOWED_TARGETS ; const CREATE : fn (Span) -> AttributeKind = AttributeKind :: MacroEscape ; }}}
mkitem!{mkstruct!{#[doc = " `#[macro_use]` attributes can either:"] #[doc = " - Use all macros from a crate, if provided without arguments"] #[doc = " - Use specific macros from a crate, if provided with arguments `#[macro_use(macro1, macro2)]`"] #[doc = " A warning should be provided if an use all is combined with specific uses, or if multiple use-alls are used."] #[derive (Default)] pub (crate) struct MacroUseParser { state : MacroUseArgs , #[doc = " Spans of all `#[macro_use]` arguments with arguments, used for linting"] uses_attr_spans : ThinVec < Span > , #[doc = " If `state` is `UseSpecific`, stores the span of the first `#[macro_use]` argument, used as the span for this attribute"] #[doc = " If `state` is `UseAll`, stores the span of the first `#[macro_use]` arguments without arguments"] first_span : Option < Span > , }}}
mkitem!{const MACRO_USE_TEMPLATE : AttributeTemplate = template ! (Word , List : & ["name1, name2, ..."] , "https://doc.rust-lang.org/reference/macros-by-example.html#the-macro_use-attribute") ;}
mkitem!{const MACRO_USE_ALLOWED_TARGETS : AllowedTargets = AllowedTargets :: AllowListWarnRest (& [Allow (Target :: Mod) , Allow (Target :: ExternCrate) , Allow (Target :: Crate) , Error (Target :: WherePredicate) ,]) ;}
...
mkitem!{mkstruct!{pub (crate) struct AllowInternalUnsafeParser ;}}
mkitem!{mkimpl!{impl < S : Stage > NoArgsAttributeParser < S > for AllowInternalUnsafeParser { const PATH : & [Symbol] = & [sym :: allow_internal_unsafe] ; const ON_DUPLICATE : OnDuplicate < S > = OnDuplicate :: Ignore ; const ALLOWED_TARGETS : AllowedTargets = AllowedTargets :: AllowList (& [Allow (Target :: Fn) , Allow (Target :: MacroDef) , Warn (Target :: Field) , Warn (Target :: Arm) ,]) ; const CREATE : fn (Span) -> AttributeKind = | span | AttributeKind :: AllowInternalUnsafe (span) ; }}}