...
mkuse!{use rustc_ast :: { LitKind , MetaItem , MetaItemInner , MetaItemKind , MetaItemLit , NodeId } ;}
mkuse!{use rustc_ast_pretty :: pprust ;}
mkuse!{use rustc_feature :: { Features , GatedCfg , find_gated_cfg } ;}
...
mkuse!{use rustc_session :: lint :: { BuiltinLintDiag , Lint } ;}
...
mkuse!{use crate :: { fluent_generated , parse_version } ;}
mkitem!{mktrait!{#[doc = " Emitter of a builtin lint from `cfg_matches`."] #[doc = ""] #[doc = " Used to support emitting a lint (currently on check-cfg), either:"] #[doc = "  - as an early buffered lint (in `rustc`)"] #[doc = "  - or has a \"normal\" lint from HIR (in `rustdoc`)"] pub trait CfgMatchesLintEmitter { fn emit_span_lint (& self , sess : & Session , lint : & 'static Lint , sp : Span , diag : BuiltinLintDiag) ; }}}
mkitem!{mkimpl!{impl CfgMatchesLintEmitter for NodeId { fn emit_span_lint (& self , sess : & Session , lint : & 'static Lint , sp : Span , diag : BuiltinLintDiag) { sess . psess . buffer_lint (lint , sp , * self , diag) ; } }}}
mkitem!{mkstruct!{#[derive (Clone , Debug)] pub struct Condition { pub name : Symbol , pub name_span : Span , pub value : Option < Symbol > , pub value_span : Option < Span > , pub span : Span , }}}

macro_rules! cfg_matches_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function cfg_matches in module {}", module_path!());
    };
}

mkfn!{
    cfg_matches_introspect!();
...
macro_rules! try_gate_cfg_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function try_gate_cfg in module {}", module_path!());
    };
}

mkfn!{
    try_gate_cfg_introspect!();
...
macro_rules! gate_cfg_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function gate_cfg in module {}", module_path!());
    };
}
...
macro_rules! eval_condition_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function eval_condition in module {}", module_path!());
    };
}

mkfn!{
    eval_condition_introspect!();
