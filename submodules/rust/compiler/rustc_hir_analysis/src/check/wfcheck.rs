...
mkuse!{use rustc_errors :: { Applicability , ErrorGuaranteed , pluralize , struct_span_code_err } ;}
mkuse!{use rustc_hir :: def :: { DefKind , Res } ;}
mkuse!{use rustc_hir :: def_id :: { DefId , LocalDefId } ;}
mkuse!{use rustc_hir :: lang_items :: LangItem ;}
mkuse!{use rustc_hir :: { AmbigArg , ItemKind } ;}
mkuse!{use rustc_infer :: infer :: outlives :: env :: OutlivesEnvironment ;}
mkuse!{use rustc_infer :: infer :: { self , InferCtxt , SubregionOrigin , TyCtxtInferExt } ;}
mkuse!{use rustc_lint_defs :: builtin :: SUPERTRAIT_ITEM_SHADOWING_DEFINITION ;}
mkuse!{use rustc_macros :: LintDiagnostic ;}
mkuse!{use rustc_middle :: mir :: interpret :: ErrorHandled ;}
mkuse!{use rustc_middle :: traits :: solve :: NoSolution ;}
mkuse!{use rustc_middle :: ty :: trait_def :: TraitSpecializationKind ;}
mkuse!{use rustc_middle :: ty :: { self , AdtKind , GenericArgKind , GenericArgs , GenericParamDefKind , Ty , TyCtxt , TypeFlags , TypeFoldable , TypeSuperVisitable , TypeVisitable , TypeVisitableExt , TypeVisitor , TypingMode , Upcast , } ;}
...
mkuse!{use rustc_trait_selection :: regions :: { InferCtxtRegionExt , OutlivesEnvironmentBuildExt } ;}
mkuse!{use rustc_trait_selection :: traits :: misc :: { ConstParamTyImplementationError , type_allowed_to_implement_const_param_ty , } ;}
mkuse!{use rustc_trait_selection :: traits :: query :: evaluate_obligation :: InferCtxtExt as _ ;}
mkuse!{use rustc_trait_selection :: traits :: { self , FulfillmentError , Obligation , ObligationCause , ObligationCauseCode , ObligationCtxt , WellFormedLoc , } ;}
mkuse!{use tracing :: { debug , instrument } ;}
mkuse!{use { rustc_ast as ast , rustc_hir as hir } ;}
mkuse!{use crate :: autoderef :: Autoderef ;}
mkuse!{use crate :: constrained_generic_params :: { Parameter , identify_constrained_generic_params } ;}
mkuse!{use crate :: errors :: InvalidReceiverTyHint ;}
mkuse!{use crate :: { errors , fluent_generated as fluent } ;}
mkitem!{mkstruct!{pub (super) struct WfCheckingCtxt < 'a , 'tcx > { pub (super) ocx : ObligationCtxt < 'a , 'tcx , FulfillmentError < 'tcx > > , body_def_id : LocalDefId , param_env : ty :: ParamEnv < 'tcx > , }}}
mkitem!{mkimpl!{impl < 'a , 'tcx > Deref for WfCheckingCtxt < 'a , 'tcx > { type Target = ObligationCtxt < 'a , 'tcx , FulfillmentError < 'tcx > > ; fn deref (& self) -> & Self :: Target { & self . ocx } }}}
mkitem!{mkimpl!{impl < 'tcx > WfCheckingCtxt < '_ , 'tcx > { fn tcx (& self) -> TyCtxt < 'tcx > { self . ocx . infcx . tcx } fn normalize < T > (& self , span : Span , loc : Option < WellFormedLoc > , value : T) -> T where T : TypeFoldable < TyCtxt < 'tcx > > , { self . ocx . normalize (& ObligationCause :: new (span , self . body_def_id , ObligationCauseCode :: WellFormed (loc)) , self . param_env , value ,) } # [doc = " Convenience function to *deeply* normalize during wfcheck. In the old solver,"] # [doc = " this just dispatches to [`WfCheckingCtxt::normalize`], but in the new solver"] # [doc = " this calls `deeply_normalize` and reports errors if they are encountered."] # [doc = ""] # [doc = " This function should be called in favor of `normalize` in cases where we will"] # [doc = " then check the well-formedness of the type, since we only use the normalized"] # [doc = " signature types for implied bounds when checking regions."] pub (super) fn deeply_normalize < T > (& self , span : Span , loc : Option < WellFormedLoc > , value : T) -> T where T : TypeFoldable < TyCtxt < 'tcx > > , { if self . infcx . next_trait_solver () { match self . ocx . deeply_normalize (& ObligationCause :: new (span , self . body_def_id , ObligationCauseCode :: WellFormed (loc)) , self . param_env , value . clone () ,) { Ok (value) => value , Err (errors) => { self . infcx . err_ctxt () . report_fulfillment_errors (errors) ; value } } } else { self . normalize (span , loc , value) } } pub (super) fn register_wf_obligation (& self , span : Span , loc : Option < WellFormedLoc > , term : ty :: Term < 'tcx > ,) { let cause = traits :: ObligationCause :: new (span , self . body_def_id , ObligationCauseCode :: WellFormed (loc) ,) ; self . ocx . register_obligation (Obligation :: new (self . tcx () , cause , self . param_env , ty :: ClauseKind :: WellFormed (term) ,)) ; } }}}

macro_rules! enter_wf_checking_ctxt_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function enter_wf_checking_ctxt in module {}", module_path!());
    };
}

mkfn!{
    enter_wf_checking_ctxt_introspect!();
...
macro_rules! check_well_formed_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_well_formed in module {}", module_path!());
    };
}

mkfn!{
    check_well_formed_introspect!();
...
macro_rules! check_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_item in module {}", module_path!());
    };
}

mkfn!{
    check_item_introspect!();
...
macro_rules! check_foreign_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_foreign_item in module {}", module_path!());
    };
}

mkfn!{
    check_foreign_item_introspect!();
...
macro_rules! check_trait_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_trait_item in module {}", module_path!());
    };
}

mkfn!{
    check_trait_item_introspect!();
...
macro_rules! check_gat_where_clauses_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_gat_where_clauses in module {}", module_path!());
    };
}

mkfn!{
    check_gat_where_clauses_introspect!();
...
macro_rules! augment_param_env_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function augment_param_env in module {}", module_path!());
    };
}

mkfn!{
    augment_param_env_introspect!();
...
macro_rules! gather_gat_bounds_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function gather_gat_bounds in module {}", module_path!());
    };
}

mkfn!{
    gather_gat_bounds_introspect!();
...
macro_rules! ty_known_to_outlive_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function ty_known_to_outlive in module {}", module_path!());
    };
}

mkfn!{
    ty_known_to_outlive_introspect!();
...
macro_rules! region_known_to_outlive_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function region_known_to_outlive in module {}", module_path!());
    };
}

mkfn!{
    region_known_to_outlive_introspect!();
...
macro_rules! test_region_obligations_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function test_region_obligations in module {}", module_path!());
    };
}

mkfn!{
    test_region_obligations_introspect!();
...
mkitem!{mkstruct!{# [doc = " TypeVisitor that looks for uses of GATs like"] # [doc = " `<P0 as Trait<P1..Pn>>::GAT<Pn..Pm>` and adds the arguments `P0..Pm` into"] # [doc = " the two vectors, `regions` and `types` (depending on their kind). For each"] # [doc = " parameter `Pi` also track the index `i`."] struct GATArgsCollector < 'tcx > { gat : DefId , regions : FxIndexSet < (ty :: Region < 'tcx > , usize) > , types : FxIndexSet < (Ty < 'tcx > , usize) > , }}}
mkitem!{mkimpl!{impl < 'tcx > GATArgsCollector < 'tcx > { fn visit < T : TypeFoldable < TyCtxt < 'tcx > > > (gat : DefId , t : T ,) -> (FxIndexSet < (ty :: Region < 'tcx > , usize) > , FxIndexSet < (Ty < 'tcx > , usize) >) { let mut visitor = GATArgsCollector { gat , regions : FxIndexSet :: default () , types : FxIndexSet :: default () } ; t . visit_with (& mut visitor) ; (visitor . regions , visitor . types) } }}}
mkitem!{mkimpl!{impl < 'tcx > TypeVisitor < TyCtxt < 'tcx > > for GATArgsCollector < 'tcx > { fn visit_ty (& mut self , t : Ty < 'tcx >) { match t . kind () { ty :: Alias (ty :: Projection , p) if p . def_id == self . gat => { for (idx , arg) in p . args . iter () . enumerate () { match arg . kind () { GenericArgKind :: Lifetime (lt) if ! lt . is_bound () => { self . regions . insert ((lt , idx)) ; } GenericArgKind :: Type (t) => { self . types . insert ((t , idx)) ; } _ => { } } } } _ => { } } t . super_visit_with (self) } }}}

macro_rules! lint_item_shadowing_supertrait_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function lint_item_shadowing_supertrait_item in module {}", module_path!());
    };
}

mkfn!{
    lint_item_shadowing_supertrait_item_introspect!();
...
macro_rules! check_param_wf_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_param_wf in module {}", module_path!());
    };
}

mkfn!{
    check_param_wf_introspect!();
...
macro_rules! check_associated_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_associated_item in module {}", module_path!());
    };
}

mkfn!{
    check_associated_item_introspect!();
    # [instrument (level = "debug" , skip (tcx))] pub (crate) fn check_associated_item (tcx : TyCtxt < '_ > , item_id : LocalDefId ,) -> Result < () , ErrorGuaranteed > { let loc = Some (WellFormedLoc :: Ty (item_id)) ; enter_wf_checking_ctxt (tcx , item_id , | wfcx | { let item = tcx . associated_item (item_id) ; tcx . ensure_ok () . coherent_trait (tcx . parent (item . trait_item_or_self () ?)) ? ; let self_ty = match item . container { ty :: AssocContainer :: Trait => tcx . types . self_param , ty :: AssocContainer :: InherentImpl | ty :: AssocContainer :: TraitImpl (_) => { tcx . type_of (item . container_id (tcx)) . instantiate_identity () } } ; let span = tcx . def_span (item_id) ; match item . kind { ty :: AssocKind :: Const { .. } => { let ty = tcx . type_of (item . def_id) . instantiate_identity () ; let ty = wfcx . deeply_normalize (span , Some (WellFormedLoc :: Ty (item_id)) , ty) ; wfcx . register_wf_obligation (span , loc , ty . into ()) ; check_sized_if_body (wfcx , item . def_id . expect_local () , ty , Some (span) , ObligationCauseCode :: SizedConstOrStatic ,) ; Ok (()) } ty :: AssocKind :: Fn { .. } => { let sig = tcx . fn_sig (item . def_id) . instantiate_identity () ; let hir_sig = tcx . hir_node_by_def_id (item_id) . fn_sig () . expect ("bad signature for method") ; check_fn_or_method (wfcx , sig , hir_sig . decl , item_id) ; check_method_receiver (wfcx , hir_sig , item , self_ty) } ty :: AssocKind :: Type { .. } => { if let ty :: AssocContainer :: Trait = item . container { check_associated_type_bounds (wfcx , item , span) } if item . defaultness (tcx) . has_value () { let ty = tcx . type_of (item . def_id) . instantiate_identity () ; let ty = wfcx . deeply_normalize (span , Some (WellFormedLoc :: Ty (item_id)) , ty) ; wfcx . register_wf_obligation (span , loc , ty . into ()) ; } Ok (()) } } }) }
}

macro_rules! check_type_defn_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_type_defn in module {}", module_path!());
    };
}

mkfn!{
    check_type_defn_introspect!();
...
macro_rules! check_trait_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_trait in module {}", module_path!());
    };
}

mkfn!{
    check_trait_introspect!();
...
macro_rules! check_associated_type_bounds_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_associated_type_bounds in module {}", module_path!());
    };
}

mkfn!{
    check_associated_type_bounds_introspect!();
...
macro_rules! check_item_fn_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_item_fn in module {}", module_path!());
    };
}

mkfn!{
    check_item_fn_introspect!();
...
macro_rules! check_static_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_static_item in module {}", module_path!());
    };
}

mkfn!{
    check_static_item_introspect!();
...
macro_rules! check_const_item_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_const_item in module {}", module_path!());
    };
}

mkfn!{
    check_const_item_introspect!();
...
macro_rules! check_where_clauses_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_where_clauses in module {}", module_path!());
    };
}

mkfn!{
    check_where_clauses_introspect!();
...
macro_rules! check_fn_or_method_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_fn_or_method in module {}", module_path!());
    };
}

mkfn!{
    check_fn_or_method_introspect!();
...
macro_rules! check_sized_if_body_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_sized_if_body in module {}", module_path!());
    };
}

mkfn!{
    check_sized_if_body_introspect!();
...
mkitem!{mkenum!{# [doc = " The `arbitrary_self_types_pointers` feature implies `arbitrary_self_types`."] # [derive (Clone , Copy , PartialEq)] enum ArbitrarySelfTypesLevel { Basic , WithPointers , }}}

macro_rules! check_method_receiver_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_method_receiver in module {}", module_path!());
    };
}

mkfn!{
    check_method_receiver_introspect!();
...
mkitem!{mkenum!{# [doc = " Error cases which may be returned from `receiver_is_valid`. These error"] # [doc = " cases are generated in this function as they may be unearthed as we explore"] # [doc = " the `autoderef` chain, but they're converted to diagnostics in the caller."] enum ReceiverValidityError { # [doc = " The self type does not get to the receiver type by following the"] # [doc = " autoderef chain."] DoesNotDeref , # [doc = " A type was found which is a method type parameter, and that's not allowed."] MethodGenericParamUsed , }}}

macro_rules! confirm_type_is_not_a_method_generic_param_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function confirm_type_is_not_a_method_generic_param in module {}", module_path!());
    };
}

mkfn!{
    confirm_type_is_not_a_method_generic_param_introspect!();
...
macro_rules! receiver_is_valid_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function receiver_is_valid in module {}", module_path!());
    };
}

mkfn!{
    receiver_is_valid_introspect!();
...
macro_rules! legacy_receiver_is_implemented_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function legacy_receiver_is_implemented in module {}", module_path!());
    };
}

mkfn!{
    legacy_receiver_is_implemented_introspect!();
...
macro_rules! check_variances_for_type_defn_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_variances_for_type_defn in module {}", module_path!());
    };
}

mkfn!{
    check_variances_for_type_defn_introspect!();
...
mkitem!{mkstruct!{# [doc = " Detects cases where an ADT/LTA is trivially cyclical -- we want to detect this so"] # [doc = " we only mention that its parameters are used cyclically if the ADT/LTA is truly"] # [doc = " cyclical."] # [doc = ""] # [doc = " Notably, we don't consider substitutions here, so this may have false positives."] struct IsProbablyCyclical < 'tcx > { tcx : TyCtxt < 'tcx > , item_def_id : DefId , seen : FxHashSet < DefId > , }}}
mkitem!{mkimpl!{impl < 'tcx > IsProbablyCyclical < 'tcx > { fn visit_def (& mut self , def_id : DefId) -> ControlFlow < () , () > { match self . tcx . def_kind (def_id) { DefKind :: Struct | DefKind :: Enum | DefKind :: Union => { self . tcx . adt_def (def_id) . all_fields () . try_for_each (| field | { self . tcx . type_of (field . did) . instantiate_identity () . visit_with (self) }) } DefKind :: TyAlias if self . tcx . type_alias_is_lazy (def_id) => { self . tcx . type_of (def_id) . instantiate_identity () . visit_with (self) } _ => ControlFlow :: Continue (()) , } } }}}
mkitem!{mkimpl!{impl < 'tcx > TypeVisitor < TyCtxt < 'tcx > > for IsProbablyCyclical < 'tcx > { type Result = ControlFlow < () , () > ; fn visit_ty (& mut self , ty : Ty < 'tcx >) -> ControlFlow < () , () > { let def_id = match ty . kind () { ty :: Adt (adt_def , _) => Some (adt_def . did ()) , ty :: Alias (ty :: Free , alias_ty) => Some (alias_ty . def_id) , _ => None , } ; if let Some (def_id) = def_id { if def_id == self . item_def_id { return ControlFlow :: Break (()) ; } if self . seen . insert (def_id) { self . visit_def (def_id) ? ; } } ty . super_visit_with (self) } }}}
mkitem!{mkstruct!{# [doc = " Collect usages of the `param_def_id` and `Res::SelfTyAlias` in the HIR."] # [doc = ""] # [doc = " This is used to report places where the user has used parameters in a"] # [doc = " non-variance-constraining way for better bivariance errors."] struct CollectUsageSpans < 'a > { spans : & 'a mut Vec < Span > , param_def_id : DefId , }}}
mkitem!{mkimpl!{impl < 'tcx > Visitor < 'tcx > for CollectUsageSpans < '_ > { type Result = () ; fn visit_generics (& mut self , _g : & 'tcx rustc_hir :: Generics < 'tcx >) -> Self :: Result { } fn visit_ty (& mut self , t : & 'tcx hir :: Ty < 'tcx , AmbigArg >) -> Self :: Result { if let hir :: TyKind :: Path (hir :: QPath :: Resolved (None , qpath)) = t . kind { if let Res :: Def (DefKind :: TyParam , def_id) = qpath . res && def_id == self . param_def_id { self . spans . push (t . span) ; return ; } else if let Res :: SelfTyAlias { .. } = qpath . res { self . spans . push (t . span) ; return ; } } intravisit :: walk_ty (self , t) ; } }}}
...
macro_rules! check_type_wf_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_type_wf in module {}", module_path!());
    };
}

mkfn!{
    check_type_wf_introspect!();
...
macro_rules! lint_redundant_lifetimes_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function lint_redundant_lifetimes in module {}", module_path!());
    };
}

mkfn!{
    lint_redundant_lifetimes_introspect!();
...
mkitem!{mkstruct!{# [derive (LintDiagnostic)] # [diag (hir_analysis_redundant_lifetime_args)] # [note] struct RedundantLifetimeArgsLint < 'tcx > { # [doc = " The lifetime we have found to be redundant."] victim : ty :: Region < 'tcx > , candidate : ty :: Region < 'tcx > , }}}