...
mkuse!{use rustc_middle :: ty :: { self , Ty , TyCtxt , TypeFoldable , TypeSuperVisitable , TypeVisitor } ;}
mkuse!{use rustc_span :: Span ;}
mkuse!{use tracing :: debug ;}
mkitem!{mkstruct!{# [derive (Clone , PartialEq , Eq , Hash , Debug)] pub (crate) struct Parameter (pub u32) ;}}
mkitem!{mkimpl!{impl From < ty :: ParamTy > for Parameter { fn from (param : ty :: ParamTy) -> Self { Parameter (param . index) } }}}
mkitem!{mkimpl!{impl From < ty :: EarlyParamRegion > for Parameter { fn from (param : ty :: EarlyParamRegion) -> Self { Parameter (param . index) } }}}
mkitem!{mkimpl!{impl From < ty :: ParamConst > for Parameter { fn from (param : ty :: ParamConst) -> Self { Parameter (param . index) } }}}

macro_rules! parameters_for_impl_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function parameters_for_impl in module {}", module_path!());
    };
}

mkfn!{
    parameters_for_impl_introspect!();
...
macro_rules! parameters_for_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function parameters_for in module {}", module_path!());
    };
}

mkfn!{
    parameters_for_introspect!();
