...
mkuse!{use rustc_errors :: { Applicability , Diag , DiagCtxtHandle , E0228 , ErrorGuaranteed , StashKey , struct_span_code_err , } ;}
mkuse!{use rustc_hir :: attrs :: AttributeKind ;}
mkuse!{use rustc_hir :: def :: DefKind ;}
mkuse!{use rustc_hir :: def_id :: { DefId , LocalDefId } ;}
mkuse!{use rustc_hir :: intravisit :: { InferKind , Visitor , VisitorExt } ;}
mkuse!{use rustc_hir :: { self as hir , GenericParamKind , HirId , Node , PreciseCapturingArgKind , find_attr } ;}
mkuse!{use rustc_infer :: infer :: { InferCtxt , TyCtxtInferExt } ;}
mkuse!{use rustc_infer :: traits :: { DynCompatibilityViolation , ObligationCause } ;}
mkuse!{use rustc_middle :: query :: Providers ;}
mkuse!{use rustc_middle :: ty :: util :: { Discr , IntTypeExt } ;}
mkuse!{use rustc_middle :: ty :: { self , AdtKind , Const , IsSuggestable , Ty , TyCtxt , TypeVisitableExt , TypingMode , fold_regions , } ;}
...
mkuse!{use rustc_trait_selection :: traits :: { FulfillmentError , ObligationCtxt , hir_ty_lowering_dyn_compatibility_violations , } ;}
mkuse!{use tracing :: { debug , instrument } ;}
mkuse!{use crate :: errors ;}
mkuse!{use crate :: hir_ty_lowering :: { FeedConstTy , HirTyLowerer , InherentAssocCandidate , RegionInferReason , } ;}
mkmod!{dump, { 
                getname!(dump);
                getsrc!(dump);
                getpath!(dump);
...
mkmod!{generics_of, { 
                ...
                getsrc!(generics_of);
                getpath!(generics_of);
                get_deps!(generics_of);
                get_crates!(generics_of);
                mkinclude!(generics_of);
...
mkmod!{item_bounds, { 
                ...
                getsrc!(item_bounds);
                getpath!(item_bounds);
                get_deps!(item_bounds);
                get_crates!(item_bounds);
                mkinclude!(item_bounds);
                 
            }}
mkmod!{predicates_of, { 
                getname!(predicates_of);
                getsrc!(predicates_of);
                getpath!(predicates_of);
                get_deps!(predicates_of);
                get_crates!(predicates_of);
                mkinclude!(predicates_of);
...
macro_rules! bad_placeholder_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function bad_placeholder in module {}", module_path!());
    };
}

mkfn!{
    bad_placeholder_introspect!();
...
mkitem!{mkimpl!{impl < 'tcx > ItemCtxt < 'tcx > { pub (crate) fn new (tcx : TyCtxt < 'tcx > , item_def_id : LocalDefId) -> ItemCtxt < 'tcx > { ItemCtxt { tcx , item_def_id , tainted_by_errors : Cell :: new (None) } } pub (crate) fn lower_ty (& self , hir_ty : & hir :: Ty < 'tcx >) -> Ty < 'tcx > { self . lowerer () . lower_ty (hir_ty) } pub (crate) fn hir_id (& self) -> hir :: HirId { self . tcx . local_def_id_to_hir_id (self . item_def_id) } pub (crate) fn node (& self) -> hir :: Node < 'tcx > { self . tcx . hir_node (self . hir_id ()) } fn check_tainted_by_errors (& self) -> Result < () , ErrorGuaranteed > { match self . tainted_by_errors . get () { Some (err) => Err (err) , None => Ok (()) , } } fn report_placeholder_type_error (& self , placeholder_types : Vec < Span > , infer_replacements : Vec < (Span , String) > ,) -> ErrorGuaranteed { let node = self . tcx . hir_node_by_def_id (self . item_def_id) ; let generics = node . generics () ; let kind_id = match node { Node :: GenericParam (_) | Node :: WherePredicate (_) | Node :: Field (_) => { self . tcx . local_parent (self . item_def_id) } _ => self . item_def_id , } ; let kind = self . tcx . def_descr (kind_id . into ()) ; let mut diag = placeholder_type_error_diag (self , generics , placeholder_types , infer_replacements . iter () . map (| & (span , _) | span) . collect () , false , None , kind ,) ; if ! infer_replacements . is_empty () { diag . multipart_suggestion (format ! ("try replacing `_` with the type{} in the corresponding trait method \
                        signature" , rustc_errors :: pluralize ! (infer_replacements . len ()) ,) , infer_replacements , Applicability :: MachineApplicable ,) ; } diag . emit () } }}}
mkitem!{mkimpl!{impl < 'tcx > HirTyLowerer < 'tcx > for ItemCtxt < 'tcx > { fn tcx (& self) -> TyCtxt < 'tcx > { self . tcx } fn dcx (& self) -> DiagCtxtHandle < '_ > { self . tcx . dcx () . taintable_handle (& self . tainted_by_errors) } fn item_def_id (& self) -> LocalDefId { self . item_def_id } fn re_infer (& self , span : Span , reason : RegionInferReason < '_ >) -> ty :: Region < 'tcx > { if let RegionInferReason :: ObjectLifetimeDefault = reason { let e = struct_span_code_err ! (self . dcx () , span , E0228 , "the lifetime bound for this object type cannot be deduced \
                from context; please supply an explicit bound") . emit () ; ty :: Region :: new_error (self . tcx () , e) } else { ty :: Region :: new_error_with_message (self . tcx () , span , "unelided lifetime in signature") } } fn ty_infer (& self , _ : Option < & ty :: GenericParamDef > , span : Span) -> Ty < 'tcx > { if ! self . tcx . dcx () . has_stashed_diagnostic (span , StashKey :: ItemNoType) { self . report_placeholder_type_error (vec ! [span] , vec ! []) ; } Ty :: new_error_with_message (self . tcx () , span , "bad placeholder type") } fn ct_infer (& self , _ : Option < & ty :: GenericParamDef > , span : Span) -> Const < 'tcx > { self . report_placeholder_type_error (vec ! [span] , vec ! []) ; ty :: Const :: new_error_with_message (self . tcx () , span , "bad placeholder constant") } fn register_trait_ascription_bounds (& self , _ : Vec < (ty :: Clause < 'tcx > , Span) > , _ : HirId , span : Span ,) { self . dcx () . span_delayed_bug (span , "trait ascription type not allowed here") ; } fn probe_ty_param_bounds (& self , span : Span , def_id : LocalDefId , assoc_ident : Ident ,) -> ty :: EarlyBinder < 'tcx , & 'tcx [(ty :: Clause < 'tcx > , Span)] > { self . tcx . at (span) . type_param_predicates ((self . item_def_id , def_id , assoc_ident)) } # [instrument (level = "debug" , skip (self , _span) , ret)] fn select_inherent_assoc_candidates (& self , _span : Span , self_ty : Ty < 'tcx > , candidates : Vec < InherentAssocCandidate > ,) -> (Vec < InherentAssocCandidate > , Vec < FulfillmentError < 'tcx > >) { assert ! (! self_ty . has_infer ()) ; let self_ty = self . tcx . expand_free_alias_tys (self_ty) ; debug ! ("select_inherent_assoc_candidates: self_ty={:?}" , self_ty) ; let candidates = candidates . into_iter () . filter (| & InherentAssocCandidate { impl_ , .. } | { let impl_ty = self . tcx () . type_of (impl_) . instantiate_identity () ; let impl_ty = self . tcx . expand_free_alias_tys (impl_ty) ; debug ! ("select_inherent_assoc_candidates: impl_ty={:?}" , impl_ty) ; ty :: DeepRejectCtxt :: relate_rigid_infer (self . tcx) . types_may_unify_with_depth (self_ty , impl_ty , usize :: MAX ,) }) . collect () ; (candidates , vec ! []) } fn lower_assoc_item_path (& self , span : Span , item_def_id : DefId , item_segment : & rustc_hir :: PathSegment < 'tcx > , poly_trait_ref : ty :: PolyTraitRef < 'tcx > ,) -> Result < (DefId , ty :: GenericArgsRef < 'tcx >) , ErrorGuaranteed > { if let Some (trait_ref) = poly_trait_ref . no_bound_vars () { let item_args = self . lowerer () . lower_generic_args_of_assoc_item (span , item_def_id , item_segment , trait_ref . args ,) ; Ok ((item_def_id , item_args)) } else { let (mut mpart_sugg , mut inferred_sugg) = (None , None) ; let mut bound = String :: new () ; match self . node () { hir :: Node :: Field (_) | hir :: Node :: Ctor (_) | hir :: Node :: Variant (_) => { let item = self . tcx . hir_expect_item (self . tcx . hir_get_parent_item (self . hir_id ()) . def_id) ; match & item . kind { hir :: ItemKind :: Enum (_ , generics , _) | hir :: ItemKind :: Struct (_ , generics , _) | hir :: ItemKind :: Union (_ , generics , _) => { let lt_name = get_new_lifetime_name (self . tcx , poly_trait_ref , generics) ; let (lt_sp , sugg) = match generics . params { [] => (generics . span , format ! ("<{lt_name}>")) , [bound , ..] => (bound . span . shrink_to_lo () , format ! ("{lt_name}, ")) , } ; mpart_sugg = Some (errors :: AssociatedItemTraitUninferredGenericParamsMultipartSuggestion { fspan : lt_sp , first : sugg , sspan : span . with_hi (item_segment . ident . span . lo ()) , second : format ! ("{}::" , self . tcx . instantiate_bound_regions_uncached (poly_trait_ref , | _ | { ty :: Region :: new_early_param (self . tcx , ty :: EarlyParamRegion { index : 0 , name : Symbol :: intern (& lt_name) , }) }) ,) , }) ; } _ => { } } } hir :: Node :: Item (hir :: Item { kind : hir :: ItemKind :: Struct (..) | hir :: ItemKind :: Enum (..) | hir :: ItemKind :: Union (..) , .. }) => { } hir :: Node :: Item (_) | hir :: Node :: ForeignItem (_) | hir :: Node :: TraitItem (_) | hir :: Node :: ImplItem (_) => { inferred_sugg = Some (span . with_hi (item_segment . ident . span . lo ())) ; bound = format ! ("{}::" , self . tcx . anonymize_bound_vars (poly_trait_ref) . skip_binder () ,) ; } _ => { } } Err (self . tcx () . dcx () . emit_err (errors :: AssociatedItemTraitUninferredGenericParams { span , inferred_sugg , bound , mpart_sugg , what : self . tcx . def_descr (item_def_id) , })) } } fn probe_adt (& self , _span : Span , ty : Ty < 'tcx >) -> Option < ty :: AdtDef < 'tcx > > { ty . ty_adt_def () } fn record_ty (& self , _hir_id : hir :: HirId , _ty : Ty < 'tcx > , _span : Span) { } fn infcx (& self) -> Option < & InferCtxt < 'tcx > > { None } fn lower_fn_sig (& self , decl : & hir :: FnDecl < 'tcx > , _generics : Option < & hir :: Generics < '_ > > , hir_id : rustc_hir :: HirId , _hir_ty : Option < & hir :: Ty < '_ > > ,) -> (Vec < Ty < 'tcx > > , Ty < 'tcx >) { let tcx = self . tcx () ; let mut infer_replacements = vec ! [] ; let input_tys = decl . inputs . iter () . enumerate () . map (| (i , a) | { if let hir :: TyKind :: Infer (()) = a . kind && let Some (suggested_ty) = self . lowerer () . suggest_trait_fn_ty_for_impl_fn_infer (hir_id , Some (i)) { infer_replacements . push ((a . span , suggested_ty . to_string ())) ; return Ty :: new_error_with_message (tcx , a . span , suggested_ty . to_string ()) ; } self . lowerer () . lower_ty (a) }) . collect () ; let output_ty = match decl . output { hir :: FnRetTy :: Return (output) => { if let hir :: TyKind :: Infer (()) = output . kind && let Some (suggested_ty) = self . lowerer () . suggest_trait_fn_ty_for_impl_fn_infer (hir_id , None) { infer_replacements . push ((output . span , suggested_ty . to_string ())) ; Ty :: new_error_with_message (tcx , output . span , suggested_ty . to_string ()) } else { self . lower_ty (output) } } hir :: FnRetTy :: DefaultReturn (..) => tcx . types . unit , } ; if ! infer_replacements . is_empty () { self . report_placeholder_type_error (vec ! [] , infer_replacements) ; } (input_tys , output_ty) } fn dyn_compatibility_violations (& self , trait_def_id : DefId) -> Vec < DynCompatibilityViolation > { hir_ty_lowering_dyn_compatibility_violations (self . tcx , trait_def_id) } }}}

macro_rules! get_new_lifetime_name_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function get_new_lifetime_name in module {}", module_path!());
    };
}

mkfn!{
    get_new_lifetime_name_introspect!();
...
macro_rules! lower_variant_ctor_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function lower_variant_ctor in module {}", module_path!());
    };
}

mkfn!{
    lower_variant_ctor_introspect!();
...
macro_rules! lower_enum_variant_types_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function lower_enum_variant_types in module {}", module_path!());
    };
}

mkfn!{
    lower_enum_variant_types_introspect!();
...
macro_rules! adt_def_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function adt_def in module {}", module_path!());
    };
}
...
macro_rules! trait_def_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function trait_def in module {}", module_path!());
    };
}

mkfn!{
    trait_def_introspect!();
...
macro_rules! fn_sig_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function fn_sig in module {}", module_path!());
    };
}
...
macro_rules! lower_fn_sig_recovering_infer_ret_ty_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function lower_fn_sig_recovering_infer_ret_ty in module {}", module_path!());
    };
}

mkfn!{
    lower_fn_sig_recovering_infer_ret_ty_introspect!();
...
macro_rules! recover_infer_ret_ty_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function recover_infer_ret_ty in module {}", module_path!());
    };
}

mkfn!{
    recover_infer_ret_ty_introspect!();
...
macro_rules! suggest_impl_trait_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function suggest_impl_trait in module {}", module_path!());
    };
}

mkfn!{
    suggest_impl_trait_introspect!();
...
macro_rules! impl_trait_header_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function impl_trait_header in module {}", module_path!());
    };
}

mkfn!{
    impl_trait_header_introspect!();
...
macro_rules! check_impl_constness_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function check_impl_constness in module {}", module_path!());
    };
}

mkfn!{
    check_impl_constness_introspect!();
...
macro_rules! polarity_of_impl_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function polarity_of_impl in module {}", module_path!());
    };
}

mkfn!{
    polarity_of_impl_introspect!();
...
macro_rules! early_bound_lifetimes_from_generics_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function early_bound_lifetimes_from_generics in module {}", module_path!());
    };
}

mkfn!{
    early_bound_lifetimes_from_generics_introspect!();
...
macro_rules! compute_sig_of_foreign_fn_decl_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function compute_sig_of_foreign_fn_decl in module {}", module_path!());
    };
}

mkfn!{
    compute_sig_of_foreign_fn_decl_introspect!();
...
macro_rules! coroutine_kind_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function coroutine_kind in module {}", module_path!());
    };
}

mkfn!{
    coroutine_kind_introspect!();
...
macro_rules! coroutine_for_closure_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function coroutine_for_closure in module {}", module_path!());
    };
}

mkfn!{
    coroutine_for_closure_introspect!();
...
macro_rules! opaque_ty_origin_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function opaque_ty_origin in module {}", module_path!());
    };
}

mkfn!{
    opaque_ty_origin_introspect!();
...
macro_rules! rendered_precise_capturing_args_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function rendered_precise_capturing_args in module {}", module_path!());
    };
}

mkfn!{
    rendered_precise_capturing_args_introspect!();
...
macro_rules! const_param_default_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function const_param_default in module {}", module_path!());
    };
}

mkfn!{
    const_param_default_introspect!();
...
macro_rules! anon_const_kind_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function anon_const_kind in module {}", module_path!());
    };
}

mkfn!{
    anon_const_kind_introspect!();
