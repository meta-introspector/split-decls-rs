...
mkuse!{use rustc_ast :: { ExprKind , LitIntType , LitKind , StrStyle , UintTy , token } ;}
...
mkuse!{use crate :: util :: get_exprs_from_tts ;}

macro_rules! invalid_type_err_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function invalid_type_err in module {}", module_path!());
    };
}

mkfn!{
    invalid_type_err_introspect!();
    # [doc = " Emits errors for literal expressions that are invalid inside and outside of an array."] fn invalid_type_err (cx : & ExtCtxt < '_ > , token_lit : token :: Lit , span : Span , is_nested : bool ,) -> ErrorGuaranteed { use errors :: { ConcatBytesInvalid , ConcatBytesInvalidSuggestion , ConcatBytesNonU8 , ConcatBytesOob , } ; let snippet = cx . sess . source_map () . span_to_snippet (span) . ok () ; let dcx = cx . dcx () ; match LitKind :: from_token_lit (token_lit) { Ok (LitKind :: CStr (_ , style)) => { let sugg = if let Some (mut as_bstr) = snippet && style == StrStyle :: Cooked && as_bstr . starts_with ('c') && as_bstr . ends_with ('"') { as_bstr . replace_range (0 .. 1 , "b") ; as_bstr . pop () ; as_bstr . push_str (r#"\0""#) ; Some (ConcatBytesInvalidSuggestion :: CStrLit { span , as_bstr }) } else { None } ; dcx . emit_err (ConcatBytesInvalid { span , lit_kind : "C string" , sugg , cs_note : Some (()) }) } Ok (LitKind :: Char (_)) => { let sugg = snippet . map (| snippet | ConcatBytesInvalidSuggestion :: CharLit { span , snippet }) ; dcx . emit_err (ConcatBytesInvalid { span , lit_kind : "character" , sugg , cs_note : None }) } Ok (LitKind :: Str (_ , _)) => { let sugg = if ! is_nested { snippet . map (| snippet | ConcatBytesInvalidSuggestion :: StrLit { span , snippet }) } else { None } ; dcx . emit_err (ConcatBytesInvalid { span , lit_kind : "string" , sugg , cs_note : None }) } Ok (LitKind :: Float (_ , _)) => { dcx . emit_err (ConcatBytesInvalid { span , lit_kind : "float" , sugg : None , cs_note : None }) } Ok (LitKind :: Bool (_)) => dcx . emit_err (ConcatBytesInvalid { span , lit_kind : "boolean" , sugg : None , cs_note : None , }) , Ok (LitKind :: Int (_ , _)) if ! is_nested => { let sugg = snippet . map (| snippet | ConcatBytesInvalidSuggestion :: IntLit { span , snippet }) ; dcx . emit_err (ConcatBytesInvalid { span , lit_kind : "numeric" , sugg , cs_note : None }) } Ok (LitKind :: Int (val , LitIntType :: Unsuffixed | LitIntType :: Unsigned (UintTy :: U8))) => { assert ! (val . get () > u8 :: MAX . into ()) ; dcx . emit_err (ConcatBytesOob { span }) } Ok (LitKind :: Int (_ , _)) => dcx . emit_err (ConcatBytesNonU8 { span }) , Ok (LitKind :: ByteStr (..) | LitKind :: Byte (_)) => unreachable ! () , Ok (LitKind :: Err (guar)) => guar , Err (err) => report_lit_error (& cx . sess . psess , err , token_lit , span) , } }
}

macro_rules! handle_array_element_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function handle_array_element in module {}", module_path!());
    };
}

mkfn!{
    handle_array_element_introspect!();
...
macro_rules! expand_concat_bytes_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_concat_bytes in module {}", module_path!());
    };
}

mkfn!{
    expand_concat_bytes_introspect!();
