...
mkuse!{use rustc_ast :: { MetaItem , Mutability } ;}
...
macro_rules! expand_deriving_hash_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_deriving_hash in module {}", module_path!());
    };
}

mkfn!{
    expand_deriving_hash_introspect!();
...
macro_rules! hash_substructure_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function hash_substructure in module {}", module_path!());
    };
}

mkfn!{
    hash_substructure_introspect!();
