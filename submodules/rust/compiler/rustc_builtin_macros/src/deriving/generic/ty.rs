...
mkitem!{mkstruct!{# [doc = " Bounds on type parameters."] # [derive (Clone)] pub (crate) struct Bounds { pub bounds : Vec < (Symbol , Vec < Path >) > , }}}
mkitem!{mkimpl!{impl Bounds { pub (crate) fn empty () -> Bounds { Bounds { bounds : Vec :: new () } } pub (crate) fn to_generics (& self , cx : & ExtCtxt < '_ > , span : Span , self_ty : Ident , self_generics : & Generics ,) -> Generics { let params = self . bounds . iter () . map (| & (name , ref bounds) | mk_ty_param (cx , span , name , bounds , self_ty , self_generics)) . collect () ; Generics { params , where_clause : ast :: WhereClause { has_where_token : false , predicates : ThinVec :: new () , span , } , span , } } }}}

macro_rules! get_explicit_self_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function get_explicit_self in module {}", module_path!());
    };
}

mkfn!{
    get_explicit_self_introspect!();
