...
mkuse!{use rustc_ast :: { join_path_idents , token } ;}
mkuse!{use rustc_ast_pretty :: pprust ;}
mkuse!{use rustc_expand :: base :: { DummyResult , ExpandResult , ExtCtxt , MacEager , MacResult , MacroExpanderResult , resolve_path , } ;}
mkuse!{use rustc_expand :: module :: DirOwnership ;}
mkuse!{use rustc_lint_defs :: BuiltinLintDiag ;}
mkuse!{use rustc_parse :: lexer :: StripTokens ;}
mkuse!{use rustc_parse :: parser :: ForceCollect ;}
mkuse!{use rustc_parse :: { new_parser_from_file , unwrap_or_emit_fatal , utf8_error } ;}
mkuse!{use rustc_session :: lint :: builtin :: INCOMPLETE_INCLUDE ;}
mkuse!{use rustc_session :: parse :: ParseSess ;}
mkuse!{use rustc_span :: source_map :: SourceMap ;}
mkuse!{use rustc_span :: { ByteSymbol , Pos , Span , Symbol } ;}
mkuse!{use smallvec :: SmallVec ;}
mkuse!{use crate :: errors ;}
mkuse!{use crate :: util :: { check_zero_tts , get_single_str_from_tts , get_single_str_spanned_from_tts , parse_expr , } ;}

macro_rules! expand_line_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_line in module {}", module_path!());
    };
}

mkfn!{
    expand_line_introspect!();
...
macro_rules! expand_column_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_column in module {}", module_path!());
    };
}

mkfn!{
    expand_column_introspect!();
...
macro_rules! expand_file_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_file in module {}", module_path!());
    };
}

mkfn!{
    expand_file_introspect!();
...
macro_rules! expand_stringify_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_stringify in module {}", module_path!());
    };
}

mkfn!{
    expand_stringify_introspect!();
...
macro_rules! expand_mod_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_mod in module {}", module_path!());
    };
}

mkfn!{
    expand_mod_introspect!();
...
macro_rules! expand_include_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_include in module {}", module_path!());
    };
}

mkfn!{
    expand_include_introspect!();
...
macro_rules! expand_include_str_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_include_str in module {}", module_path!());
    };
}

mkfn!{
    expand_include_str_introspect!();
...
macro_rules! expand_include_bytes_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function expand_include_bytes in module {}", module_path!());
    };
}

mkfn!{
    expand_include_bytes_introspect!();
...
macro_rules! load_binary_file_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function load_binary_file in module {}", module_path!());
    };
}

mkfn!{
    load_binary_file_introspect!();
...
macro_rules! find_path_suggestion_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function find_path_suggestion in module {}", module_path!());
    };
}

mkfn!{
    find_path_suggestion_introspect!();
