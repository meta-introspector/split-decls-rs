...
mkuse!{use std :: io :: { self , Read } ;}
mkuse!{use std :: path :: { Path , PathBuf } ;}
mkuse!{use std :: { env , fs } ;}
mkuse!{use rustc_data_structures :: memmap :: Mmap ;}
mkuse!{use rustc_serialize :: Encoder ;}
mkuse!{use rustc_serialize :: opaque :: { FileEncodeResult , FileEncoder } ;}
mkuse!{use rustc_session :: Session ;}
mkuse!{use tracing :: debug ;}
mkuse!{use crate :: errors ;}
mkitem!{# [doc = " The first few bytes of files generated by incremental compilation."] const FILE_MAGIC : & [u8] = b"RSIC" ;}
mkitem!{# [doc = " Change this if the header format changes."] const HEADER_FORMAT_VERSION : u16 = 0 ;}

macro_rules! write_file_header_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function write_file_header in module {}", module_path!());
    };
}

mkfn!{
    write_file_header_introspect!();
...
macro_rules! save_in_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function save_in in module {}", module_path!());
    };
}
...
macro_rules! report_format_mismatch_introspect {
    () => {
        emit_message!("📊 INTROSPECT: Function report_format_mismatch in module {}", module_path!());
    };
}

mkfn!{
    report_format_mismatch_introspect!();
