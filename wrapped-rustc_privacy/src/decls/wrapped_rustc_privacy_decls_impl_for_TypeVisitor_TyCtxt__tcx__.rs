    ...
    fn visit_ty(&mut self, ty: Ty<'tcx>) -> Self::Result {
        ...
        match ty_kind {
            ...
            ty::Alias(kind @ (ty::Inherent | ty::Free | ty::Projection), data) => {
                ...
                try_visit!(
                    self.def_id_visitor.visit_def_id(data.def_id, match kind {
                    ty::Inherent | ty::Projection => "associated type", ty::Free =>
                    "type alias", ty::Opaque => unreachable!(), }, & LazyDefPathStr {
                    def_id : data.def_id, tcx },)
                );
                return if V::SHALLOW {
                    V::Result::output()
                    ...
                    V::Result::from_branch(
                        data
                            .args
                            .iter()
                            .try_for_each(|arg| arg.visit_with(self).branch()),
                    )
                };
            }
            ...
            ty::Alias(ty::Opaque, ty::AliasTy { def_id, .. }) => {
                if self.visited_tys.insert(ty) {
                    try_visit!(
                        self.visit_clauses(tcx.explicit_item_bounds(def_id)
                        .skip_binder())
                    );
                }
            }
            ...
            | ty::Error(_)
            | ty::CoroutineWitness(..) => {}
            ty::Placeholder(..) | ty::Infer(..) => bug!("unexpected type: {:?}", ty),
        }
