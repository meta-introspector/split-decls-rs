...
macro_rules! Depcrate_parse_ignoreshould {
() => {
// Module: crate::parse::ignore
// Provides: {"should"}
// Dependencies: {}
# [cfg (test)] mod should { use super :: * ; use crate :: test :: { assert_eq , * } ; use rstest_test :: assert_in ; # [rstest] # [case ("fn simple(a: u32) {}")] # [case ("fn more(a: u32, b: &str) {}")] # [case ("fn gen<S: AsRef<str>>(a: u32, b: S) {}")] # [case ("fn attr(#[case] a: u32, #[values(1,2)] b: i32) {}")] fn not_change_anything_if_no_ignore_attribute_found (# [case] item_fn : & str) { let mut item_fn : ItemFn = item_fn . ast () ; let orig = item_fn . clone () ; let by_refs = extract_ignores (& mut item_fn) . unwrap () ; assert_eq ! (orig , item_fn) ; assert ! (by_refs . is_empty ()) ; } # [rstest] # [case :: simple ("fn f(#[ignore] a: u32) {}" , "fn f(a: u32) {}" , & ["a"])] # [case :: more_than_one ("fn f(#[ignore] a: u32, #[ignore] b: String, #[ignore] c: std::collection::HashMap<usize, String>) {}" , r#"fn f(a: u32, 
                b: String, 
