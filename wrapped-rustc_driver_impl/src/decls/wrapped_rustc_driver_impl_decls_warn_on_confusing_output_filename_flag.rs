...
fn warn_on_confusing_output_filename_flag(
    ...
    if let Some(name) = matches.opt_str("o")
        && let Some(suspect) = args
            .iter()
            .find(|arg| arg.starts_with("-o") && *arg != "-o")
    {
        ...
        if optgroups
            .iter()
            .any(|option| eq_ignore_separators(option.long_name(), filename))
            || config::CG_OPTIONS
                .iter()
                .any(|option| eq_ignore_separators(option.name(), filename))
