...
impl<'a> State<'a> {
    ...
    #[doc = " Prints an expr using syntax that's acceptable in a condition position, such as the `cond` in"]
    #[doc = " `if cond { ... }`."]
    fn print_expr_as_cond(&mut self, expr: &hir::Expr<'_>) {
        self.print_expr_cond_paren(expr, Self::cond_needs_par(expr))
    }
    #[doc = " Prints `expr` or `(expr)` when `needs_par` holds."]
    ...
    #[doc = " Print a `let pat = expr` expression."]
