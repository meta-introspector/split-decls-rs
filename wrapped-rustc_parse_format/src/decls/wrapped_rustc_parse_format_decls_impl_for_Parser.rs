...
impl<'input> Parser<'input> {
    ...
    pub fn new(
        ...
        let input_vec: Vec<(Range<usize>, usize, char)> = if pre_input_vec.is_empty() {
            ...
            input
                ...
                .zip(pre_input_vec)
                .map(|((i, c), (r, _))| (
                    r.start + quote_offset..r.end + quote_offset,
                    i,
                    c,
                ))
                .collect()
    ...
    fn consume_pos(&mut self, ch: char) -> Option<(Range<usize>, usize)> {
        if let Some((r, i, c)) = self.peek() && ch == c {
    ...
    fn ws(&mut self) {
        ...
        let step = rest
            .iter()
            .position(|&(_, _, c)| !c.is_whitespace())
            .unwrap_or(rest.len());
    ...
    fn string(&mut self, start: usize) -> &'input str {
        while let Some((r, i, c)) = self.peek() {
            match c {
                ...
                _ => {
                    self.input_vec_index += 1;
                    if self.is_source_literal && r.start == self.cur_line_start
                        && c.is_whitespace()
                    {
                        self.cur_line_start = r.end;
                    }
    ...
    fn argument(&mut self) -> Argument<'input> {
        ...
        let position_span = self
            .input_vec_index2range(start_idx)
            .start..self.input_vec_index2range(end_idx).start;
        Argument {
            position,
            position_span,
            format,
            ...
            match self.peek() {
                Some((range, _, c)) if rustc_lexer::is_id_start(c) => {
                            ...
                            .is_some_and(|(_, _, c)| rustc_lexer::is_id_start(c))
                    {
                        ...
                        let full_span = start..self
                            .input_vec_index2range(self.input_vec_index)
                            .start;
                        self.errors
                            .insert(
                                0,
    ...
    fn input_vec_index2pos(&self, index: usize) -> usize {
        if let Some((_, pos, _)) = self.input_vec.get(index) {
            *pos
        } else {
            self.input.len()
        }
    ...
    fn format(&mut self) -> FormatSpec<'input> {
        ...
        if let Some((range, _)) = self.consume_pos('.') {
            ...
            spec.precision_span = Some(
                range.start..self.input_vec_index2range(self.input_vec_index).start,
            );
        }
        let start_idx = self.input_vec_index;
        if self.consume('x') {
        ...
        if overflow {
            ...
            self.errors
                .push(ParseError {
                    ...
                    span: self
                        .input_vec_index2range(start_index)
                        .start..self.input_vec_index2range(self.input_vec_index).end,
                    secondary_label: None,
                    suggestion: Suggestion::None,
                });
        }
        ...
        if let (Some((range, _)), Some(_)) = (
            ...
            self.errors
                .insert(
                    0,
                    ParseError {
                        description: "expected format parameter to occur after `:`"
                            .to_owned(),
                        note: Some(
                            format!(
                                "`?` comes after `:`, try `{}:{}` instead", word, "?"
                            ),
                        ),
        ...
        if let Some((range, _)) = self.consume_pos(alignment) {
            self.errors
                .insert(
                    0,
                    ParseError {
                        description: "expected alignment specifier after `:` in format string; example: `{:>?}`"
                            .to_owned(),
                        note: None,
                    ...
                    ArgumentIs(_) => {
                        self.errors
                            .insert(
                                0,
                                ParseError {
                                    description: "tuple index access isn't supported"
                                        .to_string(),
                                    note: None,
                                    label: "not supported".to_string(),
                                    span: arg.position_span.start..field.position_span.end,
