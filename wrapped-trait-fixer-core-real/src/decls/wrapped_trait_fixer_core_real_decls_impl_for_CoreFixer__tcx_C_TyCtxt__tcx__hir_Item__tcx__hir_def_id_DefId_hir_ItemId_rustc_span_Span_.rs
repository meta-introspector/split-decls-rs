    ...
    C: ConfigTrait,
{
    ...
    fn process_hir(&mut self) {
        RustcQueryContextTyCtxt(self.tcx)
            .walk_hir_tops(|item| {
        ...
        for rule in self.config.get_rules() {
            match &rule.kind {
                RuleKind::AddDerive => {
                    ...
                    | ItemKind::Enum(..)
                    | ItemKind::Union(..) = real_hir_info.get_item_kind()
                    {
                        if rule
                            .apply_to
                            .contains(
                                &format!("{:?}", real_hir_info.get_item_kind())
                                    .to_lowercase(),
                            )
                        {
                            if rule.condition == "always" {
                                let trait_name = rule.trait_name[0].clone();
                                if !RustcAttributeReaderTyCtxt(self.tcx)
                                    .has_derive_attr(
                ...
                RuleKind::AddImpl if rule.trait_name[0] == "Clone" => {
                    if let ItemKind::Struct(..) = real_hir_info.get_item_kind() {
                        if rule.condition == "all_fields_clone" {
                            ...
                            let adt_ty = self
                                .tcx
                                .type_of(real_hir_info.get_owner_id())
                                .instantiate(
                                    self.tcx,
                            ...
                            let trait_def_id = RustcLangItemsTyCtxt(self.tcx)
                                .get_clone_trait_def_id();
                            if let Some(trait_def_id) = trait_def_id {
                                if RustcTraitCheckerTyCtxt(self.tcx)
