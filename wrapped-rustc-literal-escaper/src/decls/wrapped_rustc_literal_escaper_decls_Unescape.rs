...
#[doc = " Trait for unescaping escape sequences in strings"]
trait Unescape {
    #[doc = " Unit type of the implementing string type (`char` for string, `u8` for byte string)"]
    type Unit;
    #[doc = " Result of unescaping the zero char ('\\0')"]
    const ZERO_RESULT: Result<Self::Unit, EscapeError>;
    #[doc = " Converts non-zero bytes to the unit type"]
    fn nonzero_byte2unit(b: NonZero<u8>) -> Self::Unit;
    #[doc = " Converts chars to the unit type"]
    fn char2unit(c: char) -> Result<Self::Unit, EscapeError>;
    #[doc = " Converts the byte of a hex escape to the unit type"]
    fn hex2unit(b: u8) -> Result<Self::Unit, EscapeError>;
    #[doc = " Converts the result of a unicode escape to the unit type"]
    fn unicode2unit(r: Result<char, EscapeError>) -> Result<Self::Unit, EscapeError>;
    #[doc = " Unescape a single unit (single quote syntax)"]
    ...
    #[doc = " Unescape the first unit of a string (double quoted syntax)"]
    ...
    #[doc = ""]
    #[doc = " Takes the contents of a raw string literal (without quotes)"]
    #[doc = " and produces a sequence of `Result<Self::Unit, EscapeError>`"]
    #[doc = " which are returned via `callback`."]
    fn unescape(
        src: &str,
