...
impl Hasher {
    #[doc = " Create a new `Hasher`."]
    #[doc = ""]
    #[doc = " This will perform a CPU feature detection at runtime to select the most"]
    #[doc = " optimal implementation for the current processor architecture."]
    pub fn new() -> Self {
        Self::new_with_initial(DEFAULT_INIT_STATE)
    }
    #[doc = " Create a new `Hasher` with an initial CRC32 state."]
    #[doc = ""]
    #[doc = " This works just like `Hasher::new`, except that it allows for an initial"]
    #[doc = " CRC32 state to be passed in."]
    pub fn new_with_initial(init: u32) -> Self {
        Self::new_with_initial_len(init, 0)
    }
    #[doc = " Create a new `Hasher` with an initial CRC32 state."]
    #[doc = ""]
    #[doc = " As `new_with_initial`, but also accepts a length (in bytes). The"]
    #[doc = " resulting object can then be used with `combine` to compute `crc(a ||"]
    #[doc = " b)` from `crc(a)`, `crc(b)`, and `len(b)`."]
    ...
    #[doc = " Process the given byte slice and update the hash state."]
    ...
    #[doc = " Finalize the hash state and return the computed CRC32 value."]
    ...
    #[doc = " Combine the hash state with the hash state for the subsequent block of bytes."]
