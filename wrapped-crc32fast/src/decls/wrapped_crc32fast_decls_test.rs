...
mod test {
    ...
    quickcheck::quickcheck! {
        ...
        bool { let mut hash_a = Hasher::new(); hash_a.update(& bytes_1); let mut hash_b =
        Hasher::new(); hash_b.update(& bytes_2); let mut hash_ab = Hasher::new(); hash_ab
        .update(& bytes_1); hash_ab.update(& bytes_2); let ab = hash_ab.finalize();
        hash_a.combine(& hash_b); hash_a.finalize() == ab }
    }
}
