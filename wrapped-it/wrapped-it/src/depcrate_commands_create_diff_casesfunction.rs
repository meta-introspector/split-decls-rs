...
"#)) ; } let script_file = destination_dir . join (script_name) ; eprintln ! ("{prefix} write script file at '{script_file}'" , script_file = script_file . display ()) ; if ! dry_run { let script = blocks . join ("\n") ; std :: fs :: write (script_file , script) ? ; } Ok (()) } }
};
}
