...
#[doc = " Private helper methods for `Splice::drop`"]
impl<T> Drain<'_, T> {
    #[doc = " The range from `self.vec.len` to `self.tail_start` contains elements"]
    #[doc = " that have been moved out."]
    #[doc = " Fill that range as much as possible with new elements from the `replace_with` iterator."]
    #[doc = " Returns `true` if we filled the entire range. (`replace_with.next()` didn’t return `None`.)"]
