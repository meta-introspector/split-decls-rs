...
impl<'db> MatchFinder<'db> {
    ...
    pub fn in_context(
        ...
        let file_id = sema
            ...
            .unwrap_or_else(|| EditionedFileId::current_edition(
                db,
                lookup_context.file_id,
            ));
        let resolution_scope = resolving::ResolutionScope::new(
                &sema,
                hir::FilePosition {
                    file_id,
                    offset: lookup_context.offset,
                },
            )
    ...
    pub fn debug_where_text_equal(
        ...
        let file_text = self
            .sema
            .db
            .file_text(file_id.file_id(self.sema.db))
            .text(self.sema.db);
    ...
    fn output_debug_for_nodes_at_range(
        ...
        for node in node.children() {
            ...
            if node_range.file_id != range.file_id
                || !node_range.range.contains_range(range.range)
            {
                continue;
            }
