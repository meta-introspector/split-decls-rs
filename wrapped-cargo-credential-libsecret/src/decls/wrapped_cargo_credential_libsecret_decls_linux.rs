...
mod linux {
    ...
    impl LibSecretCredential {
        pub fn new() -> Result<LibSecretCredential, Error> {
            let libsecret = unsafe { Library::new("libsecret-1.so.0") }
                .context(
        ...
        fn perform(
            ...
            match action {
                cargo_credential::Action::Get(_) => {
                    unsafe {
                        ...
                        if !error.is_null() {
                            return Err(
                                format!(
                                    "failed to get token: {}", CStr::from_ptr((* error).message)
                ...
                cargo_credential::Action::Login(options) => {
                    ...
                    let token = CString::new(read_token(options, registry)?.expose())
                        .unwrap();
                    unsafe {
                        ...
                        if !error.is_null() {
                            return Err(
                                format!(
                                    "failed to store token: {}", CStr::from_ptr((* error)
                                    .message).to_str().unwrap_or_default()
                                )
                                    .into(),
                            );
                        }
                ...
                cargo_credential::Action::Logout => {
                    unsafe {
                        ...
                        if !error.is_null() {
                            return Err(
                                format!(
                                    "failed to erase token: {}", CStr::from_ptr((* error)
                                    .message).to_str().unwrap_or_default()
                                )
                                    .into(),
                            );
