...
#[doc = " Obtain just the main MIR (no promoteds) and run some cleanups on it. This also runs"]
#[doc = " mir borrowck *before* doing so in order to ensure that borrowck can be run and doesn't"]
#[doc = " end up missing the source MIR due to stealing happening."]
