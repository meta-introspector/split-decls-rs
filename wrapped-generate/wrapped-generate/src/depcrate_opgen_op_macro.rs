...
" , ops . iter () . map (| op | format ! ("`{}`" , op . token)) . collect ::< Vec < _ >> () . join (", "))) ; for op in ops . iter () { result . push_str (& format ! ("---\nOperator `{token}`. Expands to `{operator}`.
...
# }}
```\n
" , token = op . token , operator = op . operator , ex0 = op . example . 0 , ex1 = op . example . 1)) ; } result . push_str (& format ! ("*/
...
")) ; for fun in ops . iter () . filter (| f | f . op_type == Function) { result . push_str (& format ! ("
(@stack[$($stack:ident,)*] @queue[$($queue:ident,)*] @tail: {f_token} $($tail:tt)*) => (
    __op_internal__!(@stack[{f_op}, $($stack,)*] @queue[$($queue,)*] @tail: $($tail)*)
);" , f_token = fun . token , f_op = fun . operator)) ; } result . push_str ("
...
);" ,) ; for o1 in ops . iter () . filter (| op | op . op_type == Operator) { for o2 in ops . iter () . filter (| op | op . op_type == Operator) . filter (| o2 | o1 . precedence <= o2 . precedence) { result . push_str (& format ! ("
...
);" , fun_op = fun . operator)) ; } result . push_str ("
