// Generated macro for emit_impls (function)
macro_rules! Depcrate_generic_const_mappingsemit_impls {
() => {
// Module: crate::generic_const_mappings
// Provides: {"emit_impls"}
// Dependencies: {}
pub fn emit_impls () -> String { let mut result = String :: new () ; result . push_str ("
...
") ; for uint in uints () { result . push_str (& format ! ("
    {cfg}
    impl ToUInt for Const<{uint}> {{
        type Output = U{uint};
    }}
\
            " , uint = uint , cfg = feature_gate_to_pointer_width (uint) ,)) ; } result }
};
}
