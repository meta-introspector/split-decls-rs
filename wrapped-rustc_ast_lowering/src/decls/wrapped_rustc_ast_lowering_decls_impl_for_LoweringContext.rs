...
impl<'a, 'hir> LoweringContext<'a, 'hir> {
    fn create_def(
        ...
        assert!(
            ...
            node_id, def_kind, self.tcx.hir_def_key(self.local_def_id(node_id)),
        );
        let def_id = self
            .tcx
            .at(span)
    ...
    fn with_hir_id_owner(
        ...
        let current_ident_and_label_to_local_id = std::mem::take(
            &mut self.ident_and_label_to_local_id,
        );
        ...
        let current_local_counter = std::mem::replace(
            &mut self.item_local_id_counter,
    ...
    fn make_owner_info(
        ...
        let rustc_middle::hir::Hashes {
            opt_hash_including_bodies,
            attrs_hash,
            delayed_lints_hash,
        } = self
            .tcx
            .hash_owner_nodes(node, &bodies, &attrs, &delayed_lints, define_opaque);
        ...
        let attrs = hir::AttributeMap {
            map: attrs,
            opt_hash: attrs_hash,
            define_opaque,
        };
        let delayed_lints = hir::lints::DelayedLints {
            lints: delayed_lints,
            opt_hash: delayed_lints_hash,
        };
        self.arena
            .alloc(hir::OwnerInfo {
                nodes,
    ...
    fn lower_res(&mut self, res: Res<NodeId>) -> Res {
        let res: Result<Res, ()> = res
            .apply_id(|id| {
                let owner = self.current_hir_id_owner;
                let local_id = self
                    .ident_and_label_to_local_id
                    .get(&id)
                    .copied()
                    .ok_or(())?;
    ...
    fn lifetime_res_to_generic_param(
        ...
        let (name, kind) = match res {
            ...
            res => {
                panic!(
                    "Unexpected lifetime resolution {:?} for {:?} at {:?}", res, ident,
                    ident.span
                )
            }
        };
    ...
    fn lower_attrs(
        ...
        if attrs.is_empty() {
            ...
            let lowered_attrs = self
                .lower_attrs_vec(attrs, self.lower_span(target_span), id, target);
        ...
        let kind = match &constraint.kind {
            ...
            AssocItemConstraintKind::Bound { bounds } => {
                if self.is_in_dyn_type {
                    ...
                    let guar = self
                        .dcx()
                        .emit_err(errors::MisplacedAssocTyBinding {
                            span: constraint.span,
                            suggestion,
                        });
                    let err_ty = &*self
                        .arena
                        .alloc(self.ty(constraint.span, hir::TyKind::Err(guar)));
                    hir::AssocItemConstraintKind::Equality {
                        term: err_ty.into(),
                    }
                } else {
                    ...
                    hir::AssocItemConstraintKind::Bound {
                        bounds,
                    }
                }
            }
    ...
    fn emit_bad_parenthesized_trait_in_assoc_ty(&self, data: &ParenthesizedArgs) {
        let sub = if data.inputs.is_empty() {
            let parentheses_span = data
                .inputs_span
                .shrink_to_lo()
                .to(data.inputs_span.shrink_to_hi());
            AssocTyParenthesesSub::Empty {
                parentheses_span,
            }
        } else {
            let open_param = data
                .inputs_span
                .shrink_to_lo()
                .to(data.inputs.first().unwrap().span.shrink_to_lo());
    ...
    fn lower_generic_arg(
        ...
        match arg {
                    ...
                    TyKind::Path(None, path) => {
                            ...
                            .and_then(|partial_res| partial_res.full_res())
                        {
                                ...
                                && path.is_potential_trivial_const_arg(false)
                            {
                                ...
                                let ct = self
                                    .lower_const_path_to_const_arg(path, res, ty.id, ty.span);
            ...
            ast::GenericArg::Const(ct) => {
                GenericArg::Const(
                    self.lower_anon_const_to_const_arg(ct).try_as_ambig_ct().unwrap(),
                )
            }
        }
    }
    ...
    fn lower_path_ty(
        ...
        if qself.is_none() && let Some(partial_res) = self.resolver.get_partial_res(t.id)
            && let Some(Res::Def(DefKind::Trait | DefKind::TraitAlias, _)) = partial_res
                .full_res()
        {
            let (bounds, lifetime_bound) = self
                .with_dyn_type_scope(
                    true,
                    |this| {
                        let bound = this
    ...
    fn lower_ty_direct(&mut self, t: &Ty, itctx: ImplTraitContext) -> hir::Ty<'hir> {
        let kind = match &t.kind {
            ...
            TyKind::PinnedRef(region, mt) => {
                ...
                let args = self
                    .arena
                    .alloc(hir::GenericArgs {
                        args: self
                            .arena
                            .alloc([hir::GenericArg::Type(self.arena.alloc(arg))]),
                        constraints: &[],
                        parenthesized: hir::GenericArgsParentheses::No,
                        span_ext: span,
                    });
                let path = self
                    .make_lang_item_qpath(hir::LangItem::Pin, span, Some(args));
            ...
            TyKind::Tup(tys) => {
                hir::TyKind::Tup(
                    self
                        .arena
                        .alloc_from_iter(
                            tys.iter().map(|ty| self.lower_ty_direct(ty, itctx)),
                        ),
                )
            }
            TyKind::Paren(ty) => {
                    ...
                    hir::QPath::Resolved(
                        ...
                        self
                            ...
                            .alloc(hir::Path {
                                ...
                                segments: arena_vec![
                                    self;
                                    hir::PathSegment::new(Ident::with_dummy_span(kw::SelfUpper),
                                    hir_id, res)
                                ],
                                span: self.lower_span(t.span),
                            }),
            ...
            TyKind::Typeof(expr) => {
                hir::TyKind::Typeof(self.lower_anon_const_to_anon_const(expr))
            }
                ...
                let (bounds, lifetime_bound) = self
                    .with_dyn_type_scope(
                        ...
                        |this| {
                            ...
                            let lifetime_bound = lifetime_bound
                                .unwrap_or_else(|| this.elided_dyn_bound(t.span));
                            (bounds, lifetime_bound)
                        },
                    );
                ...
                match itctx {
                    ...
                    ImplTraitContext::Universal => {
                            ...
                            })
                        {
                            self.tcx
                                .dcx()
                                .emit_err(errors::NoPreciseCapturesOnApit {
                                    span,
                                });
                        }
                        ...
                        let (param, bounds, path) = self
                            .lower_universal_param_and_bounds(
                                *def_node_id,
                                span,
                                ident,
                                bounds,
    ...
    fn lower_ty_direct_lifetime(
        ...
        let (region, syntax) = match region {
            ...
            None => {
                let id = if let Some(LifetimeRes::ElidedAnchor { start, end }) = self
                    .resolver
                    .get_lifetime_res(t.id)
                {
    ...
    fn lower_opaque_impl_trait(
        ...
        let opaque_ty_span = self
            .mark_span_with_reason(DesugaringKind::OpaqueTy, span, None);
                ...
                precise_capturing_args
                    ...
                    .map(|arg| match arg {
                        PreciseCapturingArg::Lifetime(lt) => {
                            hir::PreciseCapturingArg::Lifetime(
                                self
                                    .lower_lifetime(
                                        lt,
                            ...
                            let res = self
                                ...
                                .map_or(
                                    ...
                                    |partial_res| {
                                        partial_res
                                            .full_res()
                                            .expect("no partial res expected for precise capture arg")
                                    },
                                );
            ...
            .alloc_from_iter(
                decl
                    ...
                    .map(|param| match param.pat.kind {
                        ...
                        PatKind::Wild => {
                            Some(
                                Ident::new(kw::Underscore, self.lower_span(param.pat.span)),
                            )
                        }
                        _ => {
                            self.dcx()
    ...
    fn lower_fn_decl(
                        ...
                        let itctx = match kind {
                            ...
                            FnDeclKind::ExternFn => {
                                ImplTraitContext::Disallowed(
                                    ImplTraitPosition::ExternFnParam,
                                )
                            }
                            FnDeclKind::Closure => {
                                ImplTraitContext::Disallowed(
                                    ImplTraitPosition::ClosureParam,
                                )
                            }
                            FnDeclKind::Pointer => {
                                ImplTraitContext::Disallowed(
                                    ImplTraitPosition::PointerParam,
                                )
                            }
                        };
        ...
        let output = match coro {
            Some(coro) => {
                ...
                self.lower_coroutine_fn_ret_ty(
                    &decl.output,
                    fn_def_id,
                    coro,
                    kind,
                ...
                match &decl.output {
                    FnRetTy::Ty(ty) => {
                        let itctx = match kind {
                            ...
                            FnDeclKind::ExternFn => {
                                ImplTraitContext::Disallowed(
                                    ImplTraitPosition::ExternFnReturn,
                                )
                            }
                            FnDeclKind::Closure => {
                                ImplTraitContext::Disallowed(
                                    ImplTraitPosition::ClosureReturn,
                                )
                            }
                            FnDeclKind::Pointer => {
                                ImplTraitContext::Disallowed(
                                    ImplTraitPosition::PointerReturn,
                                )
                            }
                        };
                    ...
                    .map_or(
                        ...
                        |arg| {
                            let is_mutable_pat = matches!(
                                arg.pat.kind, PatKind::Ident(hir::BindingMode(_,
                                Mutability::Mut), ..)
                            );
                            match &arg.ty.kind {
    ...
    fn lower_coroutine_fn_ret_ty(
        ...
        let opaque_ty_span = self
            .mark_span_with_reason(DesugaringKind::Async, span, allowed_features);
        let in_trait_or_impl = match fn_kind {
            ...
            FnDeclKind::ExternFn | FnDeclKind::Closure | FnDeclKind::Pointer => {
                unreachable!()
            }
        };
        let opaque_ty_ref = self
            .lower_opaque_inner(
                opaque_ty_node_id,
    ...
    fn lower_coroutine_fn_output_type_to_bound(
        ...
        let output_ty = match output {
            FnRetTy::Ty(ty) => self.lower_ty(ty, itctx),
            FnRetTy::Default(ret_ty_span) => {
                self.arena.alloc(self.ty_tup(*ret_ty_span, &[]))
            }
        };
        ...
        let bound_args = self
            ...
            .alloc(hir::GenericArgs {
                args: &[],
                constraints: arena_vec![
                    self; self.assoc_ty_binding(assoc_ty_name, opaque_ty_span, output_ty)
                ],
                parenthesized: hir::GenericArgsParentheses::No,
                span_ext: DUMMY_SP,
            });
    ...
    fn new_named_lifetime(
        ...
        let res = match res {
            ...
            LifetimeRes::Static { .. } => {
                assert!(
                    matches!(ident.name, kw::StaticLifetime | kw::UnderscoreLifetime)
                );
    ...
    fn lower_generic_param_kind(
        ...
        match &param.kind {
            GenericParamKind::Lifetime => {
                ...
                let param_name = if let Some(LifetimeRes::Error) = self
                    .resolver
                    .get_lifetime_res(param.id)
                {
            ...
            GenericParamKind::Type { default, .. } => {
                let default = default
                    ...
                    .filter(|_| match source {
                        ...
                        hir::GenericParamSource::Binder => {
                            self.dcx()
                                .emit_err(errors::GenericParamDefaultInBinder {
                                    span: param.span(),
                                });
                            false
                        }
                    ...
                    .map(|def| {
                        self.lower_ty(
                            def,
                            ImplTraitContext::Disallowed(
                                ImplTraitPosition::GenericDefault,
                            ),
                        )
                    });
    ...
    fn lower_poly_trait_ref(
        ...
        let bound_generic_params = self
            .lower_lifetime_binder(trait_ref.ref_id, bound_generic_params);
                ...
                let gate = |context, subject| {
                    ...
                    let mut diag = self
                        ...
                        .struct_span_err(
                            span,
                            format!(
                                "relaxed {prefix}bounds are not permitted in {context}"
                            ),
                        );
                    if is_sized {
                        diag.note(
            ...
            hir::QPath::Resolved(
                ...
                self
                    ...
                    .alloc(hir::Path {
                        ...
                        res,
                        segments: arena_vec![
                            self; hir::PathSegment::new(self.lower_ident(ident), hir_id,
                            res)
                        ],
                    }),
            ),
    ...
    fn lower_trait_bound_modifiers(
        ...
        let polarity = match modifiers.polarity {
            ...
            BoundPolarity::Negative(span) => {
                BoundPolarity::Negative(self.lower_span(span))
            }
