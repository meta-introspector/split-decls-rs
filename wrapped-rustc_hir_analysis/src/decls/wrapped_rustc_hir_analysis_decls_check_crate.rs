...
pub fn check_crate(tcx: TyCtxt<'_>) {
    ...
    tcx.sess
        .time(
            "emit_ast_lowering_delayed_lints",
            || {
                #[cfg(debug_assertions)]
                {
                    for owner_id in tcx.hir_crate_items(()).owners() {
                        if let Some(delayed_lints) = tcx
                            .opt_ast_lowering_delayed_lints(owner_id)
                        {
                            if !delayed_lints.lints.is_empty() {
                                assert!(
                                    tcx.hir_crate_items(()).delayed_lint_items().any(| i | i ==
                                    owner_id)
                                );
                            }
                        }
    ...
    tcx.par_hir_body_owners(|item_def_id| {
        ...
        match def_kind {
            ...
            DefKind::Const if !tcx
                .generics_of(item_def_id)
                .own_requires_monomorphization()
                && !find_attr!(
                    tcx.get_all_attrs(item_def_id), AttributeKind::TypeConst(_)
                ) => {
