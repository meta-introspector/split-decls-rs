...
macro_rules! Depcrate_asttokenizable {
() => {
// Module: crate::ast
// Provides: {"tokenizable"}
// Dependencies: {}
# [doc = " Utility macro for creating [Token] enums."] macro_rules ! tokenizable { ($ (# [doc = $ enum_doc : expr]) * enum $ name : ident { $ ($ (# [doc = $ arm_doc : expr]) * $ arm : ident ($ token : literal) ,) + }) => { $ (# [doc = $ enum_doc]) * # [derive (Clone , Copy , Debug , PartialEq)] pub (crate) enum $ name { $ (# [doc = " ```sh"] # [doc = $ token] # [doc = " ```"] # [doc = ""] $ (# [doc = $ arm_doc]) * $ arm) ,+ } impl self :: Token for $ name { fn token (& self) -> &'static str { match self { $ (Self ::$ arm => $ token) ,+ } } } } ; }
};
}
