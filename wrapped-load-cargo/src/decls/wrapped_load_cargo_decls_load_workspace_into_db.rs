...
pub fn load_workspace_into_db(
    ...
    let proc_macro_server = match &load_config.with_proc_macro_server {
        ProcMacroServerChoice::Sysroot => {
            ws.find_sysroot_proc_macro_srv()
                .map(|it| {
                    it.and_then(|it| {
                            ProcMacroClient::spawn(&it, extra_env).map_err(Into::into)
                        })
        ...
        ProcMacroServerChoice::Explicit(path) => {
            Some(
                ProcMacroClient::spawn(path, extra_env)
                    .map_err(|e| {
                        ProcMacroLoadingError::ProcMacroSrvError(
                            e.to_string().into_boxed_str(),
                        )
                    }),
            )
    ...
    match &proc_macro_server {
        Some(Ok(server)) => {
            tracing::info!(
                manifest =% ws.manifest_or_root(), path =% server.server_path(),
                "Proc-macro server started"
            )
        }
        Some(Err(e)) => {
            tracing::info!(
                manifest =% ws.manifest_or_root(), % e,
                "Failed to start proc-macro server"
            )
        }
        None => {
            tracing::info!(
                manifest =% ws.manifest_or_root(), "No proc-macro server started"
            )
        }
    }
    let (crate_graph, proc_macros) = ws
        .to_crate_graph(
            &mut |path: &AbsPath| {
    ...
    let proc_macros = {
        let proc_macro_server = match &proc_macro_server {
            ...
            None => {
                Err(
                    ProcMacroLoadingError::ProcMacroSrvError(
                        "proc-macro-srv is not running, workspace is missing a sysroot"
                            .into(),
                    ),
                )
            }
                        ...
                        .map_or_else(
                            ...
                            |(_, path)| {
                                proc_macro_server
                                    .as_ref()
                                    .map_err(Clone::clone)
                                    .and_then(|proc_macro_server| load_proc_macro(
                                        proc_macro_server,
                                        &path,
                                        &[],
                                    ))
