...
impl<'tcx> TraitChecker<'tcx, TyCtxt<'tcx>, DefId, Ty<'tcx>> for RustcTyCtxt<'tcx> {
    ...
    fn type_implements_trait(
        ...
        let predicates = [Obligation {
            cause: ObligationCause::new(
                DUMMY_SP,
                DefId::local(DefIndex::from_usize(0)).expect_local(),
                ObligationCauseCode::Misc,
            ),
            param_env,
            predicate: tcx.mk_predicate(ty::Binder::dummy(ty::PredicateKind::Clause(
                ty::ClauseKind::Trait(ty::TraitPredicate {
                    trait_ref: ty::TraitRef::new(tcx, trait_def_id, tcx.mk_args(&[adt_ty.into()])),
                    polarity: ty::PredicatePolarity::Positive,
                }),
            ))),
            recursion_depth: 0,
        }];
        let mut fulfill_cx: FulfillmentContext<'_, FulfillmentError<'tcx>> =
