...
use quote::quote;
use split_decls_types::{PatchSpec, SplitDeclsConfig, StringReplacement};
...
fn main() -> Result<()> {
    ...
    let config_path =
        PathBuf::from(std::env::var("CARGO_MANIFEST_DIR")?).join(".split-decls-config.toml");
    let config = SplitDeclsConfig::load_from_file(&config_path).context(format!(
        "Failed to load config from {}",
        config_path.display()
    ))?;
    let current_crate_name_for_patch = "wrapped_rustc_expand_base_lib_custom_macros".to_string();
    if let Some(patches_for_crate) = config.patches.get(&current_crate_name_for_patch) {
        for patch_spec in patches_for_crate {
            let patch_path =
                PathBuf::from(std::env::var("CARGO_MANIFEST_DIR")?).join(&patch_spec.path);
