...
impl<'tcx, C>
    CoreFixer<
        ...
        hir::ItemId,
        rustc_span::Span,
    > for TraitFixer<'tcx, C>
where
    C: ConfigTrait,
{
    fn new(tcx: TyCtxt<'tcx>, config: C) -> Self {
        Self {
            tcx,
            config,
            fixes: Vec::new(),
    ...
    fn check_item(&mut self, item: &'tcx hir::Item<'tcx>) {
        let real_hir_info = RealHirInfoItem(*item);
        for rule in self.config.get_rules() {
            match &rule.kind {
                RuleKind::AddDerive => {
                    if let ItemKind::Struct(..) | ItemKind::Enum(..) | ItemKind::Union(..) =
                        real_hir_info.get_item_kind()
                    {
                        if rule.apply_to.contains(
                            &format!("{:?}", real_hir_info.get_item_kind()).to_lowercase(),
                        ) {
                            if rule.condition == "always" {
                                let trait_name = rule.trait_name[0].clone();
                                if !RustcAttributeReaderTyCtxt(self.tcx).has_derive_attr(
                                    real_hir_info.get_owner_id().to_def_id(),
                                    &trait_name,
                                ) {
                ...
                RuleKind::AddImpl if rule.trait_name[0] == "Clone" => {
                    if let ItemKind::Struct(..) = real_hir_info.get_item_kind() {
                        if rule.condition == "all_fields_clone" {
                            ...
                            let adt_ty =
                                self.tcx.type_of(real_hir_info.get_owner_id()).instantiate(
                                    self.tcx,
                                    GenericArgs::identity_for_item(
                                        self.tcx,
                                        real_hir_info.get_owner_id().to_def_id(),
                                    ),
                                );
                            let trait_def_id =
                                RustcLangItemsTyCtxt(self.tcx).get_clone_trait_def_id();
                            if let Some(trait_def_id) = trait_def_id {
                                if RustcTraitCheckerTyCtxt(self.tcx).type_implements_trait(
                                    self.tcx,
                                    adt_ty,
                                    real_hir_info.get_owner_id().to_def_id(),
                                    trait_def_id,
