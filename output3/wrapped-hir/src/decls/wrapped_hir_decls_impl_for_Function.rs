...
impl Function {
    ...
    pub fn returns_impl_future(self, db: &dyn HirDatabase) -> bool {
        ...
        let Some(future_trait_id) = LangItem::Future
            .resolve_trait(db, self.ty(db).env.krate) else {
            return false;
        };
        let Some(sized_trait_id) = LangItem::Sized
            .resolve_trait(db, self.ty(db).env.krate) else {
            return false;
        };
        ...
        impl_traits
            ...
            .all(|t| t.is_auto(db)) && has_impl_future
    }
    ...
    pub fn is_main(self, db: &dyn HirDatabase) -> bool {
        db.attrs(self.id.into()).export_name() == Some(&sym::main)
            || self.module(db).is_crate_root()
                && db.function_signature(self.id).name == sym::main
    }
    ...
    pub fn is_unsafe_to_call(
        ...
        let (target_features, target_feature_is_safe_in_target) = caller
            .map(|caller| {
                let target_features = hir_ty::TargetFeatures::from_attrs(
                    &db.attrs(caller.id.into()),
                );
                let target_feature_is_safe_in_target = match &caller
                    .krate(db)
                    .id
                    .workspace_data(db)
                    .target
        ...
        matches!(
            ...
            target_feature_is_safe_in_target), hir_ty::Unsafety::Unsafe
        )
    }
    ...
    pub fn as_proc_macro(self, db: &dyn HirDatabase) -> Option<Macro> {
        ...
        if !(attrs.is_proc_macro() || attrs.is_proc_macro_attribute()
