...
// Macro to find code patterns in symbol map
macro_rules! findcode {
    ($pattern:expr, $symbol_map:expr) => {{
        let mut matches = Vec::new();
        for (symbol, _) in $symbol_map.iter() {
...
// Macro to wrap code with function exports
macro_rules! wrapcode {
    ($symbol:expr, $symbol_map:expr) => {{
        if let Some(entry) = $symbol_map.get($symbol) {
            if let Some(name) = entry.get("name").and_then(|n| n.as_str()) {
                println!("  📦 Wrapping: {} -> export_{}", $symbol, name);
                // Generate wrapper function
                let wrapper_name = format!("export_{}", name.replace("::", "_"));
                println!("    pub fn {}() {{ {}() }}", wrapper_name, $symbol);
            }
        }
    }};
}
...
fn wrap_item(item: &syn::Item) -> proc_macro2::TokenStream {
    match item {
        syn::Item::Mod(item_mod) => {
            let mod_name = &item_mod.ident;
            let content = if let Some((_, items)) = &item_mod.content {
                let wrapped_items: Vec<_> = items.iter().map(wrap_item).collect();
                quote::quote! { #(#wrapped_items)* }
            } else {
                quote::quote! {}
            };
            quote::quote! { mkmod!(#mod_name, { #content }) }
        }
        syn::Item::Use(item_use) => {
            quote::quote! { mkuse!(#item_use) }
        }
        syn::Item::Fn(item_fn) => {
            quote::quote! { mkitem!(mkfn!(#item_fn)) }
        }
        syn::Item::Struct(item_struct) => {
            quote::quote! { mkitem!(mkstruct!(#item_struct)) }
        }
        syn::Item::Enum(item_enum) => {
            quote::quote! { mkitem!(mkenum!(#item_enum)) }
        }
        syn::Item::Trait(item_trait) => {
            quote::quote! { mkitem!(mktrait!(#item_trait)) }
        }
        syn::Item::Impl(item_impl) => {
            quote::quote! { mkitem!(mkimpl!(#item_impl)) }
        }
        _ => quote::quote! { mkitem!(#item) }
    }
}
...
fn semantic_patch_content(content: &str, file_name: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let result = quote::quote! { #(#wrapped_items)* };
    Ok(result.to_string())
                continue;
            }
...
fn process_symbol_map_content(content: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Count crates, modules, and items
    let mut crate_count = std::collections::HashSet::new();
    let mut module_count = std::collections::HashSet::new();
    let mut item_count = 0;
    
    for (symbol_name, _) in symbol_map.iter() {
        item_count += 1;
        
        // Extract crate name (before first ::)
        if let Some(first_colon) = symbol_name.find("::") {
            let crate_name = &symbol_name[..first_colon];
            crate_count.insert(crate_name.to_string());
            
            // Extract module path (everything except last component)
            if let Some(last_colon) = symbol_name.rfind("::") {
                let module_path = &symbol_name[..last_colon];
                module_count.insert(module_path.to_string());
            }
        }
    }
    
    println!("📊 Found {} crates with {} modules with {} items", 
             crate_count.len(), module_count.len(), item_count);
             
    // Find and wrap all main routines
    println!("🔍 Finding main routines...");
    let main_routines = findcode!("main", &symbol_map);
    println!("📋 Found {} main routines", main_routines.len());
    
    // Generate exports file
    let mut exports_content = String::new();
    exports_content.push_str("// Auto-generated main routine exports\n");
    exports_content.push_str("// Generated by build.rs findcode!/wrapcode! macros\n\n");
    
    for main_routine in &main_routines {
        if let Some(entry) = symbol_map.get(main_routine) {
            if let Some(name) = entry.get("name").and_then(|n| n.as_str()) {
                println!("  🎯 {}", main_routine);
                
                // Clean up the symbol name for valid Rust function names
                let clean_symbol = main_routine
                    .replace("-", "_")
                    .replace("{", "_")
                    .replace("}", "_")
                    .replace(",", "_")
                ...
                let wrapper_name = format!("export_{}_{}", 
                    clean_name,
                    exports_content.lines().count() // Make unique
                );
                
                exports_content.push_str(&format!("pub fn {}() {{ /* {} */ }}\n", wrapper_name, clean_symbol));
            }
        }
    }
    
    fs::write("src/exports.rs", exports_content)?;
    println!("📝 Generated src/exports.rs with {} export functions", main_routines.len());
    
    // Resolve dependencies for each binary
    println!("🔍 Resolving dependencies for each binary...");
    resolve_binary_dependencies(&symbol_map)?;
...
fn resolve_binary_dependencies(symbol_map: &HashMap<String, Value>) -> Result<(), Box<dyn std::error::Error>> {
    // Find all binary entry points (main functions)
    let binaries: Vec<String> = symbol_map.keys()
        .filter(|k| k.contains("::main") && !k.contains("::main::"))
        .cloned()
        .collect();
    
    println!("📋 Found {} binary entry points", binaries.len());
    
    for binary in &binaries {
        println!("🎯 Resolving dependencies for: {}", binary);
        
        // Recursively resolve all dependencies
        let mut resolved_deps = HashSet::new();
        let mut to_resolve = vec![binary.clone()];
        
        while let Some(current) = to_resolve.pop() {
            if resolved_deps.contains(&current) {
                continue;
            }
            resolved_deps.insert(current.clone());
            
            if let Some(entry) = symbol_map.get(&current) {
                if let Some(deps) = entry.get("dependencies").and_then(|d| d.as_array()) {
                    for dep in deps {
                        if let Some(dep_str) = dep.as_str() {
                            to_resolve.push(dep_str.to_string());
                        }
                    }
                }
            }
        ...
        println!("  📊 Resolved {} total dependencies", resolved_deps.len());
        
        // Generate module tree for this binary
        generate_module_tree(binary, &resolved_deps, symbol_map)?;
    }
    
    Ok(())
}

fn generate_module_tree(binary: &str, deps: &HashSet<String>, symbol_map: &HashMap<String, Value>) -> Result<(), Box<dyn std::error::Error>> {
    let safe_name = binary.replace("::", "_").replace(".", "_");
    let output_path = format!("src/bin_trees/{}.rs", safe_name);
    
    // Create directory if it doesn't exist
    fs::create_dir_all("src/bin_trees")?;
    
    let mut tree_content = String::new();
    tree_content.push_str(&format!("// Auto-generated module tree for: {}\n", binary));
    tree_content.push_str(&format!("// Dependencies resolved: {}\n\n", deps.len()));
    
    // Generate include macros for each dependency
    let mut macro_id = 0;
    let mut dep_macros = HashMap::new();
        ...
        if let Some(entry) = symbol_map.get(dep) {
            if let Some(source_file) = entry.get("source_file").and_then(|s| s.as_str()) {
                ...
                if std::path::Path::new(&processed_path).exists() {
                    if let Ok(file_content) = fs::read_to_string(&processed_path) {
                        let macro_name = format!("include_dep_{}", macro_id);
    ...
    // Group dependencies by crate/module and generate module structure
    ...
    for dep in deps {
        ...
        if parts.len() >= 2 {
            ...
            crate_modules.entry(crate_name).or_default().push(dep.clone());
        }
    }
    
    // Generate module structure with macro calls
    for (crate_name, symbols) in &crate_modules {
        tree_content.push_str(&format!("pub mod {} {{\n", crate_name.replace("-", "_")));
        
        for symbol in symbols {
            if let Some(macro_name) = dep_macros.get(symbol) {
    ...
    tree_content.push_str(&format!("pub fn main() {{\n"));
    tree_content.push_str(&format!("    {}()\n", binary));
    tree_content.push_str("}\n");
    
    fs::write(&output_path, tree_content)?;
    println!("  📝 Generated module tree: {}", output_path);
