    ...
    T: rustc_type_ir::TypeFoldable<DbInterner<'db>> + Clone,
{
    ...
    impl<'db> FallibleTypeFolder<DbInterner<'db>> for ErrorReplacer<'db> {
        ...
        fn try_fold_ty(&mut self, t: Ty<'db>) -> Result<Ty<'db>, Self::Error> {
            ...
            #[cfg(not(debug_assertions))]
            let error = || Ok(
                Ty::new_error(self.interner, crate::next_solver::ErrorGuaranteed),
            );
            match t.kind() {
                TyKind::Error(_) => {
                    let var = rustc_type_ir::BoundVar::from_usize(self.vars.len());
                    self.vars
                        .push(CanonicalVarKind::Ty {
                ...
                TyKind::Bound(
                    BoundVarIndexKind::Bound(index),
                    _,
                ) if index > self.binder => error(),
        ...
        fn try_fold_const(&mut self, ct: Const<'db>) -> Result<Const<'db>, Self::Error> {
            ...
            match ct.kind() {
                ConstKind::Error(_) => {
                    ...
                    self.vars
                        .push(
                            CanonicalVarKind::Const(rustc_type_ir::UniverseIndex::ZERO),
                        );
        ...
        fn try_fold_region(
            ...
            match region.kind() {
                RegionKind::ReError(_) => {
                    ...
                    self.vars
                        .push(
                            CanonicalVarKind::Region(rustc_type_ir::UniverseIndex::ZERO),
                        );
                    Ok(
                        Region::new_bound(
                            self.interner,
