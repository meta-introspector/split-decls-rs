...
pub fn shebling(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        println!("------------------------------------"); #[allow(dead_code)] const
        SHELL_SCRIPT_CONTENT : & str = # script_content; #[allow(dead_code)] pub struct
        SheblingAnalysis { pub script_path : &'static str, pub content_len : usize, pub
        ast_dump : &'static str, } #[allow(dead_code)] pub const SHEBLING_ANALYSIS :
        SheblingAnalysis = SheblingAnalysis { script_path : # script_path, content_len :
        # script_content.len(), ast_dump : # parsed_ast_string, };
    };
    expanded.into()
}
