...
pub fn bash(input: TokenStream) -> TokenStream {
    ...
    let sat_check_code = if is_simulated_unsat {
        quote_spanned! {
            span =>
            compile_error!(format!("Bash script is UNSAT! Logical contradiction detected by SAT solver. (Simulated UNSAT for 'grep' and 'echo' together). Raw SAT result: {:?}",
            # sat_result));
        }
    } else {
        quote! {
            match # sat_result { Ok(Some(Assignment::Sat)) => {
            ...
            Err(e) => { compile_error!(format!("SAT solver encountered an error: {:?}",
            e)); }, }
        }
    };
    ...
    let has_pipeline_fact = if script_content.contains("|") {
        quote! {
            datalog_engine.add_fact("has_pipeline", & []);
        }
    } else {
        quote! {}
    };
    ...
    let datalog_query_result = datalog_engine
        .query("?- valid_script.")
        .expect("Failed to query Datalog engine.");
    ...
    let expanded = quote! {
        ...
        println!("------------------------------------"); # sat_check_code #
        datalog_check_code #[allow(dead_code)] const BASH_SNIPPET_CONTENT : & str = #
        script_content; #[allow(dead_code)] pub struct BashSnippetAnalysis { pub content
        : &'static str, pub ast_dump : &'static str, } #[allow(dead_code)] pub const
        BASH_SNIPPET_ANALYSIS : BashSnippetAnalysis = BashSnippetAnalysis { content : #
        script_content, ast_dump : # parsed_ast_string, };
    };
    expanded.into()
}
